...
| Permission-PDA creator | Program `admin` **or** a dedicated create-permission-PDA owner (different value on mainnet vs devnet) | Call `CreatePermissionPda` to grant an authority access to `InitializeWithPermission`. Since the 2026-09 upgrade the dedicated owner can also call `ClosePermissionPda`, so it both grants and revokes; before that upgrade revocation was admin-only. |

Creator-fee collection does not introduce a privileged role. `CollectCreatorFeePermissionless` accepts any payer, constrains the recipient owner to `PoolState.pool_creator`, and constrains both destinations to that creator's canonical ATAs. A caller can fund missing ATAs and trigger the sweep but cannot redirect the fees.
...
| `permission_pda_admin` | Program `admin` **or** a dedicated `permission_pda_admin` key (different value on mainnet vs devnet) | `CreatePermissionPda` / `ClosePermissionPda` — grant or revoke an authority's right to call `CreatePermissionedPool`. The grant is the mere existence of a `["permission", authority]` PDA; it does **not** confer any pool-mutation power. |
...
Holding a `Permission` PDA lets its authority call `CreatePermissionedPool` — creating additional pools for a pair that already has a canonical one, each at a distinct `seed_index`-derived address. The permission is scoped to pool creation only: it cannot move funds, change fees, or touch any existing pool. Revoking it (`ClosePermissionPda`) stops the authority from creating further pools but leaves already-created pools untouched.
...
|------|-------------------|----------------|
| Program upgrade | Squads multisig (3/4) | Deploy new program bytecode |
| Program admin | Shared Raydium admin | Create and update `GlobalConfig` accounts; operate other admin-only paths |
| Delegated config creator | Dedicated cluster-specific authority | Create a new `GlobalConfig`; cannot update an existing config |
| Platform admin | PDA seed authority for one `PlatformConfig` | Update platform settings and create or close that platform's `PlatformAllowConfig` accounts |
| Launch creator (per launch) | The launch's creator wallet | Claim bonding-curve creator fees; receives CPMM creator-fee rights only when the platform does not override `platform_cp_creator` |

The platform-admin allowlist is self-restricting: it limits which `GlobalConfig` accounts launches under that platform may use. It does not grant authority over other platforms or existing launch funds. Canonical delegated authority addresses live in [`reference/program-addresses`](/reference/program-addresses).
...
No Raydium program is currently frozen or slow-moving enough to be treated as an immutable alternative — AMM v4, historically the most static of them, was itself redeployed on 2026-07-22. The last-deployed slot for every program is public; read it before assuming any of them is dormant:

| Program | Last deployed (mainnet) |
|---------|--------------------------|
...
(From each program's `ProgramData` account; `solana program show <PROGRAM_ID>` reports the same slot.)
...
`AmmConfig` has no `admin` field — the program admin is a pubkey compiled into the program (`crate::admin::ID`), and the per-config operational authorities are `protocolOwner` / `fundOwner`. The BPF upgrade authority is a separate, loader-level concern; read it with `solana program show`.
...
- [Solana program & loader docs](https://solana.com/docs/core/programs) — the upgrade mechanism, and how revoking the upgrade authority makes a program immutable.
