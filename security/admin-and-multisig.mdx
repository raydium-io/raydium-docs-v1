...
| Permission-PDA creator | Program `admin` **or** a dedicated create-permission-PDA owner (different value on mainnet vs devnet) | Call `CreatePermissionPda` to grant an authority access to `InitializeWithPermission`. The dedicated owner cannot call `ClosePermissionPda`; revocation remains admin-only. |
