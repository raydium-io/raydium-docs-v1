...
- A new position is frozen only when it uses a V2 open path and an underlying vault mint's freeze authority matches CLMM's restricted-issuer list. That matching position cannot be transferred or have its token-account owner changed; all other new positions remain transferable.

### Raydium's posture

Position NFTs implement Metaplex's metadata standard; wallet apps that understand CLMM positions display them as liquidity positions rather than tradeable NFTs. Most major Solana wallets surface them specially as of 2026. Restricted-issuer freezing is targeted and does not protect ordinary transferable positions.
