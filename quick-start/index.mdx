...
- **A funded wallet.** 
  The swap script needs only its input amount plus a small fee/rent cushion (~0.02 SOL is comfortable). Creating a pool costs more — the CPMM script alone has a ~0.15 SOL one-time pool-creation fee before counting seed liquidity or rent. See each script's own Setup section for the current figure before funding your wallet.
