...
description: "Four-file TypeScript flow that creates a new CLMM pool, then opens an initial concentrated position. Run with npx tsx; mirrors the official raydium-sdk-V2-demo flow."
---

<Info>
  **What this does.** Creates a new CLMM pool at the fee tier of your choice, then opens an initial concentrated position. Two transactions across four files — `config.ts`, `createPool.ts`, `createPosition.ts`, and `utils.ts` — run with `npx tsx`. Code is lifted from the official demos in [`raydium-sdk-V2-demo/src/clmm`](https://github.com/raydium-io/raydium-sdk-V2-demo/tree/master/src/clmm) and adapted to run standalone.
</Info>
...
In addition to the deps in the quickstart, you need:
