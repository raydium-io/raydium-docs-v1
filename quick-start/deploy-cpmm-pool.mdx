...
You'll also need to **fund the wallet with the seed amounts** of both mints, plus enough SOL to cover the one-time pool-creation fee and account rent (~0.19 SOL total on mainnet — see [`reference/fee-comparison`](/reference/fee-comparison#creation-costs) for the rent/fee breakdown, or [`ray/protocol-fees`](/ray/protocol-fees#pool-creation-fees) for just the fee).

**Running this on devnet.** This script pins an explicit `programId: CREATE_CPMM_POOL_PROGRAM`, a named import that doesn't change when you switch `cluster` to `"devnet"`. Check the SDK's exports for a devnet equivalent — the same `DEVNET_PROGRAM_ID` pattern used in the [CLMM script](/quick-start/deploy-clmm-pool) — before pointing this at devnet.
...
- [`user-flows/choosing-a-pool-type`](/user-flows/choosing-a-pool-type) — should you have used CLMM instead?
