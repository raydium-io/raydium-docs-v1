...
|---------|---------------------|-------|
| **AMM v4** | `[b"amm authority"]` | **Tüm AMM v4 havuzları tarafından paylaşılan tek PDA.** Bump'ı her `AmmInfo` üzerinde `nonce` olarak yansıtılır. |
| **CPMM** | `[b"vault_and_lp_mint_auth_seed"]` | **Tüm CPMM havuzları tarafından paylaşılan tek PDA.** Havuz özgü tohum yok. |
| **CLMM** | *(yok — kasa yetkisi `pool_state`'in kendisidir)* | Havuz başına. Kasalar `[b"pool_vault", pool_state, mint]` konumundaki PDA'lardır; sahibi `pool_state`'tir ve onlar için `pool_state` imza atar. CLMM'de global bir kasa-yetkisi PDA'sı yoktur. |
| **Farm v3 / v5 / v6** | `[farmId]` | Çiftlik başına. PDA o çiftlik için bahis + ödül kasalarına sahiptir. |
| **LaunchLab** | `[b"vault_auth_seed"]` | **Tüm başlatmalar tarafından paylaşılan tek PDA.** İki ek global PDA — `[b"platform_fee_vault_auth_seed"]` ve `[b"creator_fee_vault_auth_seed"]` — toplu ücret kasalarının sahibidir. |

Bundan birkaç şey çıkar:

- AMM v4, CPMM ve LaunchLab için yetki PDA'sı **genel bir hesaptır** — o türün her havuzu veya başlatması bunu kullanır. CPMM'ye CPI yapıyorsanız buna havuz başına değil bir kez ihtiyacınız vardır.
- CLMM istisnadır: hiç yetki PDA'sı yoktur. Her kasa, havuz ve mint üzerine tohumlanmış bir PDA'dır ve onun için havuz hesabı imza atar.
- Çiftlik başına yetkililer için PDA'yı çiftlik kimliğinden türetirsiniz. SDK bunu `getPoolKeys` / `getFarmKeys` içinde yapar; doğrudan entegre oluyorsanız `findProgramAddressSync` ile türetirsiniz.
