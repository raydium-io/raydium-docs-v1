...
<Info>
  **Raydium Perps, Orderly Network üzerinde beyaz etiketli bir dağıtımdır.** Emir kitabı, eşleştirme motoru ve hesap durumunun tümü Orderly üzerinde yaşamaktadır. Raydium SDK v2 (`@raydium-io/raydium-sdk-v2`) perps'ı kapsamaz — programlı erişim için Orderly'nin REST + WebSocket API'sini doğrudan kullanın. Aşağıdaki kod parçacıkları en yaygın akışları gösterir; kanonik referans [Orderly API'sine giriş](https://orderly.network/docs/build-on-omnichain/introduction) belgesidir; [REST uç noktaları](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) ve [WebSocket akışları](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction) onun altında belgelenmiştir.
</Info>

<Info>
  **Sürüm başlığı.**

  - Arka uç: Orderly Network REST + WebSocket API ([belgeler](https://orderly.network/docs/build-on-omnichain/introduction))
  ...
  Orderly'nin API yüzeyi gelişmektedir; bu kod parçacıklarını üretime kopyalamadan önce [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction) adresini kontrol edin.
</Info>
...
- [Orderly Network geliştirici belgeleri](https://orderly.network/docs/build-on-omnichain/introduction) — yukarıda kullanılan API yüzeyi için kanonik referans. Raydium Perps bunu doğrudan tüketir.
- [Orderly'nin desteklenen zincirleri](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — Solana'nın durumunu ve zincir başına takas kasası adreslerini doğrular.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — aynı REST katmanının üzerinde topluluğa açık bir JS sarmalayıcısı. **Yayınlanmış hiçbir sürümü yoktur** ve belirtilmiş bir bakım politikası da yoktur; bu nedenle onu bir bağımlılık değil, referans kod olarak görün — yukarıdaki imzalama katmanı zaten kendiniz sahiplenebileceğiniz kadar küçüktür.
