...
SDK, görüntüleme düzeyindeki görünümü `raydium.api.fetchFarmInfoById({ ids })` üzerinde, ham hesap anahtarlarını — yetki, LP kasası, yuva başına ödül kasaları — `raydium.api.fetchFarmKeysById({ ids })` üzerinde döndürür. (`raydium.farm.getFarmById` diye bir şey yoktur.) Aynı veriler HTTP üzerinden `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>` adresinde de mevcuttur.
