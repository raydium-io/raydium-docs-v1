...
| # | Ad | W | S | Notlar |
|---|------|---|---|-------|
| 1 | `beneficiary` | W | S | `vesting_record.beneficiary` ile eşit olmalıdır. |
| 2 | `authority` |   |   | PDA `[b"vault_auth_seed"]`; kasa transferini imzalar. |
| 3 | `pool_state` | W |   | Yalnızca takvimin yeniden doğrulanması gerekiyorsa mutasyona uğratılır. |
| 4 | `vesting_record` | W |   | `claimed_amount` güncellenir. |
| 5 | `base_vault` | W |   | Havuzun base-token kasası; borçlandırılır. |
| 6 | `user_base_token` | W |   | Yararlanıcının base mint üzerindeki ATA'sı; kilidi açılan tokenları alır. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Havuzun base mint'i. |
| 8 | `base_token_program` |   |   | Base mint'in sahibi olan program — SPL Token veya Token-2022. |
| 9 | `system_program` |   |   | Yararlanıcı ATA'sının oluşturulması gerektiğinde onu finanse eder. |
| 10 | `associated_token_program` |   |   | Yararlanıcı ATA'sını oluşturur. |

<Warning>
  `system_program`, `associated_token_program`'dan **önce** gelir. Her ikisi de sabit adreslerle
  `Program<'info, _>` olarak tiplenmiştir; bu nedenle takas edilmiş bir çift adres kısıtını
  geçemez ve yararlanıcı talepte bulunamaz.
</Warning>

**Ön koşullar**

- `block_time >= pool_state.vesting_schedule.start_time` (aksi takdirde `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — mezuniyet zaten gerçekleşmiş olmalıdır. Bundan önce çağrı yapmak `PoolNotMigrated` ile geri döner.
...
`2027-09-28` de (`start_time` sonrası 90 gün), Benefisiyeri A `ClaimVestedToken` çağırır:
...
Altı ay sonra (`2028-03-26`, `start_time` sonrası 270 gün), A tekrar talep eder:
