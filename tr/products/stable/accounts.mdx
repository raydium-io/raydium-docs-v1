...
<Note>
  **Saf AMM.** Stable AMM tüm likiditeyi kendi vault'larında tutar ve OpenBook'a bağlı değildir. Yaşamının erken döneminde bir OpenBook pazar yapıcılığı yolu taşıyordu, ancak bu yol yıllardır hareketsiz kalmıştır ve [2026-06-22 yükseltmesi](/tr/reference/changelog) kalan kodu kaldırmıştır. Aşağıdaki `serum_*` pazar hesapları ve `amm_open_orders` bu nedenle **eski**: geriye dönük uyumluluk için eski düzen işlemlerinde hala görünebilirler, ancak program bunları doğrulamaz veya okumaz ve yeni düzen talimatları bunları tamamen çıkarır.
</Note>
...
pub struct Fees {
    ...
    pub trade_fee_numerator: u64,       // 0 (eski OpenBook alanı, kullanılmıyor)
    pub trade_fee_denominator: u64,     // 10_000
    pub pnl_numerator: u64,             // 12
    pub pnl_denominator: u64,           // 100 → ücretin %12'si = hacmin %0,0024'ü
...
- **`fees`** — AMM v4 ile özdeş *yapı*, farklı *değerler*: `swap_fee = 2 / 10_000` (%0,02), `pnl = 12 / 100`, `trade_fee = 0`. Bkz. [`products/stable/fees`](/tr/products/stable/fees).
...
Arama tablosu. Fiyat/miktar noktalarının büyük bir seyrek dizisi.
...
pub struct DataElement {
    pub x: u64,         // tablo X (örn., coin miktarı)
    pub y: u64,         // tablo Y (örn., pc miktarı)
    pub price: u64,     // (x, y)'deki fiyat
}
...
Bu tabloları oluşturan kurulum talimatları — `InitModelData` (hesabı oluşturdu) ve `UpdateModelData` (elemanları doldurdu, `valid_data_count` ayarladı) — **2026-06-22 yükseltmesinde kaldırıldı**. Mevcut havuzlardaki tablolar artık **sabitlenmiştir**. Çalışma zamanında, kalan çağrılabilir talimatlar hala bunları tüketir:
...
Tabloyu doldururken, yönetici bunları önceden ölçeklenmiş olarak belirtir. Program sıra düzenini zincir üzerinde doğrulamaz (hız için), bu nedenle yanlış sıralama yanlış tekliflere neden olur.
...
AMM v4 ile özdeştir. Swap/deposit/withdraw/crank'ın etkin olup olmadığını kontrol eder.
...
**`ModelDataInfo` büyüktür** (~1,2 MB, 50.000 eleman × eleman başına 24 bayt olduğundan). Bu nedenle bir Stable havuzu oluşturmak açık kira ve hesap ön tahsisini gerektirir. Raydium SDK'sı ve araçları bunu şeffaf bir şekilde işler; entegratörler nadiren manuel olarak tahsis etmek zorunda kalırlar.
...
- **Enterpolasyon tabloyu nasıl kullanır**: [`products/stable/math`](/tr/products/stable/math).
