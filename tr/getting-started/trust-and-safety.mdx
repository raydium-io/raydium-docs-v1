...
Solana'da iptal edilmesi gereken şey bir SPL-Token **delegesidir**: `Approve` komutu, token hesaplarınızdan birinde bir delege ve bir miktar belirtir, `Revoke` ise bunu temizler. Cüzdanınızın token onayları / bağlı uygulamalar görünümünü açın ve tanımadığınız her şeyi iptal edin; Phantom ve Solflare bunu gösterir. Terminalden ise token hesabı başına tek bir komuttur:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

Bu, gelecek boşaltma işlemlerini durdurur ancak zaten boşaltılan fonları kurtarmaz.

<Warning>
  [revoke.cash](https://revoke.cash) **yalnızca EVM içindir** — Ethereum ve 100'den fazla EVM zincirini (Solana'nın EVM katmanı olan Neon dahil) kapsar, ancak Solana'nın kendisini kapsamaz. Solana kullanıcılarını oraya yönlendirmeyin.
</Warning>
