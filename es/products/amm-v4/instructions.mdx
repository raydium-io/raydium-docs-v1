...
| 2 | `associated_token_program` |   |   | Se usa para crear el ATA de LP del creador. |
| 3 | `system_program` |   |   | |
| 4 | `rent` |   |   | Todavía se reenvía a los CPI `initialize_account` / `initialize_mint` — no lo omitas. |
| 5 | `amm` | W |   | PDA `AmmInfo` `[programId, market, "amm_associated_seed"]`; se crea aquí. |
| 6 | `amm_authority` |   |   | PDA `["amm authority"]`; el argumento `nonce` debe ser igual a su bump. |
| 7 | `lp_mint` | W |   | PDA `[programId, market, "lp_mint_associated_seed"]`; se crea aquí. |
| 8 | `coin_mint` |   |   | Debe ser distinto de `pc_mint` (`InvalidCoinMint`). |
| 9 | `pc_mint` |   |   | |
...
| 13 | `amm_config` |   |   | PDA de configuración a nivel de programa `["amm_config_account_seed"]`; `InvalidConfigAccount` si no coincide. |
| 14 | `create_fee_destination` | W |   | Debe ser igual a `config_feature::create_pool_fee_address`; en caso contrario, `InvalidFee`. |
| 15 | `market` |   |   | No se valida — se usa solo como semilla variable de cada PDA de arriba, y se almacena como campo de referencia. |
| 16 | `user_wallet` | W | S | Creador. Paga la renta, la comisión de creación de pool y el depósito inicial. |
| 17 | `user_token_coin` | W |   | |
| 18 | `user_token_pc` | W |   | |
| 19 | `user_token_lp` | W |   | ATA de LP del creador; lo crea el programa. |
...
<Note>
  **Dos diseños aceptados.** La lista de 19 cuentas de arriba es la recomendada. Por
  ...
  inserta un `amm_open_orders` ignorado en la posición 7 y un `market_program` ignorado en la
...
<Warning>
  **Los recuentos aceptados son 11, 14 o 15 — nada más.** El diseño heredado es de 14 cuentas (o
  15 con una cuenta final ignorada): la misma lista con un `amm_open_orders` ignorado en la
  posición 4, un `market` ignorado en la posición 9 y un `market_event_queue` ignorado añadido
  al final en la posición 14. Cualquier otro recuento revierte con `WrongAccountsNumber`.
</Warning>
...
<Warning>
  **`Withdraw` no es `Deposit` al revés.** Los recuentos aceptados son 11, o de 20 a 23. El
  diseño heredado de 20 cuentas coloca la cuenta de LP del usuario *antes* de las dos ATA
  receptoras e intercala cinco cuentas de mercado ignoradas:
  ...
  Las formas de 22 y 23 cuentas insertan dos cuentas de relleno ignoradas después de la
  posición 8. Cualquier otro recuento revierte con `WrongAccountsNumber`.
</Warning>
...
- `AmmStatus::from_u64(amm.status).swap_permission()` es verdadero — es decir, `status` es `1` (`Initialized`), `6` (`SwapOnly`) o `7` (`WaitingTrade`). `status` es un valor de enumeración, no una máscara de bits; ver [`products/amm-v4/accounts`](/es/products/amm-v4/accounts#status).
...
| El programa AMM v4 mismo (`AmmInfo`, `TargetOrders`, `AmmConfig`) | El programa debita la cuenta directamente hasta `rent.minimum_balance(data_len)`. |
...
| `Deposit` | + | + ambos | — |
| `Withdraw` | − | − ambos | — |
| `SwapBaseIn` / `SwapBaseInV2` | — | + entrada, − salida | — (se realiza más tarde) |
| `SwapBaseOut` / `SwapBaseOutV2` | — | + entrada, − salida | — (se realiza más tarde) |
