...
| 6 | `user_base_token` | W |   | ATA del beneficiario sobre el mint base; recibe los tokens desbloqueados. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Mint base del pool. |
| 8 | `base_token_program` |   |   | El programa que posee el mint base — SPL Token o Token-2022. |
| 9 | `system_program` |   |   | Financia el ATA del beneficiario cuando hay que crearlo. |
| 10 | `associated_token_program` |   |   | Crea el ATA del beneficiario. |

<Warning>
  `system_program` va **antes** de `associated_token_program`. Ambos son de tipo `Program<'info, _>`
  con direcciones fijas, así que un par intercambiado falla la restricción de dirección y el
  beneficiario no puede reclamar.
</Warning>

**Precondiciones**

- `block_time >= pool_state.vesting_schedule.start_time` (de lo contrario `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — la graduación ya debe haber ocurrido. Llamar antes de eso revierte con `PoolNotMigrated`.
...
En `2027-09-28` (90 días después de `start_time`), el Beneficiario A llama a `ClaimVestedToken`:
...
Seis meses después (`2028-03-26`, 270 días después de `start_time`), A reclama nuevamente:
