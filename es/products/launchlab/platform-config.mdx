...
`platform_scale + creator_scale + burn_scale` debe ser igual a `1_000_000`, **y `creator_scale` debe ser exactamente `0`** — el programa comprueba `require_eq!(creator_scale, 0)` tanto en `CreatePlatformConfig` como en la ruta de actualización de `MigrateNftInfo`, así que una división configurable es en realidad solo `platform_scale + burn_scale = 1_000_000`. El campo sobrevive únicamente para que las configuraciones anteriores al 2026-08-17 sigan decodificándose; en ellas, cualquier `creator_scale` que almacenaran se suma al LP bloqueado de la plataforma en la migración en lugar de ir al creador del token.

Ejemplos configurables:

- `(100_000, 0, 900_000)` — 90% LP quemado, 10% bloqueado para la plataforma.
- `(150_000, 0, 850_000)` — 85% quemado, 15% bloqueado para la plataforma.
