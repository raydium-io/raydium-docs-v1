...
1. **Decaimiento de referencia**. Si `now - last_update_timestamp >= filter_period`, la referencia de volatilidad decae. Ten en cuenta que ambos límites son `>=`, no `>`:
...
Los campos del layout son `tokenFeesOwedA` / `tokenFeesOwedB` (no `...Owed0` / `...Owed1`), y cada entrada de `rewardInfos` es `{ growthInsideLastX64, rewardAmountOwed }` — el mint de recompensa en sí proviene de los `rewardInfos` del pool, no de los de la posición.

`tokenFeesOwed*` y `rewardAmountOwed` son instantáneas del último momento en que se tocó la posición. Para ver los valores **actuales** (que reflejan el crecimiento desde entonces), simula un `DecreaseLiquidity` con liquidez cero, o recalcula a partir del `fee_growth_*` global y las dos instantáneas tick-outside.
