...
description: "Representación de sqrt-price, liquidity ↔ montos de tokens, el paso de swap de un solo tick, iteración multi-tick y contabilidad de crecimiento de comisiones."
---
...
<Info>
  Esta página es operativa: proporciona las fórmulas, convenciones de punto fijo y procedimientos utilizados por el programa CLMM. Para entender el razonamiento detrás de la curva de liquidez concentrada en sí — por qué `L = sqrt(x · y)` importa — consulta [`algorithms/clmm-math`](/es/algorithms/clmm-math). Esta página asume que ya has leído esa sección.
</Info>

## Representación de sqrt-price

CLMM almacena el precio como `sqrt_price_x64` — la raíz cuadrada del precio token1-por-token0, como un número de punto fijo Q64.64:
...
Las tres identidades provienen del invariante `x = L / sqrt_p`, `y = L · sqrt_p` que la liquidez concentrada satisface dentro de un rango.
...
Un swap procede en **pasos**. Cada paso ya sea (a) consume toda la entrada disponible dentro del rango de tick actual sin cruzar un tick, o (b) mueve el precio exactamente al siguiente tick inicializado.
...
- **¿Cabe toda la entrada?** Si la entrada restante (después de comisión) es menor que `Δamount0` para alcanzar `sqrt_t`, resuelve el nuevo `sqrt_c'` exactamente:

  $$
  \text{sqrt\_c}' = \frac{L \cdot \text{sqrt\_c}}{L + \Delta\text{input} \cdot \text{sqrt\_c}}
  $$

  (para un swap exacto-entrada `token0 → token1`). El swap se completa en este paso sin cruzar un tick.
...
Para la dirección opuesta (`token1 → token0`, precio bajando), las fórmulas tienen `sqrt_c` y `sqrt_t` intercambiados y la inversión en el otro lugar.
...
La porción LP se divide entre la liquidez actualmente dentro del rango actualizando el acumulador global de crecimiento de comisiones:

$$
\text{fee\_growth\_global}_{\text{in}} \mathrel{+}= \text{lp\_portion} \cdot \frac{2^{64}}{L}
$$

— es decir, se denomina en *comisiones por unidad de liquidez*, Q64.64, de modo que una posición de tamaño `L_i` que se mantuvo dentro del rango durante este swap posteriormente leerá `L_i · Δfee_growth_global / 2^{64}` tokens adeudados.
...
La parte complicada de la contabilidad de comisiones de CLMM: una posición gana comisiones solo mientras el precio del pool está **dentro** de su rango. El pool rastrea comisiones acumulativas globalmente; la posición necesita saber las comisiones acumulativas *mientras está dentro de su rango específico*.
...
Cuando el precio cruza un tick, el programa **invierte** el `fee_growth_outside` de ese tick:
...
Un `PersonalPositionState` almacena `fee_growth_inside_0_last_x64` y `fee_growth_inside_1_last_x64`: los valores de `fee_growth_inside` en la última vez que se tocó la posición.
...
Cada uno de los hasta 3 flujos de recompensas del pool utiliza la misma maquinaria de crecimiento-dentro, en su propio acumulador `reward_growth_global_x64`. En el momento de emisión:
...
y se paga mediante `DecreaseLiquidity` / `DecreaseLiquidityV2` — no hay instrucción independiente de cobro de recompensas. Consulta [`products/clmm/fees`](/es/products/clmm/fees).
...
Usuario: `SwapBaseInput` entrada exacta 1,000 token0.
...
es decir, `sqrt_c'` cae ligeramente **por debajo** de `sqrt_c`, que es la dirección correcta: un swap `token0 → token1` agrega token0 al pool y por lo tanto baja el precio `token1/token0`.
...
```
order_phase                  : u64    id de cohorte monótono
orders_amount                : u64    total de token de entrada en la cohorte actual (más nueva)
part_filled_orders_remaining : u64    entrada restante de la cohorte que el swap está llenando actualmente
unfilled_ratio_x64           : u128   relación de llenado Q64.64 para la cohorte parcialmente llena
```
...
fn match_limit_orders_at_tick(tick, swap_input_remaining, sqrt_p):
    ...
    if tick.orders_amount > 0:
        ...
        # Recurre con la cohorte recién promovida.
        return match_limit_orders_at_tick(tick, swap_input_remaining, sqrt_p)

    return  # el tick no tiene más órdenes limitadas
```
...
Se aplican dos reglas en cada swap, en orden:
...
`tick_spacing_index_reference` ($t_{\text{ref}}$) está en unidades de tick-spacing, no ticks crudos: $t_{\text{ref}} = \lfloor \text{tick\_current} / \text{tick\_spacing} \rfloor$.
...
- [`products/clmm/ticks-and-positions`](/es/products/clmm/ticks-and-positions) para cómo el mapa de ticks participa en el recorrido.
- [`products/clmm/fees`](/es/products/clmm/fees) para el lado de comisión/recompensa de las matemáticas en detalle.
- [`algorithms/clmm-math`](/es/algorithms/clmm-math) para las derivaciones detrás de `L = sqrt(x · y)` y las fórmulas de rango-vs-liquidez.
