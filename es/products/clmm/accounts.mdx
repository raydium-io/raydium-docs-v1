...
|---------|---------|----------------|
| `AmmConfig` | Nivel de comisión: tasa de comisión comercial, participación del protocolo, participación del fondo, espaciado de ticks por defecto. Compartido entre todos los pools en este nivel. | 1 (compartido) |
...
| `PersonalPositionState` | Uno por posición de LP. Almacena el rango, liquidez y último crecimiento de comisión/recompensa visto. Autoridad = propietario del NFT. | 1 por posición |
| NFT de posición mint | Mint con suministro 1, asociado con `PersonalPositionState`. Transferir el NFT transfiere la posición. | 1 por posición |
...
| `DynamicFeeConfig` | Conjunto de parámetros reutilizable para el mecanismo de comisión dinámica. Los pools creados vía `create_customizable_pool` pueden optar por participar. Administrado por admin. | compartido (por índice) |
...
- **`fee_growth_global_{0,1}_x64`** son las comisiones acumulativas ganadas por unidad de liquidez en todo el historial del pool. Las posiciones leen esto para calcular lo que se les debe.
- **`tick_spacing`** está bloqueado al `AmmConfig` en la inicialización y nunca cambia. Determina qué índices de tick se permiten como puntos finales de posición.
...
- **`fee_on`** se fija en la creación del pool. `0` (FromInput) reproduce el comportamiento clásico de Uniswap-V3. `1` y `2` enrutan la comisión de swap a un solo lado del libro — ver [`products/clmm/fees`](/es/products/clmm/fees) para compensaciones.
...
Un conjunto típico publicado de niveles de comisión CLMM (confirma contra `GET https://api-v3.raydium.io/main/clmm-config`):

| Índice | `trade_fee_rate` | Espaciado de ticks | Uso típico |
|-------|------------------|--------------|-------------|
...
CLMM no almacena un registro único por tick. Eso sería miles de millones de cuentas. En su lugar agrupa **`TICK_ARRAY_SIZE` ticks adyacentes inicializados o no** (típicamente 60 u 88 dependiendo de la versión del programa) en un `TickArrayState` que se crea perezosamente en el primer uso.
...
- Un array de ticks se inicializa perezosamente: la primera posición o swap que toca un array no inicializado lo crea, pagando la renta.
- Un array de ticks **nunca se cierra** por el programa. Una vez asignado persiste por la vida del pool, incluso después de que cada tick dentro de él vuelva a `liquidity_gross == 0`. Las posiciones y swaps posteriores reutilizan la cuenta existente sin renta adicional. No hay ruta de limpieza impulsada por `ClosePosition` para arrays de ticks.

### `TickArrayBitmapExtension`

`PoolState.tick_array_bitmap` (en línea) cubre el rango "cercano al spot" — ±1,024 arrays de ticks. Fuera de ese rango (para valores de tick extremos), el programa mantiene una cuenta de extensión:
...
Un mint de Token SPL con suministro 1. La dirección del mint es un PDA determinista; el NFT de posición en la billetera del propietario es solo un ATA que mantiene ese token único. Transferir el NFT es cómo una posición cambia de manos — el programa vincula la autorización al **titular actual del saldo ATA del NFT**, no a una Pubkey almacenada en estado.

### `PersonalPositionState`

Uno por posición abierta. Vinculado al mint del NFT.
...
Semilla PDA: `["dynamic_fee_config", index.to_be_bytes()]`. Creado vía `create_dynamic_fee_config` (protegido por admin) y modificado vía `update_dynamic_fee_config`. Un pool creado con `enable_dynamic_fee = true` captura los cinco parámetros de calibración del config (`filter_period`, `decay_period`, `reduction_factor`, `dynamic_fee_control`, `max_volatility_accumulator`) en su propio `DynamicFeeInfo` en el momento de creación; ediciones posteriores al `DynamicFeeConfig` no afectan retroactivamente a los pools existentes.
...
3. **Liquidar** — cuando la cohorte está completamente o parcialmente llena, el propietario *o* el operador de mantenimiento llama a `settle_limit_order` para empujar tokens de salida al ATA del propietario.
...
Contador por `(wallet, nonce_index)` que permite a un usuario único ejecutar múltiples pipelines paralelos de órdenes de límite sin colisionar en PDAs.
...
| `OpenPosition[WithToken22Nft]` | NFT mint + ATA, `personalPosition`, posiblemente nuevo(s) `tickArrayState`, `tickArrayBitmapExtension` si aún no existe | — |
| `IncreaseLiquidity` | Posiblemente nuevo(s) `tickArrayState` | — |
...
Las cuentas `TickArrayState` **nunca se cierran por el programa** — persisten por la vida del pool. Una vez que un array de ticks ha sido inicializado permanece en cadena incluso cuando cada tick dentro de él vuelve a `liquidity_gross == 0`. Reutilizar un array de ticks existente es gratis; solo la primera posición que toca un array nunca inicializado paga su renta.

## Qué leer dónde

- **Matemática de ticks y mecánica de rango**: [`products/clmm/ticks-and-positions`](/es/products/clmm/ticks-and-positions).
