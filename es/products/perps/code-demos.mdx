...
<Info>
  **Raydium Perps es un despliegue de etiqueta blanca en Orderly Network.** El libro de órdenes, el motor de emparejamiento y el estado de la cuenta residen en Orderly. El SDK de Raydium v2 (`@raydium-io/raydium-sdk-v2`) **no** cubre perps — para acceso programático, usa directamente la API REST + WebSocket de Orderly. Los fragmentos a continuación muestran los flujos más comunes; la referencia canónica es la [introducción a la API de Orderly](https://orderly.network/docs/build-on-omnichain/introduction), con los [endpoints REST](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) y los [streams de WebSocket](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction) documentados bajo ella.
</Info>

<Info>
  **Banner de versión.**

  - Backend: API REST + WebSocket de [Orderly Network](https://orderly.network/docs/build-on-omnichain/introduction)
  ...
  La superficie de la API de Orderly evoluciona; verifica [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction) antes de copiar estos fragmentos en producción.
</Info>
...
- [Documentación de desarrollador de Orderly Network](https://orderly.network/docs/build-on-omnichain/introduction) — referencia canónica para la superficie de la API utilizada anteriormente. Raydium Perps lo consume directamente.
- [Cadenas soportadas por Orderly](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — confirma el estado de Solana y las direcciones de la bóveda de liquidación por cadena.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — un envoltorio JS visible para la comunidad sobre la misma capa REST. **No tiene versiones publicadas** ni política de mantenimiento declarada, así que trátalo como código de referencia y no como una dependencia; la capa de firma anterior es lo bastante pequeña para mantenerla tú mismo.
