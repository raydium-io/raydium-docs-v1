...
description: "AmmInfo, ModelDataInfo, DataElement, Fees y diseño de bóvedas. El inventario completo de cuentas de un pool de Stable AMM (un programa Raydium independiente y puro de AMM)."
---
...
| Heredada | `amm_open_orders` | OpenBook | La antigua cuenta `OpenOrders` de OpenBook del pool. **No utilizada.** |
| Heredada | `serum_market` | OpenBook | Mercado de OpenBook. **No utilizado.** |
| Heredada | `serum_bids`, `serum_asks` | OpenBook | Colas de oferta/demanda. **No utilizadas.** |
| Heredada | `serum_event_queue` | OpenBook | Cola de eventos. **No utilizada.** |
| Heredada | `serum_coin_vault`, `serum_pc_vault` | SPL Token | Bóvedas a nivel de mercado de OpenBook. **No utilizadas.** |
| Heredada | `serum_vault_signer` | OpenBook | Firmante de bóveda a nivel de mercado. **No utilizado.** |

## `AmmInfo`

Cuenta de estado raíz. El diseño es casi idéntico a AMM v4 — parámetros del pool, decimales, comisiones, referencias de bóvedas/mints — con una adición: un campo `model_data_key` que apunta a la tabla de búsqueda.
...
pub struct Fees {
    ...
    pub trade_fee_numerator: u64,       // 0 (legacy OpenBook field, unused)
    pub trade_fee_denominator: u64,     // 10_000
    pub pnl_numerator: u64,             // 12
    pub pnl_denominator: u64,           // 100 → 12% of fee = 0.0024% of volume
    pub swap_fee_numerator: u64,        // 2
    pub swap_fee_denominator: u64,      // 10_000 → 0.02%
}
...
- **`fees`** — *estructura* idéntica a AMM v4, *valores* diferentes: `swap_fee = 2 / 10_000` (0,02%), `pnl = 12 / 100`, `trade_fee = 0`. Ver [`products/stable/fees`](/es/products/stable/fees).
- **`coin_vault`**, **`pc_vault`** — las bóvedas del pool.
- **`status`** — máscara de bits que controla swap/deposit/withdraw/crank.
- **`out_put.need_take_pnl_*`** — barridas por `WithdrawPnl`.

## `ModelDataInfo`

La tabla de búsqueda. Un gran arreglo disperso de puntos de precio/cantidad.
...
- **Swap / deposit / withdraw** llaman a funciones de búsqueda que buscan binariamente e interpolan dentro de `elements[0..valid_data_count]`.
...
- **`amm_authority`** es un PDA único a nivel de programa derivado con semilla `["amm authority"]`. Posee todas las bóvedas del pool y firma sus movimientos.
...
Idéntica a AMM v4. Controla si swap/deposit/withdraw/crank están habilitados.
...
**Cálculo de activos del pool (post-desacoplamiento).** Porque ya no hay fondos en depósito como órdenes abiertas de OpenBook, los activos totales del pool ahora se calculan completamente desde las bóvedas:
...
De manera similar para bóvedas, mint de LP, órdenes objetivo, etc. En la práctica, usa el SDK o API para obtener direcciones pre-calculadas.
