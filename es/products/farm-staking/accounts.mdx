...
El SDK devuelve la vista a nivel de visualización en `raydium.api.fetchFarmInfoById({ ids })` y las claves de cuenta sin procesar — autoridad, vault de LP, vaults de recompensa por slot — en `raydium.api.fetchFarmKeysById({ ids })`. (No existe `raydium.farm.getFarmById`.) Los mismos datos están disponibles por HTTP en `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
