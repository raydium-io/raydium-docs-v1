---
title: "Cuentas CPMM"
description: "Cada PDA y cuenta compartida que toca un pool CPMM: PoolState, AmmConfig, autoridad, mint de LP, bóvedas de tokens y el búfer de anillo de observación."
---
...
| `poolState` | `"pool"`, `ammConfig`, `token0Mint`, `token1Mint` **o** cualquier par de claves aleatorio proporcionado por el firmante | CPMM | La estructura de estado del pool — par de mints, saldos de bóveda, suministro de LP, acumulación de comisiones, puntero de observación. La instrucción `Initialize` de CPMM acepta tanto el PDA canónico derivado de las cuatro semillas **como** un par de claves arbitrario firmado por el creador. La ruta de par de claves aleatorio existe para derrotar un ataque de front-running donde un adversario observa el mempool y corre para ocupar el PDA canónico antes que el creador legítimo. |
...
| Cuenta | Semilla(s) | Propietario | Propósito |
|---------|---------|-------|---------|
| `ammConfig` | `"amm_config"`, `index: u16` | CPMM | Contiene las tasas de comisiones de comercio/protocolo/fondo/creador y claves de administrador. Una por "nivel de comisión". El poolstate se vincula a uno en la creación y no puede cambiar después. |
...
<Tip>
  **Siempre ordena los mints antes de derivar el PDA del pool.** La semilla hace hash de los dos mints en orden de bytes, no en orden del usuario. Dos pools con `(A, B)` y `(B, A)` colisionarían en la cadena — la ordenación es cómo el programa hace que el mapeo sea canónico.
</Tip>

<Note>
  **El ID del pool no siempre es el PDA canónico.** `Initialize` acepta un par de claves firmante arbitrario como `pool_state` además del PDA anterior. Si la cuenta pasada no coincide con el PDA canónico, el programa requiere que sea un firmante — es decir, el creador pasa un par de claves nuevo que firma. Esta es la defensa contra front-running: cualquier tercero que corra para agarrar el PDA canónico puede ser eludido por el creador legítimo usando un par de claves aleatorio en su lugar. Los PDAs posteriores (`lpMint`, `vault0`, `vault1`, `observation`) aún se derivan de `poolState.key()`, por lo que permanecen únicos a cualquier dirección que se haya usado. Cuando indexas pools, siempre descubre el ID del pool desde el estado en la cadena (por ejemplo, cuentas `PoolState` bajo el programa CPMM), no derivando el PDA canónico — este último perderá pools con par de claves aleatorio.
</Note>
...
- **`lp_supply`** — el total interno de LP del pool. **No** es igual al suministro del mint de LP: es exactamente 100 unidades base mayor, porque las 100 unidades bloqueadas se cuentan aquí pero nunca se acuñan. Toda la matemática de participación de LP (depósito, retiro) divide por `lp_supply`, así que usa este campo y no lo sustituyas por el suministro en cadena del mint.
- **`protocol_fees_token{0,1}`**, **`fund_fees_token{0,1}`** — comisiones *acumuladas* aún no barridas. Estas no afectan el precio del swap; se sientan en las bóvedas hasta que se llama a `CollectProtocolFee` / `CollectFundFee`.
...
- **`MetadataPointer`** y **`TokenMetadata`.** Metadatos estándar en mint. Sin efecto en las matemáticas de swap.
...
Cualquier otra extensión — `PermanentDelegate`, `TransferHook`, `DefaultAccountState`, `NonTransferable`, `ConfidentialTransfer`, `Group`/`GroupMember`, `MintCloseAuthority`, etc. — causa que `Initialize` rechace con `NotSupportMint`. La única excepción es un **registro por mint**: si existe un PDA `SupportMintAssociated` en la semilla `[b"support_mint", mint]`, el mint se admite independientemente de su conjunto de extensiones. Ese PDA se crea y se elimina por el administrador (o una autoridad de soporte de mint dedicada) a través de `CreateSupportMintAssociated` / `CloseSupportMintAssociated`, por lo que incorporar un mint específico ya no necesita una actualización del programa.

<Note>
  **Cambió en 2026-09.** CPMM anteriormente también llevaba una `MINT_WHITELIST` de cuatro direcciones codificada que cortocircuitaba la verificación de extensión. Ese array fue **removido**; el PDA de registro es ahora el único bypass. Cualquier mint que dependía de la lista codificada necesita un PDA `SupportMintAssociated` antes de que se pueda crear un nuevo pool para él — los pools existentes no se ven afectados, porque la verificación se ejecuta solo en la creación del pool.
</Note>

La lista de extensiones vetadas vive en la fuente de CP-Swap bajo `programs/cp-swap/src/utils/token.rs` y puede cambiar con futuras actualizaciones del programa. Ver [`reference/token-2022-support`](/es/reference/token-2022-support) para la matriz entre programas.
...
El búfer de anillo tiene tamaño para **100 observaciones**. Cada observación es de 40 bytes (`8 + 16 + 16`), por lo que el array solo son 4,000 bytes; `ObservationState::LEN` es exactamente **4,075 bytes** (`8 + 1 + 2 + 32 + 4,000 + 8 × 4`).
