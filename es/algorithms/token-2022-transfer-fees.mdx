...
<Info>
  Matriz de compatibilidad: **CPMM** soporta Token-2022 completamente, incluidos mints con transfer-fee. **CLMM** soporta Token-2022 con transfer-fee mediante cuentas `SwapV2` explícitas. **AMM v4** no soporta Token-2022 en absoluto. **LaunchLab** crea un mint base SPL clásico a través de `InitializeV2`, pero tiene un punto de entrada dedicado `InitializeWithToken2022` que crea un mint base Token-2022 y opcionalmente adjunta un `TransferFeeConfig`; su lado de cotización acepta cualquiera de los dos programas de token. Ver [`products/launchlab/platform-config`](/es/products/launchlab/platform-config#token-2022-transfer-fee-authorities). **Farm v6** soporta Token-2022 en mints de depósito y recompensa.
</Info>
...
≈ 986.03 USDC. Sin transfer-fee saliente, así que el usuario recibe ≈ 986.03 USDC.
...
- [Docs de extensión transfer-fee de SPL Token-2022](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
