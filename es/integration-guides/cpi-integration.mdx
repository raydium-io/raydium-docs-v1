...
description: "Patrones de composición para programas en cadena que invocan a Raydium — depósitos en garantía, proxies agregadores, bóvedas de auto-capitalización — con ejemplos completos que incluyen construcción de cuentas, firmantes PDA, cuentas restantes y presupuesto de cómputo."
---
...
| CLMM swap_v2 (cruza 2 ticks) | ~220.000 | ~270.000 |
| Farm v6 deposit | ~120.000 | ~150.000 |
| Farm v6 harvest (por ranura de recompensa) | +30.000 | +40.000 |
| AMM v4 swap_base_in | ~140.000 | n/a |

Añade ~1.500 por cada marco CPI y ~20.000 por la sobrecarga de tu propio programa. Un auto-capitalizador haciendo `harvest → swap A → swap B → deposit LP → stake LP` fácilmente alcanza 700k CU.
...
match cpi::swap_base_input(cpi_ctx, amount_in, minimum_amount_out) {
    ...
    Err(err) if is_err(err, CpmmErr::ExceededSlippage) => {
        // Tu programa podría querer reintentar con un slippage mayor, o deshacer estado.
...
pub mod limit_orders {
    ...
    pub fn execute_order(
        ...
        // re-verificamos nuestro propio delta post-intercambio en caso de que un cambio futuro alguna vez lo relaje.
