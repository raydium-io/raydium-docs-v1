...
En Solana lo que hay que revocar es un **delegado** de SPL-Token: la instrucción `Approve` designa un delegado y una cantidad en una de tus cuentas de token, y `Revoke` lo borra. Abre la vista de aprobaciones de token / aplicaciones conectadas de tu billetera y revoca todo lo que no reconozcas; tanto Phantom como Solflare lo muestran. Desde una terminal es un comando por cuenta de token:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

Esto detiene futuras transacciones de drenaje pero no recupera fondos ya drenados.

<Warning>
  [revoke.cash](https://revoke.cash) es **solo para EVM** — cubre Ethereum y más de 100 cadenas EVM (incluida Neon, la capa EVM de Solana) pero no Solana en sí. No envíes ahí a los usuarios de Solana.
</Warning>
