...
|------|-------------------|----------------|
| Actualización de programa | Multisig Squads (3/4) | Desplegar nuevo bytecode de programa |
...
|------|-------------------|----------------|
| Actualización de programa | Multisig Squads (3/4) | Desplegar nuevo bytecode de programa |
| Administrador de AmmConfig | Multisig de tesorería (3/5) | Crear nuevos AmmConfigs (niveles de comisión); alternar los existentes |
| Destinatario de comisión de creación de pool | Multisig de tesorería (3/5) | Recibir la comisión única de creación de pool |
| Creador de Permission-PDA | `admin` del programa **o** un propietario dedicado de crear-permission-PDA (valor diferente en mainnet vs devnet) | Llamar a `CreatePermissionPda` para otorgar a una autoridad acceso a `InitializeWithPermission`. El propietario dedicado no puede llamar a `ClosePermissionPda`; la revocación sigue siendo solo para administrador. |
...
| Administrador de DynamicFeeConfig | Multisig de tesorería (3/5) | `CreateDynamicFeeConfig` / `UpdateDynamicFeeConfig` — calibrar los niveles de comisión dinámica en los que una llamada `CreateCustomizablePool` puede optar |
| `permission_pda_admin` | `admin` del programa **o** una clave `permission_pda_admin` dedicada (valor diferente en mainnet vs devnet) | `CreatePermissionPda` / `ClosePermissionPda` — otorgar o revocar el derecho de una autoridad a llamar a `CreatePermissionedPool`. La concesión es la mera existencia de un PDA `["permission", authority]`; **no** confiere ningún poder de mutación de pool. |
...
|------|-------------------|----------------|
| Actualización de programa | Multisig Squads (3/4) | Desplegar nuevo bytecode de programa |
| Creador de farm (por farm) | Billetera del creador del farm | Financiar bóvedas de recompensa, extender cronogramas, recuperar recompensas no utilizadas |
...
| Creador de configuración delegada | Autoridad específica del cluster dedicada | Crear un nuevo `GlobalConfig`; no puede actualizar una configuración existente |
| Administrador de plataforma | Autoridad de seed de PDA para un `PlatformConfig` | Actualizar configuraciones de plataforma y crear o cerrar cuentas `PlatformAllowConfig` de esa plataforma |
| Creador de lanzamiento (por lanzamiento) | Billetera del creador del lanzamiento | Reclamar comisiones de creador de curva de vinculación; recibe derechos de comisión de creador de CPMM solo cuando la plataforma no anula `platform_cp_creator` |
...
Por qué 3/4: suficientes firmantes para que un único compromiso sea insuficiente; pocos suficientes para que coordinar una actualización legítima sea manejable. Las cuatro autoridades son firmantes independientes de dispositivos fríos aislados de la red mantenidos por miembros del equipo principal. La firma secuencial previene aprobaciones paralelas en la misma transacción; las transacciones llevan una ventana de expiración fija. Las operaciones de multisig se revisan periódicamente en asociación con el **Programa STRIDE de Solana (Asymmetric Research)**.
...
Cada creación de nuevo AmmConfig está permisionada — el multisig de tesorería 3/5 autoriza nuevos niveles de comisión y espacios de tick. Los pools existentes hacen referencia a su AmmConfig por PDA; el nivel de comisión del pool es lo que dice el AmmConfig.
...
| 2023-11 | CLMM | Se agregó segundo multisig para operaciones solo de recompensa para reducir exposición |
...
- [`security/disclosure`](/es/security/disclosure) — reportar problemas de administrador sospechosos.

Fuentes:

- [Protocolo Squads](https://v4.squads.so) — interfaz de usuario de multisig.
- [Documentos de BPF Loader de Solana](https://docs.solana.com/developing/runtime-facilities/programs#bpf-loader) — mecanismo de actualización.
