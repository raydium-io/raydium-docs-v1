...
1. **Solicitud de cotización (fuera de cadena).** La interfaz de usuario llama a `GET https://transaction-v1.raydium.io/compute/swap-base-in` con el mint de entrada, mint de salida, cantidad y tolerancia de deslizamiento. La API consulta su indexador, elige una ruta (posiblemente a través de múltiples pools) y devuelve una cotización más la lista de IDs de programa, IDs de pool y cuentas de tarifa que el cliente necesitará.
