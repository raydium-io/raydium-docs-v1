...
3. **Tick math is uniform.** Because ticks are defined as `1.0001^i`, `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` is also an exact geometric ladder. (`√1.0001 = 1.00004999875…`, so treating it as exactly `1.00005` drifts by up to ~0.6% at `MAX_TICK`.) Each tick-cross translates to a small multiplication in `sqrt_price_x64` space.
...
- **Oracle.** Raydium's `ObservationState` ring buffer stores only `(block_timestamp: u32, tick_cumulative: i64)` per slot plus `[u64; 4]` of reserved padding — there is **no** `seconds_per_liquidity_cumulative` field, so Uniswap's liquidity-weighted derivations are not available on-chain.
