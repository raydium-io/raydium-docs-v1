...
- 24h volume: \$120M
- Fee tier: 0.05% (LP share **84%** of fees — every mainnet CLMM `AmmConfig` carries `protocol_fee_rate = 12%` plus `fund_fee_rate = 4%`)
- Total TVL: \$40M
...
The Raydium UI shows **102% APR** for the pool. (This example uses a deep, high-volume pool to keep the numbers concrete; typical CLMM pools display anywhere from 10% to 50% APR, with low-volume pools well under 10%.)
...
Raydium's SDK exposes `PoolUtils.estimateAprsForPriceRangeMultiplier` and `PoolUtils.estimateAprsForPriceRangeDelta` for this estimation.
...
The helper returns `{ feeApr, rewardsApr[], apr }` — `rewardsApr` is per reward stream and `apr` is the total. There is no `raydium.clmm.estimatedApr` method. `estimateAprsForPriceRangeDelta` is the same computation parameterized by a ± delta around spot instead of explicit ticks.
...
- **"APR is compounded automatically."** No. Fees must be claimed — via a zero-liquidity `DecreaseLiquidity`, since CLMM has no standalone collect instruction — then manually redeployed. Raydium does not auto-compound CLMM fees.
...
- Raydium SDK v2 `PoolUtils.estimateAprsForPriceRangeMultiplier` / `estimateAprsForPriceRangeDelta` implementations.
