...
Same curve math as CPMM. AMM v4's OpenBook integration is deactivated, so reserves are just the vault balances — quote impact directly off the vaults. (The old `vault + on_book.free + on_book.locked` reserve sum no longer applies.)
