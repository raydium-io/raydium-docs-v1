...
The SDK computes these per product — the three do **not** share one signature:
...
Across all three, `minAmountOut` is `amountOut × (1 − slippage)` and is what goes on-chain as the bound; `priceImpact` is deterministic from pool state alone; `fee` is the total charged.
...
Same curve math as CPMM. Since the OpenBook removal, the "effective reserves" are just the two vault balances:

- Quote off the raw vault balances. There is no on-book component to add — `Initialize2` writes `AmmInfo.open_orders = Pubkey::default()` on every new pool, and no instruction reads an `OpenOrders` account.
- Subtract the accrued protocol PnL (`state_data.need_take_pnl_coin` / `need_take_pnl_pc`) from the vault balances to get the reserves the invariant actually uses.
- There is no crank to pre-run: `MonitorStep` now panics with `unimplemented!` and must not be sent.
...
The SDK's router applies per-hop bounds automatically when you call `raydium.tradeV2.swap`. (The facade is `tradeV2` — `raydium.trade` does not exist.) For custom routers, replicate the pattern.
