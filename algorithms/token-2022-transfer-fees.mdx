...
<Info>
  Support matrix: **CPMM** supports Token-2022 fully, including transfer-fee mints. **CLMM** supports Token-2022 with transfer fees via explicit `SwapV2` accounts. **AMM v4** does not support Token-2022 at all. **LaunchLab** creates a classic SPL base mint through `InitializeV2`, but has a dedicated `InitializeWithToken2022` entrypoint that creates a Token-2022 base mint and optionally attaches a `TransferFeeConfig`; its quote side accepts either token program. See [`products/launchlab/platform-config`](/products/launchlab/platform-config#token-2022-transfer-fee-authorities). **Farm v6** supports Token-2022 on both staking and reward mints.
</Info>
...
≈ 986.03 USDC. No outbound transfer fee, so the user receives ≈ 986.03 USDC.
