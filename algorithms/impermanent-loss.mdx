...
Assume you deposit \$10,000 of SOL/USDC into a CLMM at SOL = \$160, range `[$120, $200]`. Note the deposit is **not** 50/50: at that price in that range the position takes 27.54 SOL (\$4,407) and \$5,593 USDC, which fixes `L ≈ 3,300`. The HODL benchmark below is a 50/50 \$5,000 + \$5,000 hold, matching the CPMM comparison.

- Range width: `P_hi / P_lo = 200 / 120 ≈ 1.67`. Leverage factor ≈ `1 / (1 − √(120/200)) ≈ 4.4×`.
- If SOL moves to \$180 (+12.5%), HODL value = \$10,625; CLMM position ≈ \$10,397; **IL ≈ 2.1%**.
- If SOL moves to \$200 (+25%), HODL value = \$11,250; CLMM position is now 100% USDC worth ≈ \$10,520; **IL ≈ 6.5%**.
- If SOL moves to \$240 (+50%, outside range), HODL value = \$12,500; CLMM position is still ≈ \$10,520; **IL ≈ 15.8%**.

The same capital in a CPMM (no range) would have IL of only ~2.02% at \$240 and would continue earning fees. That gap is the amplification this page opens with: roughly the leverage factor times the CPMM IL while in range. The CLMM position has higher fee capture *while in range* but much worse IL once out of range for an extended period.
...
- [`sdk-api/typescript-sdk`](/sdk-api/typescript-sdk) — `raydium.clmm.getOwnerPositionInfo({ programId })` plus `PoolUtils.getAmountsFromLiquidity(...)` return current exposure; diff against your entry basis.
...
- [`user-flows/choosing-a-pool-type`](/user-flows/choosing-a-pool-type) — LP decision guide that uses this page's break-even logic.
