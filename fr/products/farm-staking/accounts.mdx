...
Le SDK retourne la vue au niveau de l'affichage sur `raydium.api.fetchFarmInfoById({ ids })` et les clés de comptes brutes — autorité, coffre LP, coffres de récompense par emplacement — sur `raydium.api.fetchFarmKeysById({ ids })`. (Il n'existe pas de `raydium.farm.getFarmById`.) Les mêmes données sont disponibles en HTTP à `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
