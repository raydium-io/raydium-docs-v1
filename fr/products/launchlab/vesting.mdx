...
| 6 | `user_base_token` | W |   | ATA du bénéficiaire sur le mint de base ; reçoit les jetons débloqués. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Mint de jeton de base du pool. |
| 8 | `base_token_program` |   |   | Le programme qui possède le mint de base — SPL Token ou Token-2022. |
| 9 | `system_program` |   |   | Finance l'ATA du bénéficiaire lorsqu'elle doit être créée. |
| 10 | `associated_token_program` |   |   | Crée l'ATA du bénéficiaire. |

<Warning>
  `system_program` vient **avant** `associated_token_program`. Les deux sont typés
  `Program<'info, _>` avec des adresses fixes, donc une paire inversée échoue à la contrainte
  d'adresse et le bénéficiaire ne peut pas réclamer.
</Warning>

**Conditions préalables**

- `block_time >= pool_state.vesting_schedule.start_time` (sinon `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — la graduation doit avoir déjà eu lieu. Un appel avant cela revient avec `PoolNotMigrated`.
...
Le `2027-09-28` (90 jours après `start_time`), le bénéficiaire A appelle `ClaimVestedToken` :
...
Six mois plus tard (`2028-03-26`, 270 jours après `start_time`), A réclame à nouveau :
