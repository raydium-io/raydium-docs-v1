...
`platform_scale + creator_scale + burn_scale` doit égaler `1_000_000`, **et `creator_scale` doit valoir exactement `0`** — le programme applique `require_eq!(creator_scale, 0)` à la fois sur `CreatePlatformConfig` et sur le chemin de mise à jour `MigrateNftInfo`, de sorte qu'une répartition réellement configurable se réduit à `platform_scale + burn_scale = 1_000_000`. Le champ ne survit que pour que les configurations antérieures au 2026-08-17 se décodent encore ; sur celles-ci, quel que soit le `creator_scale` qu'elles ont stocké, il est intégré au LP verrouillé de la plateforme lors de la migration au lieu d'aller au créateur du token.

Exemples configurables :

- `(100_000, 0, 900_000)` — 90 % du LP brûlé, 10 % verrouillé pour la plateforme.
- `(150_000, 0, 850_000)` — 85 % brûlé, 15 % verrouillé pour la plateforme.
