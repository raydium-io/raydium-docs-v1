---
title: "Démonstrations de code pour Stable AMM"
description: "Comment identifier un pool Stable AMM via l'API, effectuer un swap via le module Liquidity (il traite nativement les pools version=5), et utiliser les assistants de courbe stable hors chaîne."
---
...
Si la paire de mints a à la fois un pool v4 (constant-product) et un pool v5 (stable), la réponse inclut les deux — choisissez celui dont vous avez besoin, ou confiez-les au programme [AMM Routing](/fr/products/routing) et laissez-le choisir la meilleure route.
...
Si vous ne voulez pas choisir vous-même une venue, le programme [AMM Routing](/fr/products/routing) considérera chaque AMM Raydium (v4 / CPMM / CLMM / Stable) et routera à travers quelle que soit la meilleure combinaison :
...
1. **Pour les swaps des utilisateurs finaux**, préférez le flux de routage [`tradeV2`](/fr/products/routing). Il traite tous les types de pools Raydium y compris Stable.
...
- [Math](/fr/products/stable/math) — comment fonctionne l'interpolation de table de consultation.
...
- [`raydium-sdk-V2/src/raydium/liquidity/liquidity.ts`](https://github.com/raydium-io/raydium-sdk-V2/blob/master/src/raydium/liquidity/liquidity.ts) — point d'entrée du module ; répartition v4 / v5.
