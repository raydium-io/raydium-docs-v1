...
<Info>
  Stable AMM est son propre programme ; sa structure de comptes côté pool ressemble à AMM v4 (AmmInfo, vaults, autorité), et elle dispose en outre d'un compte `ModelDataInfo` qui stocke la table de consultation. Cette page couvre les deux.
</Info>
...
Compte d'état racine. La disposition est presque identique à AMM v4 — paramètres du pool, décimales, frais, références aux vaults/mints — avec un ajout : un champ `model_data_key` pointant vers la table de consultation.
...
- **`model_data_key`** — l'adresse de la table de consultation. Doit être passée à chaque instruction.
- **`fees`** — *structure* identique à AMM v4, *valeurs* différentes : `swap_fee = 2 / 10_000` (0,02 %), `pnl = 12 / 100`, `trade_fee = 0`. Voir [`products/stable/fees`](/fr/products/stable/fees).
- **`coin_vault`**, **`pc_vault`** — les vaults du pool.
- **`status`** — bitmask contrôlant swap/deposit/withdraw/crank.
...
Les instructions de configuration qui ont construit ces tables — `InitModelData` (créé le compte) et `UpdateModelData` (éléments peuplés, définition de `valid_data_count`) — ont été **supprimées dans la mise à jour du 2026-06-22**. Les tables sur les pools existants sont maintenant **fixes**. À l'exécution, les instructions appelables restantes les consomment toujours :

- **Swap / deposit / withdraw** appellent des fonctions de consultation qui font une recherche binaire et interpolent dans `elements[0..valid_data_count]`.
...
Identique à AMM v4. Contrôle si swap/deposit/withdraw/crank sont activés.
...
**Calcul des actifs du pool (post-découplage).** Parce qu'aucun fonds n'est plus séquestré en tant que commandes ouvertes OpenBook, les actifs totaux du pool sont maintenant calculés entièrement à partir des vaults :
