...
L'`AmmConfig` d'un pool fixe un **espacement des ticks** — les seuls ticks qu'une position est autorisée à utiliser comme points d'extrémité. Si `tick_spacing = 60`, seuls les ticks `…, −120, −60, 0, 60, 120, …` sont valides. Une tentative d'ouverture d'une position avec le point d'extrémité `31` revient avec `TickAndSpacingNotMatch`.
...
Un tick array est lazy : la **première** position qui référence un tick à l'intérieur initialise le tableau, en payant le loyer. Les swaps n'initialisent pas les tick arrays — ils contournent les tableaux non initialisés en utilisant le bitmap. Il n'existe pas d'instruction distincte d'initialisation de tick array : `OpenPosition*` et `IncreaseLiquidity*` appellent `TickArrayState::get_or_create_tick_array` en interne et allouent un tableau manquant aux frais du `payer`. Vous passez simplement la PDA de tick array — même lorsqu'elle est encore non initialisée et détenue par le système.
...
Trouver « le prochain tick initialisé à gauche/droite du tick actuel » doit être rapide — un swap peut traverser de nombreux ticks. Le pool stocke un bitmap 1-bit-par-tick-array en ligne dans `PoolState` pour la plage de **±512** tableaux autour du tick 0 (`TICK_ARRAY_BITMAP_SIZE = 512` ; le champ est un `[u64; 16]` = 1 024 bits, répartis en 512 négatifs / 512 positifs, soit des index de tableau de −512 … +511). En dehors de cette plage (positions full-range, configurations exotiques), `TickArrayBitmapExtension` fournit le débordement.
...
4. L'appelant a fourni les PDAs de tick array contenant `tick_lower` et `tick_upper`. Elles n'ont pas besoin d'être initialisées — le programme crée lui-même celle qui manque.
5. Le compte d'extension bitmap, si cette position s'étend dans la plage d'extension.

Si l'une des vérifications échoue, l'instruction est annulée avec `TickInvalidOrder` (règle 1), `TickAndSpacingNotMatch` (règle 2), `InvalidTickIndex` (règle 3), ou `MissingTickArrayBitmapExtensionAccount` (règle 5) ; `NotApproved` couvre le cas distinct où `pool_state.status` a le bit d'ouverture de position positionné. Voir [`reference/error-codes`](/fr/reference/error-codes).
...
- Détient 100 % d'*un seul* token. Si `tick_current < tick_lower` — prix **en dessous** de la plage — la position est entièrement en **token0** ; si `tick_current >= tick_upper` — prix **au-dessus** de la plage — elle est entièrement en **token1**. Le prix est coté `token1/token0`, donc une position traversée par un prix qui monte finit par détenir l'actif de cotation.
...
- **Points d'extrémité hors espacement.** Le code qui calcule un tick à partir d'un prix cible doit s'aligner sur un multiple de `tick_spacing` avant de le passer à `OpenPosition`, sinon l'instruction est annulée avec `TickAndSpacingNotMatch`. L'assistant du SDK est `TickUtil.getPriceAndTick({ price, mintADecimals, mintBDecimals, zeroForOne, tickSpacing })` (à noter : `TickUtil`, au singulier) ; les mathématiques maison sautent souvent cet alignement.
- **Tick arrays manquants.** L'ouverture d'une position large touche deux tick arrays ; oublier de les passer en tant que comptes inscriptibles annule l'instruction. Ils n'ont pas besoin d'exister au préalable — le programme crée celui qui manque — mais le *compte* doit figurer dans la liste.
