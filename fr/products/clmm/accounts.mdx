...
`PoolState.tick_array_bitmap` (inline) couvre la plage « proche du spot » — **±512** tableaux de ticks (`TICK_ARRAY_BITMAP_SIZE = 512` ; le champ `[u64; 16]` compte 1 024 bits répartis à parts égales entre les index de début négatifs et positifs, soit des index de tableau de −512 … +511). En dehors de cette plage (pour les valeurs de ticks extrêmes), le programme maintient un compte d'extension :
