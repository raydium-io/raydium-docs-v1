...
|---------|---------|----------------|
| `AmmConfig` | Tier de frais : taux de frais commerciaux, part du protocole, part du fonds, espacement des ticks par défaut. Partagé entre tous les pools de ce tier. | 1 (partagé) |
...
| `TickArrayBitmapExtension` | Bitmap de débordement suivant les tick arrays qui existent au-delà du bitmap inline dans `PoolState`. | 0 ou 1 |
| `PersonalPositionState` | Un par position LP. Stocke la plage, la liquidité, et la dernière croissance des frais/récompenses vue. Autorité = propriétaire du NFT. | 1 par position |
| Mint du NFT de position | Mint avec supply 1, associé à `PersonalPositionState`. Transférer transfère la position. | 1 par position |
| `ObservationState` | Ring buffer d'observations de prix pour le TWAP. | 1 |
| `token_0_vault`, `token_1_vault` | Comptes de tokens contenant les soldes du pool. Possédés par l'autorité du pool. | 2 |
...
| `LimitOrderState` | Un par ordre limite ouvert. Enregistre le propriétaire, le tick, le côté, le montant total, snapshot de la sortie réglée. | 1 par ordre |
...
Champs que vous allez réellement utiliser :
...
- **`fee_growth_global_{0,1}_x64`** sont les frais cumulatifs gagnés par unité de liquidité sur l'historique entier du pool. Les positions lisent ceci pour calculer ce qui leur est dû.
- **`tick_spacing`** est verrouillé sur `AmmConfig` à l'initialisation et ne change jamais. Il détermine quels indices de ticks sont autorisés à être des points de terminaison de position.
- **`tick_array_bitmap`** est un bitmap **inline** couvrant la plage couramment utilisée autour du prix spot. Pour les pools dont les positions s'étendent loin, le suivi du débordement vit dans le compte séparé `TickArrayBitmapExtension`.
...
| Index | `trade_fee_rate` | Espacement des ticks | Utilisation typique |
|-------|------------------|--------------|-------------|
...
CLMM ne stocke pas un seul enregistrement par tick. Ce serait des milliards de comptes. Au lieu de cela, il groupe **`TICK_ARRAY_SIZE` ticks adjacents initialisés ou non** (généralement 60 ou 88 selon la version du programme) dans un `TickArrayState` qui est créé paresseusement à la première utilisation.
...
Les quatre champs d'ordre limite sont zéro sur tout tick qui n'a jamais été utilisé pour un ordre limite. Lorsque des ordres sont ouverts sur un tick, le programme les suit comme une séquence de cohortes :
...
- Un tableau de ticks est **jamais fermé** par le programme. Une fois alloué, il persiste pour la durée de vie du pool, même après que chaque tick à l'intérieur revienne à `liquidity_gross == 0`. Les positions et swaps ultérieurs réutilisent le compte existant sans loyer supplémentaire. Il n'y a pas de chemin de nettoyage piloté par `ClosePosition` pour les tableaux de ticks.
...
Un mint SPL Token avec supply 1. L'adresse du mint est un PDA déterministe ; le NFT de position dans le portefeuille du propriétaire est juste un ATA contenant ce seul token. Transférer le NFT est comment une position change de mains — le programme lie l'autorisation au **détenteur actuel du solde ATA du NFT**, pas à une Pubkey stockée dans l'état.

### `PersonalPositionState`

Un par position ouverte. Clé sur le mint du NFT.
...
<Note>
  Les versions plus anciennes de CLMM stockaient la comptabilité agrégée par `(pool, tick_lower, tick_upper)` dans un PDA `ProtocolPositionState`. **Les versions plus récentes ne créent ni ne lisent plus ce compte.** L'emplacement apparaît toujours sur les listes de comptes `OpenPosition` / `IncreaseLiquidity` / `DecreaseLiquidity` comme un `UncheckedAccount` pour la compatibilité ABI, mais le programme n'y écrit pas. Les comptes existants on-chain sont vestigiaux ; l'admin peut appeler `CloseProtocolPosition` pour récupérer le loyer pour eux.
...
Seed PDA : `["dynamic_fee_config", index.to_be_bytes()]`. Créé via `create_dynamic_fee_config` (gated par admin) et modifié via `update_dynamic_fee_config`. Un pool créé avec `enable_dynamic_fee = true` capture les cinq paramètres de calibrage du config (`filter_period`, `decay_period`, `reduction_factor`, `dynamic_fee_control`, `max_volatility_accumulator`) dans son propre `DynamicFeeInfo` au moment de la création ; les modifications ultérieures à `DynamicFeeConfig` n'affectent pas rétroactivement les pools existants.
...
3. **Règlement** — lorsque la cohorte est entièrement ou partiellement remplie, le propriétaire *ou* le gardien opérationnel appelle `settle_limit_order` pour pousser les tokens de sortie vers l'ATA du propriétaire.
...
| `OpenPosition[WithToken22Nft]` | Mint NFT + ATA, `personalPosition`, possiblement nouveau(x) `tickArrayState`, `tickArrayBitmapExtension` s'il n'existe pas encore | — |
| `IncreaseLiquidity` | Possiblement nouveau(x) `tickArrayState` | — |
...
| `CollectRewards` | — | — |
| `UpdateRewardInfos` | — | — |
| `CloseProtocolPosition` (admin) | — | `protocolPositionState` vestigiel (loyer → admin) |

Les comptes `TickArrayState` ne sont **jamais fermés par le programme** — ils persistent pour la durée de vie du pool. Une fois qu'un tableau de ticks a été initialisé, il reste on-chain même lorsque chaque tick à l'intérieur revient à `liquidity_gross == 0`. Réutiliser un tableau de ticks existant est gratuit ; seule la première position qui touche un tableau jamais initialisé paie son loyer.

## Quoi lire où

- **Mathématiques des ticks et mécaniques de plage** : [`products/clmm/ticks-and-positions`](/fr/products/clmm/ticks-and-positions).
- **Marche des swaps et mathématiques de croissance des frais** : [`products/clmm/math`](/fr/products/clmm/math).
- **Listes de comptes d'instructions** : [`products/clmm/instructions`](/fr/products/clmm/instructions).
