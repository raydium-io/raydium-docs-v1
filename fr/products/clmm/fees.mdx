...
1. **Décroissance de la référence**. Si `now - last_update_timestamp >= filter_period`, la référence de volatilité décroît. Notez que les deux bornes sont `>=`, et non `>` :
...
Les champs du layout sont `tokenFeesOwedA` / `tokenFeesOwedB` (et non `...Owed0` / `...Owed1`), et chaque entrée de `rewardInfos` est `{ growthInsideLastX64, rewardAmountOwed }` — le mint de récompense lui-même provient des `rewardInfos` du pool, pas de ceux de la position.

`tokenFeesOwed*` et `rewardAmountOwed` sont des instantanés datant du dernier toucher de la position. Pour obtenir les valeurs **courantes** (reflétant la croissance depuis lors), simulez un `DecreaseLiquidity` à liquidité nulle, ou recalculez à partir du `fee_growth_*` global et des deux instantanés tick-outside.
