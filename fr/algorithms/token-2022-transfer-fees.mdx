...
<Info>
  Matrice de support : **CPMM** supporte complètement Token-2022, y compris les mints avec frais de transfert. **CLMM** supporte Token-2022 avec frais de transfert via les comptes `SwapV2` explicites. **AMM v4** ne supporte pas du tout Token-2022. **LaunchLab** crée un mint de base SPL classique via `InitializeV2`, mais dispose d'un point d'entrée dédié `InitializeWithToken2022` qui crée un mint de base Token-2022 et y attache éventuellement un `TransferFeeConfig` ; son côté quote accepte l'un ou l'autre programme de token. Voir [`products/launchlab/platform-config`](/fr/products/launchlab/platform-config#token-2022-transfer-fee-authorities). **Farm v6** supporte Token-2022 sur les deux mints de staking et de récompense.
</Info>
...
- [Documentation de l'extension transfer-fee SPL Token-2022](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
