...
description: "Solana réduit le minimum exempt de loyer de 90 % en cinq étapes selon SIMD-0437. Les comptes financés avant chaque étape conservent plus que nécessaire — ce qu'est cet excédent, quels programmes le restituent, et comment le trouver et le récupérer."
---
...
| Étape | `lamports_per_byte` | Réduction par rapport à l'original | Loyer pour un compte de jeton de 165 octets |
|---|---|---|---|
...
Pour un compte de jeton SPL de 165 octets après l'étape 1, c'est `293 × (6 960 − 6 333) = 183 711` lamports, ou ~0,000184 SOL par compte. Un compte Token-2022 portant des extensions est plus grand, il détient donc proportionnellement plus — un compte de 182 octets est surfinancé de `310 × 627 = 194 370` lamports.
...
<CardGroup cols={2}>
  <Card title="SPL Token et Token-2022" icon="circle-check">
    Les deux exposent `WithdrawExcessLamports`. Le compte reste ouvert, conserve son solde de jetons, et descend simplement au minimum actuel.
  </Card>
...
|---|---|---|
| Compte de jeton SPL | `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA` | Oui — `WithdrawExcessLamports` |
| Compte de jeton Token-2022 | `TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb` | Oui — `WithdrawExcessLamports` |
| Compte de jeton SOL enrobé (natif) | l'un ou l'autre programme de jeton | Non — voir ci-dessous |
...
Trois propriétés la rendent sûre de déclencher sur chaque compte d'un portefeuille :

- **Elle ne prend pas de montant.** Le programme calcule `source.lamports − rent.minimum_balance(source.data_len())` lui-même, il ne peut donc jamais faire descendre un compte en dessous du minimum actuel, et il reste correct à mesure que les étapes ultérieures s'activent.
...
Un compte de jeton gelé est toujours admissible : le gel restreint le mouvement des jetons, pas des lamports.

### Construire l'instruction

`@solana/spl-token` n'exporte pas un constructeur pour cela. À partir de `0.4.15`, l'entrée d'énumération est toujours commentée :
...
`getMinimumBalanceForRentExemption(0)` est un canal auxiliaire utile : il retourne exactement `128 × lamports_per_byte`, donc diviser par 128 vous dit à quelle étape de déploiement le cluster se trouve sans analyser la sysvar `Rent`.
...
Contre la limite de 1 232 octets de transaction, environ 26 instructions tiennent une fois que les instructions de budget de calcul et le blockhash sont comptés. **Vingt par transaction** est le nombre de travail sûr, et c'est ce que l'implémentation propre de Raydium utilise. Un portefeuille avec 116 comptes récupérables balaie donc en six transactions, à 5 000 lamports de frais de base chacune.
...
Votre portefeuille n'est pas le seul endroit où la réduction libère des lamports. Chaque pool détient aussi du loyer — les coffres, les mints LP, et les comptes d'état appartenant au programme qui ont tous été financés à l'ancien taux. Ce loyer appartient au protocole, pas aux LP : il a été payé par celui qui a créé le compte, il ne fait pas partie des réserves d'aucun pool, et il n'a jamais entré la courbe.

Trois programmes ont reçu une instruction d'administrateur le 2026-09-09 pour le restituer :

| Programme | Instruction | Signataire |
|---|---|---|
| AMM v4 | `WithdrawExcessLamports` (tag 18) | Un portefeuille dédié de collecte de lamports uniquement |
| CPMM | `CollectExcessLamports` | L'administrateur du programme ou un portefeuille dédié de collecte de lamports |
| LaunchLab | `CollectExcessLamports` | L'administrateur du programme ou un portefeuille dédié de collecte de lamports |

Les adresses sont dans [`reference/program-addresses`](/fr/reference/program-addresses#excess-lamports-collection-wallets). CLMM et Stable AMM ne faisaient pas partie de cette version.
...
Les trois suivent le même envoi, sur le propriétaire du compte source :

- **Un compte de jeton ou un mint appartenant à un PDA d'autorité du programme** — le programme CPI l'instruction `WithdrawExcessLamports` du programme de jeton (discriminant 38), en signant en tant que ce PDA.
- **Un coffre SOL enrobé** — `WithdrawExcessLamports` refuse les comptes natifs, donc le programme `SyncNative` d'abord (ce qui replie l'excédent donné dans le `amount` enrobé), mesure exactement combien le montant enrobé a grandi, `UnwrapLamports` pour ce delta, puis affirme que le solde enrobé est revenu à sa valeur pré-sync. Si cette vérification échoue, l'instruction entière revient avec une `LamportsCalculateError`. C'est pourquoi un coffre de pool côté SOL conserve sa liquidité complète à travers un balayage.
- **Un compte d'état appartenant au programme** — `AmmInfo`, `PoolState`, `AmmConfig`, `ObservationState`, un `PlatformConfig`, et ainsi de suite. Un programme peut débiter ses propres comptes directement, il déplace donc le solde jusqu'à `rent.minimum_balance(data_len)` sans CPI du tout.

Les comptes appartenant à n'importe quoi d'autre sont ignorés silencieusement, donc passer un compte non lié est inoffensif plutôt que fatal.

La séquence SOL enrobée vaut la peine d'être copiée si vous détenez vos propres comptes natifs : c'est le seul moyen de prendre l'excédent d'un compte wSOL sans changer ce que le compte rapporte comme solde de jetons.

<Note>
  **Ces chemins dépendent du programme de jeton déployé, pas d'une version de caisse.** Les trois programmes encodent à la main les instructions de jeton — un seul octet `38` pour `WithdrawExcessLamports`, `45` plus un `COption<u64>` pour `UnwrapLamports` — et les envoient au programme de jeton qui possède le compte source. Les deux instructions existent dans les programmes SPL Token et Token-2022 actuels de mainnet. Un validateur local ou un harnais de test exécutant une ancienne version groupée du SPL Token ne les implémente pas, et un balayage contre lui échoue sur un discriminateur inconnu plutôt que sur n'importe quoi dans le programme Raydium. Testez ces chemins contre un programme de jeton cloné de mainnet.
</Note>

### Ce qui ne peut toujours pas être récupéré sur place

Le loyer d'une position CLMM est inchangé par tout cela — il revient à la fermeture de la position, comme le tableau ci-dessus le dit. Même chose pour le mint de base LaunchLab : l'instruction initialize révoque `MintTokens` dans le même appel qui frappe l'offre, donc aucune clé ne peut jamais signer un `WithdrawExcessLamports` pour ce mint et son loyer est bloqué par conception.

## Devriez-vous récupérer maintenant ou attendre ?

Les deux vont bien, et la différence est petite de toute façon :

- **L'excédent ne va nulle part.** Il reste dans vos propres comptes. Rien n'expire, rien n'est balayé, aucun délai ne s'applique.
...
<CardGroup cols={2}>
  ...
  <Card title="Account model" icon="database" href="/fr/solana-fundamentals/account-model">
