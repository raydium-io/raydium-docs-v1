---
title: "2026-09-19 — CPMM : le protocole prélève une part des frais de créateur"
description: "CPMM peut désormais conserver une part configurable des frais de créateur, appliquée lors de la collecte plutôt que lors de la facturation. AmmConfig gagne creator_fee_share_rate (prélevé sur le padding, même taille), un nouveau PDA CreatorFeeShare le remplace par créateur, et CreateCreatorFeeShare / CloseCreatorFeeShare le gèrent. Les deux chemins CollectCreatorFee gagnent des comptes ajoutés en fin de liste — rupture pour les clients existants. UpdateAmmConfig gagne le paramètre 8. Aucun nouveau code d'erreur, aucun changement PoolState, la mathématique du swap est inchangée."
---

<Info>
  **Cette page est traduite automatiquement par IA. La version anglaise fait foi.**

  [Voir la version anglaise →](/reference/changelog/2026-09-19-cpmm-creator-fee-protocol-share)
</Info>

<Info>
  Cette entrée couvre une mise à jour CPMM à venir. Elle a été vérifiée par rapport à la branche de version locale (`bdeee23`, 15 septembre 2026) avant le déploiement. Confirmez le programme déployé avant de vous fier aux nouvelles instructions ou aux listes de comptes modifiées.
</Info>
...
- **Les deux instructions de collecte des frais de créateur ont changé leurs listes de comptes. C'est une rupture, mais propre sur le plan des positions.** `CollectCreatorFee` gagne `creator_fee_share` à la position 15. `CollectCreatorFeePermissionless` gagne `amm_config` à la position 15 et `creator_fee_share` à la position 16. Les deux sont ajoutés après `system_program`, donc chaque compte qu'un client existant passe déjà conserve son index — l'ancienne transaction se retrouve simplement à court de comptes. Ajoutez les nouveaux comptes en fin de liste ; ne réordonnez rien d'autre.
...
`CollectCreatorFee` — un compte ajouté en fin de liste :

| # | Avant | Après |
|---|-------|-------|
| 1–14 | `creator` … `system_program` | inchangé, même ordre |
| 15 | — | **`creator_fee_share`** (nouveau) |

`CollectCreatorFeePermissionless` — deux comptes ajoutés en fin de liste :

| # | Avant | Après |
|---|-------|-------|
| 1–14 | `payer` … `system_program` | inchangé, même ordre |
| 15 | — | **`amm_config`** (nouveau) |
| 16 | — | **`creator_fee_share`** (nouveau) |

<Warning>
  Les nouveaux comptes se situent à la **fin** des deux listes, donc rien de ce qu'un client existant passe déjà ne bouge — et le mode d'échec est le plus lisible : une ancienne transaction manque de comptes et Anchor la rejette avec `AccountNotEnoughKeys` (`3005`) avant qu'aucune contrainte ne s'exécute, plutôt que de mésinterpréter un coffre comme une config. Cela ne rend pas le changement optionnel pour autant : les deux comptes sont requis à chaque appel. Régénérez à partir du nouvel IDL, et vérifiez que toute version SDK que vous épinglez porte les nouveaux comptes avant de la pointer vers le programme mis à niveau.
</Warning>
