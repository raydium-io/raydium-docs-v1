...
|---|---|---|---|
| CPMM | Oui | `raydium-cp-swap` (dépôt public ; le chemin Rust `use` est `raydium_cp_swap`) | [Le motif général](#the-general-anchor-cpi-pattern), l'exemple travaillé |
| CLMM | Oui | `raydium-clmm` (dépôt public ; le chemin Rust `use` est `raydium_clmm`) | [Application du motif : CLMM](#applying-the-pattern-clmm) |
| LaunchLab | Oui | pas de nom de crate confirmé ; la source du programme est fermée, mais l'identifiant de métadonnées interne de l'IDL on-chain est `raydium_launchpad`, un nom technique plutôt que le nom du produit (voir la mise en garde dans [Application du motif : LaunchLab](#applying-the-pattern-launchlab)) | [Application du motif : LaunchLab](#applying-the-pattern-launchlab) |
| Farm v6 | Non confirmé ; aucun IDL public ou source trouvée (voir l'avertissement dans [Farm v6](#farm-v6)) | n/a | [Farm v6](#farm-v6) |
| AMM v4 | **Non**, antérieur à Anchor sans crate CPI | aucun | [AMM v4](#amm-v4-manual-instruction-construction) |

Si vous intégrez CPMM, CLMM ou LaunchLab, lisez d'abord [le motif général](#the-general-anchor-cpi-pattern), puis passez à la section de votre programme pour la liste des comptes et les différences. Farm v6 et AMM v4 sont suffisamment différents pour justifier la lecture de leurs sections de manière autonome.
...
# NOTE: la branche `master` de raydium-cp-swap épingle toujours anchor-lang 0.32.1. Les pins `=1.0.2`
# ci-dessus ne se résolvent que contre la branche de mise à niveau ; si vous ciblez `master`, utilisez =0.32.1
# et l'API CpiContext 0.3x.
...
# Voir « Application du motif : LaunchLab » ci-dessous.
...
Pour des exemples CPI fonctionnels qui câblent les structs de comptes de bout en bout, voir [`raydium-io/raydium-cpi-example`](https://github.com/raydium-io/raydium-cpi-example) (couvre AMM v4, CPMM et CLMM). Sa branche la plus récente est [`anchor-0.31.0`](https://github.com/raydium-io/raydium-cpi-example/tree/anchor-0.31.0) — il n'y a pas encore de branche Anchor 1.x, donc traitez ce dépôt comme la référence pour le **câblage des structs de comptes**, pas pour les pins de version que cette page impose.
...
pub fn my_proxy_swap(
    ...
    let cpi_accounts = CpmmSwap {
        // `payer` est VOTRE signataire — il doit posséder `input_token_account`.
        payer:                ctx.accounts.user.to_account_info(),
        // `authority` est le propre PDA de coffre de CPMM aux graines [b"vault_and_lp_mint_auth_seed"],
        // que CPMM signe avec lui-même. Passer votre signataire ici échoue la
        // contrainte de graines (`ConstraintSeeds`) avant que tout transfert ne s'exécute.
...
- **CLMM `SwapV2`** : [tableaux de ticks](#applying-the-pattern-clmm), ordonnés directionnellement.
- **Farm v6** : paires `(reward_vault, user_reward_ata)`, mais seulement à partir du deuxième flux de récompense en avant ; voir [Farm v6](#farm-v6) pour ce que le décodage d'une vraie transaction montre.

## Application du motif : CLMM

`SwapV2` suit [le motif général](#the-general-anchor-cpi-pattern) ci-dessus avec une liste de comptes différente et une exigence de comptes restants pour les tableaux de ticks. Le module `#[program]` de la crate est nommé `raydium_clmm`, qui est aussi son chemin Rust `use`.
...
pub struct MyProxyClmmSwap<'info> {
    ...
    /// `cpmm_program` de CPMM dans le motif général ci-dessus : c'est le programme invoqué, et
    /// `Program<T>` vérifie son adresse automatiquement.
...
LaunchLab est basé sur Anchor et IDL-publié : [`raydium_launchpad/raydium_launchpad.json`](https://github.com/raydium-io/raydium-idl/blob/master/raydium_launchpad/raydium_launchpad.json) dans le dépôt public `raydium-idl`. L'identifiant de métadonnées interne de cet IDL est `raydium_launchpad`, un nom technique pour le programme sous-jacent, pas un nom alternatif pour le produit. Contrairement à CPMM et CLMM, cependant, **la source du programme elle-même n'est pas disponible publiquement** (voir [`reference/program-addresses`](/fr/reference/program-addresses)). Il n'y a pas de dépendance `git = "..."` vers laquelle pointer Cargo, et pas de source pour confirmer quel serait le chemin Rust `use` d'une vraie crate.
...
La forme CPI suit [le motif général](#the-general-anchor-cpi-pattern). La liste des comptes et les arguments ci-dessous proviennent de l'instruction `buy_exact_in` de l'IDL on-chain, pas de `products/launchlab/instructions.mdx` :
...
pub struct MyProxyBuy<'info> {
    ...
    /// CHECK: PDA, graines = [b"vault_auth_seed"], validé par le programme
    ...
    /// SPL Token classique spécifiquement (contrairement à base_token_program ci-dessus), donc la
    /// vérification d'adresse intégrée d'Anchor sur `Program<Token>` est suffisante en soi.
    pub quote_token_program: Program<'info, Token>,

    /// CHECK: PDA, graines = [b"__event_authority"], requis par le motif emit_cpi! du programme
...
pub fn my_proxy_buy(
    ...
    // `program` sert à la fois de cible CPI ici : le propre compte de LaunchLab est à la fois l'un de
    // ses comptes requis d'instruction (pour son motif emit_cpi!) et le programme
...
match cpi::swap_base_input(cpi_ctx, amount_in, minimum_amount_out) {
    ...
    Err(e) => {
        ...
        // Relancer, ou convertir en votre propre type d'erreur.
        Err(e)
    }
}
```

Échangez le type d'erreur pertinent pour le programme que vous appelez (`raydium_clmm::error::ErrorCode` pour CLMM, et ainsi de suite). Les numéros de code d'erreur sont stables selon la politique IDL ([`sdk-api/anchor-idl`](/fr/sdk-api/anchor-idl#idl-change-policy)), donc vous pouvez tester contre des codes spécifiques en comparant la valeur numérique. Tables d'erreurs complètes : [CPMM](/fr/reference/error-codes#cpmm-standard-amm-errors), [CLMM](/fr/reference/error-codes#clmm-errors), [AMM v4, Farm v6 et LaunchLab](/fr/reference/error-codes#amm-v4-farm-v3-/-v5-/-v6-launchlab-errors).
...
let ix = Instruction {
    ...
    accounts: vec![
        ...
        // ... comptes restants par products/amm-v4/instructions ...
    ],
    data,
};
...
**Utilisez le SDK TS si c'est une option pour votre intégration.** `raydium.farm.deposit(...)` (voir [`products/farm-staking/code-demos`](/fr/products/farm-staking/code-demos)) est exercé par des démos réelles et ne dépend pas de l'existence d'une crate Anchor pour ce programme.

<Warning>
  **Farm v6 n'offre pas de chemin CPI Anchor.** Il n'y a pas de crate `raydium_farm_v6` sur crates.io, pas de dépôt source public, et pas d'IDL on-chain — le programme n'a ni un compte `anchor:idl` hérité ni une entrée dans le programme Program Metadata (voir [`sdk-api/anchor-idl`](/fr/sdk-api/anchor-idl)). Traitez-le comme un programme non-Anchor et construisez ses instructions à la main, comme ci-dessous.
</Warning>

**Si vous avez besoin du CPI Rust malgré tout**, par exemple en composant à partir d'un autre programme on-chain, construisez l'`Instruction` à la main, de la même manière que [AMM v4](#amm-v4-manual-instruction-construction) : dérivez la vraie liste de comptes et les discriminateurs d'instruction indépendamment, par exemple en décodant les dispositions TypeScript du SDK (`raydium-sdk-V2`'s farm module), en décodant les vraies transactions directement (voir ci-dessous), ou en vidant et en désassemblant le programme déployé.
...
1. **`anchor test` avec clonage de programme.** Tire le bytecode déployé mainnet dans votre validateur local ; voir [Clonage de programmes dans un validateur local](#cloning-programs-into-a-local-validator) ci-dessous pour la config `Anchor.toml` et deux choses qui trébuchent spécifiquement les tests de création de pool.
2. **Devnet.** Raydium déploie la plupart des programmes sur devnet, mais à **des ID de programme différents que mainnet** pour chaque programme (CPMM, CLMM, AMM v4, Stable AMM et LaunchLab ont chacun une adresse devnet distincte ; voir le tableau Devnet dans [`reference/program-addresses`](/fr/reference/program-addresses)). Farm v3/v5/v6 ne sont pas fiablement publiés sur devnet ; l'API en direct (`https://api-v3-devnet.raydium.io/main/info`) a l'image actuelle. Si vous utilisez les constantes `DEVNET_PROGRAM_ID` groupées de `raydium_clmm` (ou l'équivalent pour d'autres crates), ne supposez pas qu'un ID mainnet fonctionne aussi sur devnet. Exécutez `anchor test --provider.cluster devnet` pour frapper le code en direct une fois que vous avez les bonnes adresses.
...
<Warning>
  **Cloner le programme n'est pas suffisant si votre test *crée aussi* un pool** (plutôt que d'échanger contre un qui existe déjà). L'instruction `initialize` de CPMM valide ses comptes `amm_config` et `create_pool_fee` par rapport aux vraies données on-chain, donc vous devez aussi cloner ceux-ci, ou `initialize` échoue carrément. Pour CPMM spécifiquement : clonez le `AmmConfig` de niveau de frais que vous voulez (récupérez son adresse de `GET https://api-v3.raydium.io/main/cpmm-config`, l'index 0 est le niveau 0,25 %) et le [compte de token récepteur de frais](/fr/reference/program-addresses#shared-config-/-pda-conventions), validé par adresse exacte, pas créé à la volée, donc il doit déjà exister.
</Warning>

<Warning>
  **Un pool que votre test vient de créer n'est pas échangeable à l'instant même.** L'instruction `initialize` de CPMM remplace silencieusement un `open_time` demandé qui n'est pas strictement dans le futur (`if open_time <= block_timestamp { open_time = block_timestamp + 1 }`), donc même `startTime: 0` (« ouvrir immédiatement », selon le SDK) laisse un vrai écart ≥1 seconde avant que le pool n'accepte les swaps. Un test qui crée un pool et échange contre lui sans délai frappera [`NotApproved`](/fr/reference/error-codes#cpmm-standard-amm-errors). Un court `await` (1–2s) entre la création du pool et le premier swap est suffisant. Ceci est spécifique aux tests ; un humain exécutant deux commandes manuelles séparées ne remarquerait normalement pas, car la dactylographie et le démarrage du processus mangent déjà plus d'une seconde.
</Warning>
...
- [`sdk-api/anchor-idl`](/fr/sdk-api/anchor-idl) : récupération IDL et régénération client, y compris le chemin IDL-codegen pour LaunchLab.
- [`integration-guides/cpi-integration`](/fr/integration-guides/cpi-integration) : motifs d'intégration de haut niveau comme les séquences, les coffres et la composition d'agrégateur.
...
- [Docs CPI Anchor](https://www.anchor-lang.com/docs/basics/cpi)
