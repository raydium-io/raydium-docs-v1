...
No Solana, o que se revoga é um **delegado** do SPL-Token: a instrução `Approve` nomeia um delegado e um valor em uma de suas contas de token, e `Revoke` o limpa. Abra a visualização de aprovações de token / apps conectados da sua carteira e revogue qualquer coisa que você não reconheça; Phantom e Solflare ambos expõem isso. A partir de um terminal, é um comando por conta de token:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

Isso para futuras transações de drain, mas não recupera fundos já drenados.

<Warning>
  [revoke.cash](https://revoke.cash) é **somente para EVM** — cobre Ethereum e mais de 100 chains EVM (incluindo Neon, a camada EVM do Solana), mas não o Solana em si. Não envie usuários do Solana para lá.
</Warning>
