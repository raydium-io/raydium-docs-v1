...
| Criador de Permission-PDA | Program `admin` **ou** um proprietário dedicado de create-permission-PDA (valor diferente em mainnet vs devnet) | Chamar `CreatePermissionPda` para conceder a uma autoridade acesso a `InitializeWithPermission`. O proprietário dedicado não pode chamar `ClosePermissionPda`; a revogação permanece apenas para o admin. |
...
Para usuários que querem uma alternativa imutável, o programa AMM v4 mais antigo tem sido estável desde seu último audit; zero upgrades em 18 meses. Esse caminho de código é efetivamente congelado mesmo que a autoridade ainda exista.
...
- [Solana BPF Loader docs](https://docs.solana.com/developing/runtime-facilities/programs#bpf-loader) — mecanismo de upgrade.
