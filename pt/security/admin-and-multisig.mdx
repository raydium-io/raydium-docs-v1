...
| Função | Endereço da autoridade | O que pode fazer |
|--------|------------------------|------------------|
...
|--------|------------------------|------------------|
| Upgrade do programa | Squads multisig (3/4) | Implantar novo bytecode do programa |
| Admin do AmmConfig | Treasury multisig (3/5) | Criar novos AmmConfigs (níveis de taxa); alternar existentes |
| Destinatário da taxa de criação de pool | Treasury multisig (3/5) | Receber a taxa única de criação de pool |
| Criador de Permission-PDA | Program `admin` **ou** um proprietário dedicado de create-permission-PDA (valor diferente em mainnet vs devnet) | Chamar `CreatePermissionPda` para conceder a uma autoridade acesso a `InitializeWithPermission`. Desde a atualização de 2026-09, o proprietário dedicado também pode chamar `ClosePermissionPda`, portanto ele concede e revoga; antes dessa atualização a revogação era apenas para o admin. |
...
| Admin do DynamicFeeConfig | Treasury multisig (3/5) | `CreateDynamicFeeConfig` / `UpdateDynamicFeeConfig` — calibrar os níveis de taxa dinâmica que uma chamada `CreateCustomizablePool` pode optar por usar |
| `permission_pda_admin` | Program `admin` **ou** uma chave dedicada `permission_pda_admin` (valor diferente em mainnet vs devnet) | `CreatePermissionPda` / `ClosePermissionPda` — conceder ou revogar o direito de uma autoridade de chamar `CreatePermissionedPool`. A concessão é a mera existência de um PDA `["permission", authority]`; ela **não** confere nenhum poder de mutação de pool. |
...
|--------|------------------------|------------------|
| Upgrade do programa | Squads multisig (3/4) | Implantar novo bytecode do programa |
| Criador de farm (por farm) | Carteira do criador do farm | Financiar cofres de recompensa, estender cronogramas, recuperar recompensas não utilizadas |
...
|--------|------------------------|------------------|
| Upgrade do programa | Squads multisig (3/4) | Implantar novo bytecode do programa |
| Admin do programa | Admin Raydium compartilhado | Criar e atualizar contas `GlobalConfig`; operar outros caminhos apenas para admin |
| Criador de config delegado | Autoridade específica do cluster dedicada | Criar um novo `GlobalConfig`; não pode atualizar uma config existente |
| Admin da plataforma | Autoridade de seed PDA para um `PlatformConfig` | Atualizar configurações da plataforma e criar ou fechar contas `PlatformAllowConfig` dessa plataforma |
| Criador de launch (por launch) | Carteira do criador do launch | Reivindicar taxas de criador de curva de ligação; recebe direitos de taxa de criador CPMM apenas quando a plataforma não substitui `platform_cp_creator` |

A lista de permissões do admin da plataforma é auto-restritiva: limita quais contas `GlobalConfig` os launches sob essa plataforma podem usar. Não concede autoridade sobre outras plataformas ou fundos de launch existentes. Os endereços de autoridade delegada canônicos vivem em [`reference/program-addresses`](/pt/reference/program-addresses).
...
Nenhum programa da Raydium está atualmente congelado ou estático o suficiente para ser tratado como uma alternativa imutável — o AMM v4, historicamente o mais estático deles, foi ele mesmo reimplantado em 2026-07-22. O slot do último deploy de cada programa é público; leia-o antes de presumir que algum deles está dormente:

| Programa | Último deploy (mainnet) |
|---------|--------------------------|
...
(Da conta `ProgramData` de cada programa; `solana program show <PROGRAM_ID>` informa o mesmo slot.)

## Autoridade do AmmConfig

Cada criação de novo AmmConfig é permissionada — a multisig treasury 3/5 autoriza novos níveis de taxa e espaçamentos de tick. Os pools existentes referenciam seu AmmConfig por PDA; o nível de taxa do pool é o que o AmmConfig diz.
...
|----------|--------|----------|--------|
| **Upgrade do programa** | **3/4** | **24 horas** | Autoridade única para implantar novo bytecode de programa para AMM v4, CPMM, CLMM, LaunchLab, Lock, AMM Routing, Stable Swap e os programas legados Farm/Staking. Visualizar em [`app.squads.so/.../FytDr…ceZQK`](https://app.squads.so/squads/FytDrVzDybM1TwFQPGb8qaxZR7dBCzNeqT3vtQsceZQK/home). |
...
`AmmConfig` não tem campo `admin` — o administrador do programa é uma pubkey compilada no programa (`crate::admin::ID`), e as autoridades operacionais por configuração são `protocolOwner` / `fundOwner`. A autoridade de upgrade do BPF é uma questão separada, em nível de loader; leia-a com `solana program show`.
...
2. **Monitore a atividade da multisig.** A [UI do Squads](https://v4.squads.so) mostra transações pendentes; um upgrade agendado lhe dá 24 horas para se desfazer se discordar da mudança.
...
- [Documentação de programas e loaders da Solana](https://solana.com/docs/core/programs) — o mecanismo de upgrade e como revogar a autoridade de upgrade torna um programa imutável.
