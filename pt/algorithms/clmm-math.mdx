...
3. **Matemática de ticks é uniforme.** Como ticks são definidos como `1.0001^i`, `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` também é uma escada geométrica exata. (`√1.0001 = 1.00004999875…`, portanto tratá-lo como exatamente `1.00005` provoca um desvio de até ~0.6% em `MAX_TICK`.) Cada cruzamento de tick se traduz em uma pequena multiplicação no espaço `sqrt_price_x64`.
...
`tick_i` é um `i32`. O intervalo ativo é `[MIN_TICK, MAX_TICK] = [−443636, 443636]`, fornecendo um intervalo de preço de aproximadamente `[2^−64, 2^64]` — cerca de `5.4e-20` a `1.8e19`. O `tick_spacing` de cada pool é definido por seu nível de taxa: espaçamentos menores para pares justos (por exemplo, stablecoin com taxa de 0.01% usa espaçamento 1), espaçamentos maiores para pares voláteis (taxa de 0.25% usa 60, taxa de 1% usa 120).
...
- **Oracle.** O buffer ring `ObservationState` da Raydium armazena apenas `(block_timestamp: u32, tick_cumulative: i64)` por slot, mais `[u64; 4]` de preenchimento reservado — **não** existe um campo `seconds_per_liquidity_cumulative`, portanto as derivações ponderadas por liquidez do Uniswap não estão disponíveis on-chain.
