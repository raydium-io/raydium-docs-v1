...
your_APR = headline_APR
         · (your_in_range_fraction / pool_in_range_fraction)     # bônus de concentração
         · time_in_range                                          # disciplina de intervalo
         · (1 − transfer_fee_haircut)                             # imposto Token-2022
         · compounding_factor                                     # se você auto-reaplica
```
...
- **Composição.** Se você `collectFee` e reimplanta no mesmo intervalo semanalmente, o APR efetivo é cerca de `(1 + daily_APR)^365 − 1`. Sem composição é linear.
...
- Volume 24h: \$120M
- Tier de taxa: 0.05% (o LP recebe **84%** das taxas — todo `AmmConfig` de CLMM na mainnet carrega `protocol_fee_rate = 12%` mais `fund_fee_rate = 4%`)
- TVL total: \$40M
...
A interface Raydium exibe **102% APR** para o pool. (Este exemplo usa um pool profundo e de alto volume para manter os números concretos; pools CLMM típicos exibem entre 10% e 50% APR, com pools de baixo volume bem abaixo de 10%.)
...
Essa é uma estimativa, não uma garantia. O volume realizado pode cair pela metade ou dobrar em uma semana.
...
- **Teste seu intervalo específico** em dados históricos de volume e preço antes de comprometer capital significativo.
...
O SDK da Raydium expõe `PoolUtils.estimateAprsForPriceRangeMultiplier` e `PoolUtils.estimateAprsForPriceRangeDelta` para essa estimativa.
...
const apr = PoolUtils.estimateAprsForPriceRangeMultiplier({
  ...
  positionTickLowerIndex: -100,   // seu intervalo pretendido
...
O auxiliar retorna `{ feeApr, rewardsApr[], apr }` — `rewardsApr` é por fluxo de recompensa e `apr` é o total. Não existe um método `raydium.clmm.estimatedApr`. `estimateAprsForPriceRangeDelta` é o mesmo cálculo parametrizado por um delta ± em torno do preço spot, em vez de ticks explícitos.
...
- **"APR é composto automaticamente."** Não. As taxas devem ser reivindicadas — por meio de um `DecreaseLiquidity` com liquidez zero, já que o CLMM não tem instrução autônoma de coleta — e depois manualmente reimplantadas. A Raydium não auto-compõe taxas de CLMM.
...
- Implementações de `PoolUtils.estimateAprsForPriceRangeMultiplier` / `estimateAprsForPriceRangeDelta` do SDK Raydium v2.
