...
Suponha que você deposite \$10.000 de SOL/USDC em um CLMM em SOL = \$160, intervalo `[$120, $200]`. Note que o depósito **não** é 50/50: nesse preço e nesse intervalo a posição recebe 27.54 SOL (\$4.407) e \$5.593 USDC, o que fixa `L ≈ 3,300`. O referencial HODL abaixo é uma manutenção 50/50 de \$5.000 + \$5.000, correspondendo à comparação CPMM.

- Largura do intervalo: `P_hi / P_lo = 200 / 120 ≈ 1.67`. Fator de alavancagem ≈ `1 / (1 − √(120/200)) ≈ 4.4×`.
- Se SOL se move para \$180 (+12.5%), valor HODL = \$10.625; posição CLMM ≈ \$10.397; **IL ≈ 2.1%**.
- Se SOL se move para \$200 (+25%), valor HODL = \$11.250; posição CLMM agora é 100% USDC valendo ≈ \$10.520; **IL ≈ 6.5%**.
- Se SOL se move para \$240 (+50%, fora do intervalo), valor HODL = \$12.500; posição CLMM ainda é ≈ \$10.520; **IL ≈ 15.8%**.

O mesmo capital em um CPMM (sem intervalo) teria IL de apenas ~2.02% em \$240 e continuaria ganhando taxas. Essa diferença é a amplificação com que esta página começa: aproximadamente o fator de alavancagem vezes o IL do CPMM enquanto dentro do intervalo. A posição CLMM tem captura de taxa mais alta *enquanto dentro do intervalo* mas muito pior IL uma vez fora do intervalo por um período estendido.
...
- [`sdk-api/typescript-sdk`](/pt/sdk-api/typescript-sdk) — `raydium.clmm.getOwnerPositionInfo({ programId })` mais `PoolUtils.getAmountsFromLiquidity(...)` retornam a exposição atual; compare com sua base de entrada.
