...
**Nunca salte pelo mesmo pool duas vezes.** Ir `A → B → A → B` através do mesmo CLMM apenas queima taxas e slippage. Agregadores devem filtrar essas rotas na geração. (Nota: isso é *ciclar o mesmo par*, não multi-hop em geral — rotear `A → USDC → B` através de pools diferentes é o padrão útil endossado acima.)

**Mínimo por-hop vs end-to-end.** Com composição CPI ([`integration-guides/cpi-integration`](/pt/integration-guides/cpi-integration)), você pode definir cada `minimum_amount_out` do hop como 0 e impor um único mínimo end-to-end no seu proxy. Sem CPI, cada hop impõe seu próprio mínimo, o que requer computar limites intermediários razoáveis — comumente `quote_i * (1 - slippage_bps/10000)` por hop.
...
**Tamanhos de negociação menores.** Divida a negociação entre múltiplas transações para que nenhuma tx única mova o preço o suficiente para ser um alvo sandwich lucrativo. Aumenta o custo total de gás.
