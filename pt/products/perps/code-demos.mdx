...
<Info>
  **Raydium Perps é uma implementação com marca branca na Orderly Network.** O livro de ordens, o mecanismo de correspondência e o estado da conta residem na Orderly. O Raydium SDK v2 (`@raydium-io/raydium-sdk-v2`) **não** cobre perps — para acesso programático, use a API REST + WebSocket da Orderly diretamente. Os trechos abaixo mostram os fluxos mais comuns; a referência canônica é a [introdução à API da Orderly](https://orderly.network/docs/build-on-omnichain/introduction), com os [endpoints REST](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) e os [streams WebSocket](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction) documentados sob ela.
</Info>
...
<Info>
  **Banner de versão.**
  ...
  A superfície da API da Orderly evolui; confira a [documentação da API da Orderly](https://orderly.network/docs/build-on-omnichain/introduction) antes de copiar estes trechos para produção.

  A superfície da API da Orderly evolui; verifique [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction) antes de copiar esses trechos para produção.
</Info>
...
- [Documentação do desenvolvedor Orderly Network](https://orderly.network/docs/build-on-omnichain/introduction) — referência canônica para a superfície da API usada acima. O Raydium Perps consome isso diretamente.
- [Cadeias suportadas pela Orderly](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — confirma o status do Solana e os endereços do cofre de liquidação por cadeia.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — um wrapper JS visível à comunidade sobre a mesma camada REST. Ele **não tem releases publicadas** e nenhuma política de manutenção declarada, então trate-o como código de referência em vez de dependência; a camada de assinatura acima é pequena o bastante para você mesmo manter.
