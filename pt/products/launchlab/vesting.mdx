...
| 6 | `user_base_token` | W |   | ATA do beneficiário no mint base; recebe os tokens desbloqueados. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Mint base do pool. |
| 8 | `base_token_program` |   |   | O programa que possui o mint base — SPL Token ou Token-2022. |
| 9 | `system_program` |   |   | Financia a ATA do beneficiário quando ela precisa ser criada. |
| 10 | `associated_token_program` |   |   | Cria a ATA do beneficiário. |

<Warning>
  `system_program` vem **antes** de `associated_token_program`. Ambos são tipados `Program<'info, _>`
  com endereços fixos, portanto um par trocado falha na restrição de endereço e o beneficiário não
...
- `pool_state.status == PoolStatus::Trade` (`2`) — a graduação já deve ter acontecido. Chamar antes disso reverte com `PoolNotMigrated`.
...
Em `2027-09-28` (90 dias após `start_time`), o Beneficiário A chama `ClaimVestedToken`:
...
Seis meses depois (`2028-03-26`, 270 dias após `start_time`), A reivindica novamente:
