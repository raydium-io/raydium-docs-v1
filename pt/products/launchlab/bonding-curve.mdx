...
- Resolver `(40 / (3e18)) × s³ = 9.9e6` — 9.9 USDC em unidades nativas de 6 decimais, as mesmas unidades em que está o `6.827e9` acima ⇒ `s ≈ 9.06e7` tokens base comprados.
...
Os mesmos 10 USDC compram muito menos base agora porque a curva é íngreme. Resolvendo em `s₀ = 750e6` com `quote_in_after_fee = 9.9e6` dá `∆s ≈ 4.4e5` — uma redução de **~200×** em base por USDC comparado à primeira compra.
...
- "Comprar quote exato" e "vender quote exato" invertem em **forma fechada** — a curva de produto constante algebricamente, a curva de preço fixo por divisão, a curva de preço linear por uma raiz quadrada. Não existe resolvedor Newton no programa, nem limite de iterações, nem erro `NotConverged`.
...
Para a curva de **produto constante** — o padrão do LaunchLab e o único tipo ativo — `cpmm_initial_price` é exatamente `price(base_sold)`, o preço marginal da curva no momento da transição, portanto um observador que muda da UI da curva para a UI do CPMM não vê nenhum salto. Para uma curva de preço quadrático não é: a média integrada fica acima do preço marginal (por um fator de `4/3` no exemplo acima), portanto um degrau na transição é esperado.
