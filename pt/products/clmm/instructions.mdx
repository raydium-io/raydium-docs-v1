...
<Info>
  Esta página funciona em conjunto com [`products/clmm/accounts`](/pt/products/clmm/accounts) (o que são as contas) e [`products/clmm/math`](/pt/products/clmm/math) (o que é a matemática). É autoritativa para argumentos e ordenação de contas; layouts de bytes específicos vêm do IDL.
</Info>
...
| Admin | `CreateSupportMintAssociated` | Autoriza uma configuração de extensão de mint Token-2022 para uso em pools CLMM. |
...
| Pool | `CreatePermissionedPool` | Cria um pool **adicional** para um par que já possui um, usando um `seed_index` fornecido pelo cliente incorporado no PDA do pool. O pagador deve possuir um PDA `Permission`. |
| Posição | `OpenPosition` / `OpenPositionV2` / `OpenPositionWithToken22Nft` | Emite um NFT de posição. `OpenPositionV2` substitui V1 (layout de conta mais novo com slot de extensão bitmap); `OpenPositionWithToken22Nft` emite o NFT de posição como Token-2022 em vez de SPL Token. Novo código deve usar V2 ou a variante Token-2022. |
...
| Posição | `CloseProtocolPosition` | Limpeza apenas para admin de PDAs `ProtocolPositionState` legados. O programa atual não cria ou lê `ProtocolPositionState` — esta instrução existe apenas para recuperar aluguel em contas criadas por versões antigas do programa. |
| Swap | `Swap` / `SwapV2` | Swap de liquidez constante. Ambas as variantes aplicam taxa dinâmica, roteamento de taxa unilateral e correspondência de ordem limitada; a única diferença é que `SwapV2` aceita mints Token-2022 (a variante V1 requer que ambos os cofres sejam SPL Token clássico). |
...
| Ordem limitada | `SettleLimitOrder` | Envia tokens de saída preenchidos para o proprietário da ordem. Pode ser chamado pelo proprietário ou pelo operador. |
...
| Taxas | `CollectProtocolFee` | Limpeza de admin de taxas de protocolo. |
| Taxas | `CollectFundFee` | Limpeza de admin de taxas de fundo. |
...
| Recompensas | `CollectRemainingRewards` | Após o `end_time` de um fluxo de recompensa, limpa qualquer token não alocado de volta ao financiador. |
...
| 5 | `token_mint_1` |   |   | |
| 6 | `token_vault_0` | W |   | `init` aqui, possuído pelo PDA de autoridade do pool. |
| 7 | `token_vault_1` | W |   | |
...
`fee_on` e o bit de habilitação de taxa dinâmica são definidos **apenas** na criação do pool. Não há atualização in-place — pools criados via `CreatePool` legado não podem ganhar taxa dinâmica ou taxa unilateral retroativamente. Novas implantações devem usar esta instrução como padrão.

## `CreatePermissionedPool`

Tanto `CreatePool` quanto `CreateCustomizablePool` derivam o PDA do pool de `["pool", amm_config, token_mint_0, token_mint_1]`, então há exatamente **um** endereço de pool canônico por tripla `(config, mint0, mint1)` — um segundo `init` nas mesmas sementes falha. `CreatePermissionedPool` remove essa restrição incorporando um `seed_index: u16` fornecido pelo cliente nas sementes do PDA do pool, permitindo **múltiplos pools para o mesmo par e nível de taxa** — cada um em seu próprio endereço. Como um endereço de pool arbitrário é uma capacidade privilegiada, o pagador deve possuir um PDA `Permission` que o autorize.

Tudo mais sobre o pool é idêntico a `CreateCustomizablePool`: ele usa os mesmos `CreateCustomizableParams` e suporta taxa unilateral e o opt-in de taxa dinâmica.

**Argumentos**

```rust
customizable_params: CreateCustomizableParams   // mesma struct que CreateCustomizablePool
seed_index:          u16                          // deve ser diferente de zero; incorporado nas sementes do PDA do pool
```
...
| 3 | `permission` |   |   | PDA `["permission", payer]`. Anchor rejeita a chamada se não existir / não for possuído pelo programa. |
...
<Note>
  Esta instrução **não** amplia o acesso geral à criação de pool — a criação sem permissão continua através de `CreatePool` / `CreateCustomizablePool`, que permanecem um-pool-por-par. `CreatePermissionedPool` existe para o caso específico em que um operador na lista de permissões precisa de vários pools para o mesmo par (por exemplo, preços iniciais diferentes ou coortes de lançamento) e possui um PDA `Permission` concedido pelo admin.
</Note>
...
amount_0_max: u64
amount_1_max: u64
with_metadata: bool           // escreve metadados de NFT (Metaplex)
...
**Contas** — como `OpenPosition` menos o mint de NFT (posição já existe; o NFT é passado como a ATA do proprietário contendo 1 token).
...
- Liquida taxas/recompensas acumuladas desde o último toque, mesmo padrão que `IncreaseLiquidity`.
...
- Queima o NFT.
- Fecha a conta de mint de NFT e a conta `personal_position`, reembolsando aluguel ao `payer`.
...
| 13 | `tick_array_bitmap_extension` (opcional) | W |   | Se swap entra na extensão. |
...
`Swap` (V1) implementa a mesma taxa dinâmica, roteamento de taxa unilateral e correspondência de ordem limitada que `SwapV2`; o único recurso que falta é suporte Token-2022 — ambos os cofres devem ser SPL Token clássico. Pools com qualquer mint Token-2022 devem ser trocados via `SwapV2`. O agregador e SDK já preferem V2 para cada perna CLMM, então chamadores não precisam ramificar no tipo de mint.
...
amount:         u64      // valor de token de entrada
```
...
| 11 | `output_vault_mint` |   |   | Mint do lado de saída (pareamento de verificação congelada). |
| 12 | `input_token_program` |   |   | SPL ou Token-2022. |
| 13 | `system_program`, `rent` |   |   | |

<Note>
  **Mudança de lista de contas (lançamento 2026-07).** `OpenLimitOrder` agora também usa as contas do **lado de saída** — `output_token_account`, `output_vault` e `output_vault_mint` — além do lado de entrada. Elas são usadas apenas para validação: o programa rejeita a ordem se a conta de token de entrada **ou** saída do proprietário está congelada. Isso garante que um preenchimento possa realmente ser liquidado para a ATA de saída do proprietário, o que importa para mints Token-2022 com lista de permissões / congelados por padrão (por exemplo, tokens permissionados) onde uma conta pode ainda não estar descongelada. Clientes construídos contra a lista de contas antiga unilateral devem adicionar as três contas de saída.
</Note>
...
```
amount: u64    // valor adicional de token de entrada
```
...
```
amount:     u64    // valor de token de entrada para retirar (máx. = restante não preenchido)
...
- Atualiza `limit_order` de acordo. Se o novo restante não preenchido é zero, o programa fecha a conta e reembolsa aluguel ao `owner`.

## `SettleLimitOrder`

Envia tokens de saída preenchidos para o proprietário sem alterar o restante não preenchido da ordem. Útil quando keepers `auto_withdraw` querem fazer pagamentos graduais de preenchimentos parciais de longa duração.
...
Fecha uma conta de ordem totalmente consumida. Aluguel é sempre retornado ao `limit_order.owner` independentemente de quem assina.
...
max_volatility_accumulator:  u32   // limite
```
...
**Erros comuns** — `InvalidDynamicFeeConfigParams` se `decay_period <= filter_period` ou qualquer campo com valor 0 está fora dos limites.
...
Forma idêntica ao `CollectProtocolFee` / `CollectFundFee` do CPMM. O signatário deve corresponder a `AmmConfig.owner` / `AmmConfig.fund_owner`. Limpa taxas de protocolo/fundo acumuladas dos cofres do pool para um destinatário, zera os campos correspondentes `PoolState.protocol_fees_*` / `fund_fees_*`.

## `InitializeReward`

Adiciona um novo fluxo de recompensa a um pool. Até 3 fluxos podem estar ativos de uma vez.
...
Estende, reabastece ou altera taxa de emissão em um fluxo de recompensa existente. Tipicamente chamado por um criador de pool ou pelo multisig Raydium. Restrições vivem on-chain: você geralmente pode estender `end_time` ou aumentar emissões, não encolhê-las retroativamente. Verifique a lista de proprietários de `operation_state`.
...
|---|------|---|---|
| 1 | `nft_owner` |   | S |
| 2 | `nft_account` |   |   | ATA do proprietário contendo o NFT de posição. |
...
- Transfere o valor devido para a ATA do destinatário, zera `reward_amount_owed[i]`.
