...
|-------|-----------|-------------------|
| `AmmConfig` | Camada de taxa: taxa de taxa de negociação, compartilhamento de protocolo, compartilhamento de fundo, espaçamento de tick padrão. Compartilhado entre todos os pools nesta camada. | 1 (compartilhado) |
| `PoolState` | `sqrt_price_x64` atual, tick atual, liquidez total, globals de crescimento de taxa, informações de recompensa, ponteiro de observação. | 1 |
...
| `PersonalPositionState` | Um por posição de LP. Armazena o intervalo, liquidez e crescimento de taxa/recompensa visto pela última vez. Autoridade = proprietário do NFT. | 1 por posição |
| Mint do NFT de posição | Mint com suprimento 1, associado a `PersonalPositionState`. Transferir transfere a posição. | 1 por posição |
...
O estado ativo do pool, lido em cada swap e cada mudança de posição.
...
Campos que você realmente vai usar:
...
- **`tick_array_bitmap`** é um bitmap **inline** cobrindo o intervalo comumente usado ao redor do preço spot. Para pools cujas posições alcançam longe, o rastreamento de overflow vive na conta separada `TickArrayBitmapExtension`.
...
- **`dynamic_fee_info`** carrega o estado de volatilidade para a sobretaxa de taxa dinâmica. Quando habilitado, cada swap recalcula um `dynamic_fee_component` além de `AmmConfig.trade_fee_rate`. O layout é documentado sob `DynamicFeeInfo` abaixo; pools sem taxa dinâmica deixam toda a struct em zero.
...
Um conjunto típico publicado de camadas de taxa CLMM (confirme contra `GET https://api-v3.raydium.io/main/clmm-config`):
...
### Mint do NFT de posição

Um mint de Token SPL com suprimento 1. O endereço do mint é um PDA determinístico; o NFT de posição na carteira do proprietário é apenas um ATA mantendo esse único token. Transferir o NFT é como uma posição muda de mãos — o programa vincula a autorização ao **detentor atual do saldo ATA do NFT**, não a um Pubkey armazenado no estado.

### `PersonalPositionState`

Um por posição aberta. Vinculado ao mint do NFT.
...
<Note>
  Versões mais antigas do CLMM armazenavam agregação por `(pool, tick_lower, tick_upper)` em um PDA `ProtocolPositionState`. **Versões mais novas não criam ou leem mais essa conta.** O slot ainda aparece nas listas de contas `OpenPosition` / `IncreaseLiquidity` / `DecreaseLiquidity` como um `UncheckedAccount` para compatibilidade ABI, mas o programa não escreve nela. Contas existentes on-chain são vestigiais; o admin pode chamar `CloseProtocolPosition` para recuperar aluguel para elas.
...
O buffer de observação do CLMM armazena um **tick cumulativo**, não um preço cumulativo. Consumidores externos calculam o preço de média geométrica ao longo de um intervalo de `(tick_cumulative[t1] − tick_cumulative[t0]) / (t1 − t0)` e então `price = 1.0001 ** tick`. Veja [`algorithms/clmm-math`](/pt/algorithms/clmm-math).
...
Seed do PDA: `["dynamic_fee_config", index.to_be_bytes()]`. Criado via `create_dynamic_fee_config` (com gate de admin) e modificado via `update_dynamic_fee_config`. Um pool criado com `enable_dynamic_fee = true` faz um snapshot dos cinco parâmetros de calibração da config (`filter_period`, `decay_period`, `reduction_factor`, `dynamic_fee_control`, `max_volatility_accumulator`) em seu próprio `DynamicFeeInfo` no momento da criação; edições posteriores ao `DynamicFeeConfig` não afetam retroativamente pools existentes.
...
Seed do PDA: `["permission", authority.as_ref()]`. Criado por um admin via `CreatePermissionPda` e destruído via `ClosePermissionPda` (aluguel reembolsado para o chamador). Ambas as instruções de admin aceitam o `admin` do programa ou uma chave dedicada `permission_pda_admin`. Fechar o PDA revoga a concessão — a autoridade não pode mais criar pools adicionais, mas pools que já criou não são afetados.
...
| `OpenPosition[WithToken22Nft]` | Mint do NFT + ATA, `personalPosition`, possivelmente novo(s) `tickArrayState`, `tickArrayBitmapExtension` se ainda não existente | — |
| `IncreaseLiquidity` | Possivelmente novo(s) `tickArrayState` | — |
| `DecreaseLiquidity` | — | Possivelmente limpa entradas de tick (mas o `tickArrayState` em si não é fechado) |
| `ClosePosition` | — | Mint do NFT, `personalPosition` |
...
| `CreateCustomizablePool` | `poolState`, `observation`, vaults — igual a `CreatePool`. Faz snapshot de `dynamicFeeConfig` se `enable_dynamic_fee = true`. | — |
