...
1. **Decaimento da referência**. Se `now - last_update_timestamp >= filter_period`, a referência de volatilidade decai. Note que ambos os limites são `>=`, não `>`:
...
Os campos do layout são `tokenFeesOwedA` / `tokenFeesOwedB` (não `...Owed0` / `...Owed1`), e cada entrada de `rewardInfos` é `{ growthInsideLastX64, rewardAmountOwed }` — o mint de recompensa em si vem do `rewardInfos` do pool, não do da posição.

`tokenFeesOwed*` e `rewardAmountOwed` são snapshots do momento em que a posição foi tocada pela última vez. Para ver os valores **atuais** (refletindo o crescimento desde então), simule um `DecreaseLiquidity` com liquidez zero, ou recalcule a partir do `fee_growth_*` global e dos dois snapshots tick-outside.
