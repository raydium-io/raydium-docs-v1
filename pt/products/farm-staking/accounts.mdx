...
O SDK retorna a visão de nível de exibição em `raydium.api.fetchFarmInfoById({ ids })` e as chaves de conta brutas — autoridade, vault de LP, vaults de recompensa por slot — em `raydium.api.fetchFarmKeysById({ ids })`. (Não existe `raydium.farm.getFarmById`.) Os mesmos dados estão disponíveis por HTTP em `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
