...
<Info>
  Stable AMM é seu próprio programa; sua estrutura de contas no lado do pool se assemelha à AMM v4 (AmmInfo, vaults, authority), e adicionalmente possui uma conta `ModelDataInfo` que armazena a tabela de consulta. Esta página cobre ambas.
</Info>
...
| Pool | `amm_target_orders` | Programa Stable | Conta de grade no lado do pool (retida em layouts; não mais impulsiona pedidos do OpenBook). |
| Pool | `pool_coin_token_account` | SPL Token | Vault no lado da moeda do pool. |
| Pool | `pool_pc_token_account` | SPL Token | Vault no lado pc do pool. |
...
| Legado | `serum_bids`, `serum_asks` | OpenBook | Filas de compra/venda. **Não utilizadas.** |
...
- **`fees`** — *estrutura* idêntica ao AMM v4, *valores* diferentes: `swap_fee = 2 / 10_000` (0,02%), `pnl = 12 / 100`, `trade_fee = 0`. Veja [`products/stable/fees`](/pt/products/stable/fees).
- **`coin_vault`**, **`pc_vault`** — os vaults do pool.
- **`status`** — bitmask que controla swap/deposit/withdraw/crank.
...
Ao preencher a tabela, o admin especifica esses valores pré-escalados. O programa não valida a ordem de classificação on-chain (por velocidade), então uma classificação incorreta causa cotações incorretas.
...
- **`need_take_pnl_coin`**, **`need_take_pnl_pc`** — taxas de protocolo acumuladas mas ainda não removidas. `WithdrawPnl` as move.
- **`swap_coin_in_amount`**, `swap_pc_in_amount`, etc. — contadores de análise.

**Cálculo de ativos do pool (pós-desacoplamento).** Como nenhum fundo é mais mantido em custódia como pedidos abertos do OpenBook, os ativos totais do pool agora são calculados inteiramente a partir dos vaults:

```
Antigo: ativos totais = saldos de vault + fundos de pedidos abertos (native_coin_total / native_pc_total) − PnL pendente (need_take_pnl)
...
A **`ModelDataInfo` é grande** (~1,2 MB, já que 50.000 elementos × 24 bytes por elemento). É por isso que criar um pool Stable requer alocação explícita de rent e pré-alocação de conta. O SDK e ferramentas do Raydium lidam com isso de forma transparente; integradores raramente precisam alocar manualmente.
