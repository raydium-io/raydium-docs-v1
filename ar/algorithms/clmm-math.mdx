...
2. **التحكم في الامتلاء.** `sqrt_price · L` يناسب `u256` لجميع المعاملات المعقولة؛ `price · L` قد يتجاوز الحد أبكر بكثير.
3. **حسابيات التيك موحدة.** لأن التيكات معرّفة كـ `1.0001^i`، فإن `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` هو أيضًا سلم هندسي دقيق. (`√1.0001 = 1.00004999875…`، لذا فإن التعامل معه كـ `1.00005` بالضبط ينحرف بما يصل إلى ~0.6% عند `MAX_TICK`.) كل عبور تيك ينقل إلى ضرب صغير في فضاء `sqrt_price_x64`.
...
`tick_i` هو `i32`. النطاق النشط هو `[MIN_TICK, MAX_TICK] = [−443636, 443636]`، مما يعطي نطاق سعر تقريبي `[2^−64, 2^64]` — أي من نحو `5.4e-20` إلى `1.8e19`. يتم تعيين `tick_spacing` لكل مجموعة من قبل طبقة الرسوم الخاصة بها: مسافات أصغر للأزواج الضيقة (مثلاً المستقرات 0.01% تستخدم المسافة 1)، مسافات أكبر للأزواج المتقلبة (طبقة 0.25% تستخدم 60، طبقة 1% تستخدم 120).
...
- **الأوراكل.** تخزن الحاوية الحلقية `ObservationState` الخاصة بـ Raydium فقط `(block_timestamp: u32, tick_cumulative: i64)` لكل فتحة، بالإضافة إلى `[u64; 4]` من الحشو المحجوز — **لا** يوجد حقل `seconds_per_liquidity_cumulative`، لذا فإن اشتقاقات Uniswap الموزونة بالسيولة غير متاحة على السلسلة.
