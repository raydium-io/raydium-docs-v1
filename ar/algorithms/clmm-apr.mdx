...
- فئة الرسوم: 0.05% (حصة المراوح **84%** من الرسوم — كل `AmmConfig` لـ CLMM على mainnet يحمل `protocol_fee_rate = 12%` بالإضافة إلى `fund_fee_rate = 4%`)
...
تعرض واجهة Raydium **102% APR** للمجمع. (يستخدم هذا المثال مجمعاً عميقاً بحجم كبير لإبقاء الأرقام محددة؛ عادة ما تعرض مجمعات CLMM نموذجية ما بين 10% إلى 50% APR، مع مجمعات منخفضة الحجم أقل بكثير من 10%.)
...
تكشف SDK الخاصة بـ Raydium عن `PoolUtils.estimateAprsForPriceRangeMultiplier` و `PoolUtils.estimateAprsForPriceRangeDelta` لهذا التقدير.
...
يُرجع المساعد `{ feeApr, rewardsApr[], apr }` — `rewardsApr` لكل مجرى مكافآت و `apr` هو المجموع. لا توجد طريقة `raydium.clmm.estimatedApr`. و `estimateAprsForPriceRangeDelta` هي نفس الحساب لكن مُعامَل بدلتا ± حول السعر الفوري بدلاً من تيكات صريحة.
...
- **"يتم مضاعفة APR تلقائياً."** لا. يجب المطالبة بالرسوم — عبر `DecreaseLiquidity` بسيولة صفرية، لأن CLMM لا يحتوي على تعليمة تحصيل مستقلة — ثم إعادة نشرها يدويّاً. لا تقوم Raydium بمضاعفة رسوم CLMM تلقائياً.
...
- تنفيذات Raydium SDK v2 لـ `PoolUtils.estimateAprsForPriceRangeMultiplier` / `estimateAprsForPriceRangeDelta`.
