...
<Info>
  مصفوفة الدعم: **CPMM** يدعم Token-2022 بالكامل، بما في ذلك الرموز ذات رسوم النقل. **CLMM** يدعم Token-2022 مع رسوم النقل عبر حسابات `SwapV2` الصريحة. **AMM v4** لا يدعم Token-2022 على الإطلاق. **LaunchLab** ينشئ رمز أساس SPL كلاسيكياً عبر `InitializeV2`، لكن لديه نقطة دخول مخصصة `InitializeWithToken2022` تنشئ رمز أساس Token-2022 وتُلحق به اختيارياً `TransferFeeConfig`؛ ويقبل جانب الاقتباس لديه أي من برنامجَي الرموز. انظر [`products/launchlab/platform-config`](/ar/products/launchlab/platform-config#token-2022-transfer-fee-authorities). **Farm v6** يدعم Token-2022 على كل من رموز الرهن والمكافأة.
</Info>
...
≈ 986.03 USDC. لا رسوم نقل صادرة، لذا يستقبل المستخدم ≈ 986.03 USDC.
...
- [وثائق امتداد رسوم النقل في SPL Token-2022](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
