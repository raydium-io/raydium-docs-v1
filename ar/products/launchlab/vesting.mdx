...
| 6 | `user_base_token` | W |   | ATA المستفيد على عملة الأساس؛ يتلقى الرموز المفتوحة. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | عملة الأساس للمجمع. |
| 8 | `base_token_program` |   |   | البرنامج الذي يملك عملة الأساس — SPL Token أو Token-2022. |
| 9 | `system_program` |   |   | يموّل ATA المستفيد عندما يتعيّن إنشاؤه. |
| 10 | `associated_token_program` |   |   | ينشئ ATA المستفيد. |

<Warning>
  يأتي `system_program` **قبل** `associated_token_program`. وكلاهما مكتوب كـ `Program<'info, _>`
  بعنوان ثابت، لذا فإن تبديل الزوج يفشل في قيد العنوان ولا يستطيع المستفيد المطالبة.
</Warning>

**الشروط المسبقة**

- `block_time >= pool_state.vesting_schedule.start_time` (وإلا `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — يجب أن يكون التخرج قد حدث بالفعل. والاستدعاء قبل ذلك يعود بخطأ `PoolNotMigrated`.
...
في `2027-09-28` (90 يوم بعد `start_time`)، يستدعي المستفيد أ `ClaimVestedToken`:
...
بعد ستة أشهر (`2028-03-26`، 270 يوم بعد `start_time`)، يطالب أ مرة أخرى:
