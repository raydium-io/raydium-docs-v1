...
يعيد SDK العرض على مستوى الواجهة عبر `raydium.api.fetchFarmInfoById({ ids })`، ومفاتيح الحساب الخام — السلطة، وخزينة LP، وخزائن المكافآت لكل فتحة — عبر `raydium.api.fetchFarmKeysById({ ids })`. (لا وجود لـ `raydium.farm.getFarmById`.) والبيانات نفسها متاحة عبر HTTP على `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
