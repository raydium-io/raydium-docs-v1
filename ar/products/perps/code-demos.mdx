...
<Info>
  **Raydium Perps هو نشر بعلامة بيضاء على شبكة Orderly Network.** دفتر الطلبات، محرك المطابقة، وحالة الحساب كلها توجد على Orderly. Raydium SDK v2 (`@raydium-io/raydium-sdk-v2`) **لا يغطي** perps — للوصول البرمجي، استخدم واجهة برمجة تطبيقات Orderly REST + WebSocket مباشرة. تعرض المقاطع أدناه التدفقات الأكثر شيوعًا؛ والمرجع المعتمد هو [مقدمة واجهة Orderly البرمجية](https://orderly.network/docs/build-on-omnichain/introduction)، ومعها [نقاط نهاية REST](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) و [تدفقات WebSocket](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction) الموثّقة تحتها.
</Info>
...
<Info>
  **لافتة الإصدار.**
  ...
  - التوقيع: Solana ed25519 على حمولة Orderly بنمط EIP-712 (Orderly يستخدم مخطط EIP-712 حتى للسلاسل غير EVM؛ راجع [مستندات Orderly](https://orderly.network/docs/build-on-omnichain/introduction) للحصول على قائمة الحقول الأحدث)

  تتطور سطح واجهة برمجة تطبيقات Orderly؛ تحقق من [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction) قبل نسخ هذه المقاطع إلى الإنتاج.
</Info>
...
- [توثيق مطور Orderly Network](https://orderly.network/docs/build-on-omnichain/introduction) — المرجع المعتمد لسطح الواجهة البرمجية المستخدم أعلاه. يستهلك Raydium Perps هذا مباشرة.
- [السلاسل المدعومة في Orderly](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — يؤكّد حالة Solana وعناوين خزائن التسوية لكل سلسلة.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — غلاف JS مرئي للمجتمع فوق طبقة REST نفسها. وليس له **أي إصدارات منشورة** ولا سياسة صيانة معلنة، لذا تعامل معه ككود مرجعي وليس كاعتماد؛ فطبقة التوقيع أعلاه صغيرة بما يكفي لتملكها بنفسك.
