...
1. **تضعيف المرجع**. إذا كان `now - last_update_timestamp >= filter_period`، يتراجع مرجع التقلب. لاحظ أن كلا الحدّين `>=` وليس `>`:
...
عندما يصبح `now > end_time` ينتقل التدفق إلى حالة `Ended` غير أن `reward_growth_global_x64` يستمر في القراءة — ويظل بإمكان مزودي السيولة تنفيذ `CollectReward` للمبالغ المكتسبة تاريخياً بعد توقف الإصدار.
...
حقول التخطيط هي `tokenFeesOwedA` / `tokenFeesOwedB` (وليست `...Owed0` / `...Owed1`)، وكل مدخل في `rewardInfos` هو `{ growthInsideLastX64, rewardAmountOwed }` — أما mint المكافأة نفسه فيأتي من `rewardInfos` الخاص بالمجمع، لا الخاص بالمركز.

`tokenFeesOwed*` و`rewardAmountOwed` هما لقطتان من آخر مرة تم فيها تحديث المركز. للاطلاع على القيم **الحالية** (التي تعكس النمو منذ ذلك الحين)، حاكِ `DecreaseLiquidity` بسيولة صفرية، أو أعِد الحساب من `fee_growth_*` العالمي ولقطات "خارج النقطة" لكلا طرفي النطاق.
