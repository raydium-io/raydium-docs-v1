...
Some IDLs can also be pulled directly from mainnet:
...
| Program | Where its IDL lives | Account | Reports |
|---------|--------------------|---------|---------|
| CLMM | Legacy Anchor `anchor:idl` account | `FbDNAPsgm8Y5jWXQkXjpfN6Bp8xHeRkhMmsstAooPyY9` | `raydium_clmm` 0.1.0 — 38 instructions, 52 errors |
| LaunchLab | Legacy Anchor `anchor:idl` account | `E6wT2uNeoWUvDrwdch1R8ETsyewR1ZM4WAwsa5hLJK5Z` | `raydium_launchpad` 0.2.0 — 27 instructions |
| Lock (Burn \& Earn) | Legacy Anchor `anchor:idl` account | `HzLkUWn57cdtyQNQLJpyu2EPF8qQAiGt4iuiEo1XnEFK` | 4 instructions (pre-`metadata` IDL spec) |
| **CPMM** | **Program Metadata program** `ProgM6JCCvbYkfKqJYHePx4xxSUSqJp7rh8Lyv7nk7S` | `FHuJXL7cEttfHk4sXdTAXVaRymBdKfVs7ZH6cMgA1ihk` (canonical, seed `idl`) | `raydium_cp_swap` 0.2.0 |
| AMM v4, Stable AMM, AMM Routing, Farm v3 / v5 / v6 | Nowhere | — | Non-Anchor programs; no IDL is published |
...
# Or with the `@anchor-lang/core` TS helpers at runtime:
```
...
// Anchor >= 0.30 reads the program id from `idl.address` — these IDLs are spec 0.1.0
// and carry it. The three-argument form `new Program(idl, programId, provider)` is
// pre-0.30 and throws.
...
```toml
# Cargo.toml — the dependency key must be the crate's PACKAGE name.
[dependencies]
