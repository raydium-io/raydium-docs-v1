...
Any MCP-compatible client can point at `https://docs.raydium.io/mcp`. That endpoint *is* the discovery surface — a `GET` returns the server config (name, version, transport, instructions and the tool list), so no separate manifest file is needed or served.
...
- [Mintlify `llms.txt` support](https://www.mintlify.com/docs/ai/llmstxt) and [AI ingestion](https://mintlify.com/docs/ai-ingestion).
