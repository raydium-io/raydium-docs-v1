...
The SDK returns the display-level view on `raydium.api.fetchFarmInfoById({ ids })` and the raw account keys — authority, LP vault, per-slot reward vaults — on `raydium.api.fetchFarmKeysById({ ids })`. (There is no `raydium.farm.getFarmById`.) The same data is available over HTTP at `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
