...
<Info>
  **2026-09 program upgrade.** AMM v4 was rebuilt against `solana-program` 3.0 with `spl-token` 9.0 and `spl-associated-token-account` 8.0, and gained one admin instruction, [`WithdrawExcessLamports`](/products/amm-v4/instructions#withdrawexcesslamports), for reclaiming rent freed by [SIMD-0437](/solana-fundamentals/rent-and-reclaimable-rent). Nothing a trader or LP calls changed: swap, deposit and withdraw keep their account layouts, arguments and math. There is no breaking change — the one account-list change, `CreateConfigAccount` dropping its trailing rent sysvar, stays compatible with tooling that still passes it. See the [2026-09-09 changelog entry](/reference/changelog/2026-09-09-amm-v4-solana-3-and-excess-lamports).
</Info>
