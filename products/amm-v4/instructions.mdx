...
Admin management of the program-level `AmmConfig` PDA (seed `["amm_config_account_seed"]`), which holds the PnL owner, the create-pool fee, and the pool-creation flag.
...
| The AMM v4 program itself (`AmmInfo`, `TargetOrders`, `AmmConfig`, the withdraw queue) | The program debits the account directly down to `rent.minimum_balance(data_len)`. |
