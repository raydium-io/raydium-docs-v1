...
<Info>
  **Creator-fee collection update.** Any payer can trigger `CollectCreatorFeePermissionless`. Funds still go only to the pool creator's canonical token accounts, and the existing creator-signed `CollectCreatorFee` remains available. Since 2026-09-13 both paths hand a configurable share of the accrued creator fee to the protocol on the way out, and both take two extra accounts. See [Instructions](/products/cpmm/instructions#collectcreatorfeepermissionless) and [Fees](/products/cpmm/fees#protocol-share-of-the-creator-fee).
</Info>
