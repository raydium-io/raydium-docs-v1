...
<Info>
  **Creator-fee collection update.** Any payer can trigger `CollectCreatorFeePermissionless`. Funds still go only to the pool creator's canonical token accounts, and the existing creator-signed `CollectCreatorFee` remains available. See [Instructions](/products/cpmm/instructions#collectcreatorfeepermissionless).
</Info>
