...
<Info>
  **Creator-fee collection update.** Any payer can trigger `CollectCreatorFeePermissionless`. Funds still go only to the pool creator's canonical token accounts, and the existing creator-signed `CollectCreatorFee` remains available. Since 2026-09-19 both paths hand a configurable share of the accrued creator fee to the protocol on the way out, and both take extra accounts appended at the end of their lists — one on `CollectCreatorFee`, two on `CollectCreatorFeePermissionless`. See [Instructions](/products/cpmm/instructions#collectcreatorfeepermissionless) and [Fees](/products/cpmm/fees#protocol-share-of-the-creator-fee).
</Info>

- **Token-2022 with a vetted extension allow-list.** CPMM does **not** accept arbitrary Token-2022 mints. The program enforces a whitelist of safe extensions at pool creation: `TransferFeeConfig`, `MetadataPointer`, `TokenMetadata`, `InterestBearingConfig`, and `ScaledUiAmount`. Any other extension on the mint causes `Initialize` to reject with `NotSupportMint` — unless the mint has been registered through its `SupportMintAssociated` PDA, which is the only bypass since the 2026-09 upgrade removed the hardcoded mint whitelist. Transfer fees in particular affect swap math and are applied on the correct side of the trade — see [`algorithms/token-2022-transfer-fees`](/algorithms/token-2022-transfer-fees).
- **Predictable fees.** Each pool references an `AmmConfig` selected at creation. The config carries a trade-fee rate (split between LPs, protocol, and fund) and a separate, independent creator-fee rate. The creator fee is its own bucket — never a slice of the trade fee. Pools opt into charging it at creation. The protocol can retain a share **of the creator bucket**, set per fee tier or per creator and applied when the creator fee is collected, which leaves swap pricing untouched. Defaults and the full split math are in [`products/cpmm/fees`](/products/cpmm/fees).
...
Fee collection is a separate transaction; it does not happen on every swap. Protocol and fund collection require their respective authorities. Creator fees can be collected either by the creator-signed path or by a permissionless caller through a destination-constrained path; either way the protocol's configured share of the accrued creator fee is moved into the pool's protocol bucket rather than paid out. The math is spelled out in [`products/cpmm/math`](/products/cpmm/math) and the instruction set in [`products/cpmm/instructions`](/products/cpmm/instructions).
