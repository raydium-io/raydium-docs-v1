...
- **`creator_fees_token_{0,1}`** — accrued creator fees, swept by `CollectCreatorFee` or `CollectCreatorFeePermissionless`. Both paths zero the full counters; the permissionless path fixes the recipients to `pool_creator`'s canonical ATAs.
