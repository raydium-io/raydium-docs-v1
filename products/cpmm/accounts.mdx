...
The Permission PDA is created via `CreatePermissionPda` by either the CPMM admin or a dedicated permission-PDA creator authority. Only the CPMM admin can revoke it via `ClosePermissionPda`. End users do not interact with this account directly — it is plumbing for cross-program flows. See [`security/admin-and-multisig`](/security/admin-and-multisig) for the role boundary and [`reference/program-addresses`](/reference/program-addresses) for canonical addresses.
