...
<Note>
  **The protocol's share of the creator fee does not appear anywhere on this page, and that is deliberate.** It is applied when `CollectCreatorFee` moves accrued fees between two counters that the curve already excludes — not during a swap. Swap math, quotes and `k` are identical with and without it. See [`products/cpmm/fees`](/products/cpmm/fees#protocol-share-of-the-creator-fee).
</Note>
