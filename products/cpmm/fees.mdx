...
- **Swept via `CollectCreatorFee` or `CollectCreatorFeePermissionless`.** The original path requires `PoolState.pool_creator` to sign. The permissionless path lets any payer trigger collection but fixes both destinations to the creator's canonical ATAs.
...
| Any payer; creator does not sign | `CollectCreatorFeePermissionless` | `creator_fees_token{0,1}` | Anytime; creator ATAs are fixed destinations |
