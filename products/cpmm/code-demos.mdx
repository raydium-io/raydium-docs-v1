...
<Info>
  **Version banner.** All TypeScript demos target `@raydium-io/raydium-sdk-v2@0.2.42-alpha` against Solana mainnet-beta, verified 2026-04. The Rust CPI skeleton targets `raydium-cp-swap` on the `master` branch, which requires Anchor `0.32.1` (`anchor-lang = "0.32.1"` in its `Cargo.toml`; CLMM pins the same version exactly as `=0.32.1`). Program IDs are pulled via constants from [`reference/program-addresses`](/reference/program-addresses).
</Info>
...
Protocol and fund collection remain authority-gated. Creator fees have two paths: the existing creator-signed `CollectCreatorFee`, and the additive `CollectCreatorFeePermissionless`, which any payer can invoke but which can transfer only to the creator's canonical ATAs. The pinned SDK exposes the existing raw builders:
...
<Warning>
  `@raydium-io/raydium-sdk-v2@0.2.42-alpha` does not yet expose a builder for `CollectCreatorFeePermissionless`. Until an SDK release adds it, build from the upgraded CPMM IDL or invoke the program directly. Do not reuse `makeCollectCreatorFeeInstruction`: the new instruction has a different discriminator and replaces the creator signer / `amm_config` positions with separate `payer` and non-signing `creator` accounts.
</Warning>
...
// anchor-lang     = "0.32.1"   # match the version raydium-cp-swap pins
// anchor-spl      = "0.32.1"
