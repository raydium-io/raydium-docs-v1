...
| Position NFT mint | Mint with supply 1, associated with `PersonalPositionState`. Its token account is unfrozen by default; a V2 open freezes it only when an underlying mint matches the restricted-issuer filter. | 1 per position |
...
| `Permission` | Grants its authority the right to call `CreatePermissionedPool`. Existence of the PDA **is** the grant. Admin-managed. | 1 per authorized creator |
...
- **`seed_index`** is `[0, 0]` for every pool created through `CreatePool` / `CreateCustomizablePool` (one canonical pool per pair). A **non-zero** value means the pool was created via `CreatePermissionedPool` and the index is part of the pool's PDA seeds, letting several pools coexist for the same `(config, mint0, mint1)`. To re-derive such a pool's address you must know its `seed_index`.
...
`PoolState.tick_array_bitmap` (inline) covers the "close to spot" range — **±512** tick arrays (`TICK_ARRAY_BITMAP_SIZE = 512`; the `[u64; 16]` field is 1,024 bits split evenly between negative and positive start indices, so array indices −512 … +511). Outside that range (for extreme tick values), the program maintains an extension account:
...
An SPL Token or Token-2022 mint with supply 1. The position NFT in the owner's wallet is an ATA holding that single token. The program keys authorization to the **current holder of the NFT's ATA balance**, not to a Pubkey stored in state.

New position NFT mints set `pool_state` as their freeze authority before minting the single token and removing mint authority. **Setting a freeze authority does not itself freeze the NFT account.** The account remains unfrozen and transferable unless both conditions hold: the caller uses `OpenPositionV2` or `OpenPositionWithToken22Nft`, and at least one underlying vault mint's freeze authority appears on CLMM's restricted-issuer list. Only then does CLMM freeze the NFT account after minting. This changes no `PersonalPositionState` or `PoolState` bytes.
...
A capability account whose **existence is the grant**: if a `Permission` PDA is derived for a given authority, that authority may call `CreatePermissionedPool`. It stores nothing beyond the authority it was created for.
...
PDA seed: `["permission", authority.as_ref()]`. Created by an admin via `CreatePermissionPda` and torn down via `ClosePermissionPda` (rent refunds to the caller). Both admin instructions accept either the program `admin` or a dedicated `permission_pda_admin` key. Closing the PDA revokes the grant — the authority can no longer create additional pools, but pools it already created are unaffected.
...
| `OpenPosition[V2]` / `OpenPositionWithToken22Nft` | NFT mint + ATA, `personalPosition`, possibly new `tickArrayState`(s), `tickArrayBitmapExtension` if not yet existing; may freeze the NFT ATA for a restricted-issuer pool | — |
...
| `ClosePosition` | — | NFT ATA, `personalPosition`, and Token-2022 NFT mint; a classic SPL NFT mint remains at supply zero. Frozen path first thaws via the pool passed in remaining accounts. |
...
| `CreatePermissionedPool` | `poolState` (at a `seed_index`-derived address), `observation`, vaults — like `CreateCustomizablePool`. Requires an existing `permission`. | — |
