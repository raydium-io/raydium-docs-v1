...
```rust
// Cargo.toml
// raydium-clmm = { git = "https://github.com/raydium-io/raydium-clmm", features = ["cpi"] }
// anchor-lang  = "0.32.1"

use anchor_lang::prelude::*;
use raydium_clmm::cpi;
use raydium_clmm::program::RaydiumClmm;
use raydium_clmm::cpi::accounts::SwapSingleV2;
...
<Note>
  The instruction is `swap_v2` and the IDL calls it `SwapV2`, but the Anchor accounts struct is named **`SwapSingleV2`** — `raydium_clmm::cpi::accounts::SwapV2` does not exist and fails to compile. `SwapV2` is only valid in the instruction-args namespace (`raydium_clmm::instruction::SwapV2`).
</Note>

Remaining-account order for `SwapSingleV2`:
...
- **Not enough tick arrays supplied in `SwapSingleV2`** → `TickArrayNotFound`. Use `computeAmountOutFormat` to get the full list.
