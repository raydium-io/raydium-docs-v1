...
1. **Decay reference**. If `now - last_update_timestamp >= filter_period`, the volatility reference decays. Note both boundaries are `>=`, not `>`:
...
The layout fields are `tokenFeesOwedA` / `tokenFeesOwedB` (not `...Owed0` / `...Owed1`), and each entry of `rewardInfos` is `{ growthInsideLastX64, rewardAmountOwed }` — the reward mint itself comes from the pool's `rewardInfos`, not the position's.

`tokenFeesOwed*` and `rewardAmountOwed` are snapshots from the last time the position was touched. To see the **current** values (reflecting growth since then), simulate a zero-liquidity `DecreaseLiquidity`, or recompute from the global `fee_growth_*` and the two tick-outside snapshots.
