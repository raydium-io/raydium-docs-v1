...
A later release adds **permissioned multi-pools**: `CreatePermissionedPool` lets a whitelisted operator (one holding a `Permission` PDA) create more than one pool for the same pair and fee tier by folding a `seed_index` into the pool address, and hardens `OpenLimitOrder` to reject orders whose input or output token account is frozen. See [Instructions → CreatePermissionedPool](/products/clmm/instructions) and the [2026-07-30 changelog entry](/reference/changelog/2026-07-30-clmm-permissioned-pools).

The 2026-08 release adds a **conditional position-freeze path for selected KYC-restricted issuer pools**. Every new position NFT mint uses the pool as freeze authority, but the NFT token account remains unfrozen and transferable by default. CLMM freezes it only when the position is opened through `OpenPositionV2` or `OpenPositionWithToken22Nft` **and** either pool mint's freeze authority matches the program's restricted-issuer list. The owner can still manage liquidity and close a matching position, but cannot transfer its NFT or change the token-account owner. Existing positions are not frozen retroactively. See [Ticks and positions → Restricted-issuer positions](/products/clmm/ticks-and-positions#restricted-issuer-positions) and the [2026-08-14 changelog entry](/reference/changelog/2026-08-14-clmm-restricted-position-nft-freeze).
...
- **NFT positions.** Each position is a non-fungible token. Transferring the NFT normally transfers the position; wallets and UIs can display positions the same way they display collectibles. Only a V2 position whose underlying mint matches the restricted-issuer filter is frozen and non-transferable.
...
Transferring an unfrozen position NFT transfers the position — the personal-position account's authority is the NFT holder. A V2 position stays bound to its original owner only if either underlying vault mint matches the restricted-issuer filter and CLMM freezes the NFT token account at creation. The position remains manageable and closable through CLMM. A detailed treatment is in [`products/clmm/ticks-and-positions`](/products/clmm/ticks-and-positions).
