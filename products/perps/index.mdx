...
- The spot Raydium SDK (`@raydium-io/raydium-sdk-v2`) does **not** cover perps. For programmatic access go straight to [Orderly's REST and WebSocket API](https://orderly.network/docs/build-on-omnichain/introduction). Orderly's own SDKs are UI-layer packages — [React](https://orderly.network/docs/sdks/react/overview) and [Hooks](https://orderly.network/docs/sdks/hooks/overview) — not a general-purpose TypeScript client, so a backend integration signs the API directly (see [`products/perps/code-demos`](/products/perps/code-demos)).
