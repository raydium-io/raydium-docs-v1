...
<Info>
  **Raydium Perps is a white-labeled deployment on Orderly Network.** The order book, matching engine, and account state all live on Orderly. The Raydium SDK v2 (`@raydium-io/raydium-sdk-v2`) does **not** cover perps — for programmatic access, use Orderly's REST + WebSocket API directly. The snippets below show the most common flows; the canonical reference is the [Orderly API introduction](https://orderly.network/docs/build-on-omnichain/introduction), with the [REST endpoints](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) and [WebSocket streams](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction) documented under it.
</Info>
...
<Info>
  **Version banner.**
  ...
  Orderly's API surface evolves; check the [Orderly API docs](https://orderly.network/docs/build-on-omnichain/introduction) before copying these snippets into production.
</Info>
...
- [Orderly Network developer documentation](https://orderly.network/docs/build-on-omnichain/introduction) — canonical reference for the API surface used above. Raydium Perps consumes this directly.
- [Orderly supported chains](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — confirms Solana's status and the settlement-vault addresses per chain.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — a community-visible JS wrapper over the same REST layer. It has **no published releases** and no stated maintenance policy, so treat it as reference code rather than a dependency; the signing layer above is small enough to own.
