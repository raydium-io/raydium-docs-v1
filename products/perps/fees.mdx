...
- **A directional trader that exclusively uses market orders** pays full taker bps; at tier 1 that is 4.5 bps a side — **0.09% per round-trip** (0.045% entry + 0.045% exit) — which is small per trade but compounds quickly for a high-turnover leveraged strategy.
...
**Against a tier-1 taker perp trade (4.5 bps), spot is roughly 5.6× more expensive on the 25 bps tiers and 22× on the 100 bps tier — but *cheaper* on the 1 bps CLMM tier (0.22×).** The products are not substitutes in any case: spot gives you actual asset ownership, perps give you leveraged synthetic exposure with funding overhead.
