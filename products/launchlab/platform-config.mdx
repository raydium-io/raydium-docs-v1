...
`platform_scale + creator_scale + burn_scale` must equal `1_000_000`, **and `creator_scale` must be exactly `0`** — the program asserts `require_eq!(creator_scale, 0)` on both `CreatePlatformConfig` and the `MigrateNftInfo` update path, so a settable split is really just `platform_scale + burn_scale = 1_000_000`. The field survives only so pre-2026-08-17 configs still decode; on those, whatever `creator_scale` they stored is folded into the platform's locked LP at migration rather than going to the token creator.

Settable examples:

- `(100_000, 0, 900_000)` — 90% LP burned, 10% locked to the platform.
- `(150_000, 0, 850_000)` — 85% burned, 15% locked to the platform.
