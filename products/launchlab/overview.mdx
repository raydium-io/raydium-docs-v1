...
- **Platform lock** — for migrations executed after the 2026-08-14 upgrade, `platform_scale + creator_scale` is locked through LP-Lock and one Fee Key goes to `platform_nft_wallet` when this share is non-zero. Before the upgrade, `creator_scale` produced a separate creator Fee Key.
