...
<Info>
  **2026-09 program upgrade.** LaunchLab was rebuilt on Anchor 1.0.2 / Solana 3.1.10 and gained one admin instruction, [`CollectExcessLamports`](/products/launchlab/instructions#collectexcesslamports), for reclaiming rent freed by [SIMD-0437](/solana-fundamentals/rent-and-reclaimable-rent). Three transition mechanisms were cleared out at the same time: the deprecated `Initialize` now always fails, `MigrateToAmm` lost its arguments and every OpenBook account, and the clock-based `get_upgrade_timestamp` gate is gone. Trading, fees and vesting are unchanged. See the [2026-09-09 changelog entry](/reference/changelog/2026-09-09-launchlab-anchor-1-and-excess-lamports).
</Info>

## Important invariants

- **The base mint is inflation-free from creation.** The initialize instruction mints the full supply and revokes `mint_authority` in the same call; `freeze_authority` was never set.
- **All new launches are CPMM-only.** `InitializeV2` and `InitializeWithToken2022` require `migrate_type = CPSWAP`, and the legacy `Initialize` fails outright as of the 2026-09 upgrade. Existing legacy state can still use `MigrateToAmm`.
