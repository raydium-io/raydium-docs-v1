...
| 6 | `user_base_token` | W |   | Beneficiary's ATA on the base mint; receives the unlocked tokens. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Pool's base mint. |
| 8 | `base_token_program` |   |   | The program that owns the base mint — SPL Token or Token-2022. |
| 9 | `system_program` |   |   | Funds the beneficiary ATA when it has to be created. |
| 10 | `associated_token_program` |   |   | Creates the beneficiary ATA. |

<Warning>
  `system_program` comes **before** `associated_token_program`. Both are typed `Program<'info, _>`
  with fixed addresses, so a swapped pair fails the address constraint and the beneficiary cannot
  claim.
</Warning>
...
- `pool_state.status == PoolStatus::Trade` (`2`) — graduation must have already happened. Calling before that reverts with `PoolNotMigrated`.
