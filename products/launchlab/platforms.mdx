...
- **Post-migration share** — before the 2026-08-14 upgrade, the creator scale produced a separate creator Fee Key. For migrations executed afterward, `platform_scale + creator_scale` becomes one platform-owned locked-LP position and `burn_scale` is burned.
