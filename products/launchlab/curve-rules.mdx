...
One group, three constraints, and the creator has a 50–200 SOL corridor. Under the old whitelist this needed one entry per permitted value, and the cap of ten entries made it impossible.
...
<Info>
  **Version banner.**
  ...
  - SDK: `@raydium-io/raydium-sdk-v2@0.2.64-alpha` — the version every other code demo on this site is pinned to, and one that already ships curve-rule support. Both helpers below, plus `getPdaPlatformCurveRule`, the `PlatformCurveRule` layout and the `LaunchpadCurveRuleField` / `LaunchpadCurveRuleOp` enums, are exported from the package barrel; no hand-porting from `platform_curve_rule.rs` is needed.
