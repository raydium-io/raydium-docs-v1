...
<Info>
  **Version banner.**
  ...
  - SDK: `@raydium-io/raydium-sdk-v2@0.2.64-alpha` is the version every other code demo on this site is pinned to. The two helpers below arrive with the SDK release that ships curve-rule support; until then, port them from the program's `platform_curve_rule.rs` or call the program and read the error code.
