...
The SDK reads this with `raydium.launchpad.getRpcPoolInfo({ poolId })`, which returns the decoded `PoolState` with the bound `GlobalConfig` attached as `configInfo` (`getRpcPoolsInfo({ poolIdList })` for the batched form). There is no `getLaunchById`. The pool id itself is a PDA of `(programId, mintA, mintB)` — `getPdaLaunchpadPoolId`.
...
`PoolStatus` values (the IDL variant names are `Fund` / `Migrate` / `Trade`):
...
- **`status`** — three values, monotone (`Fund` → `Migrate` → `Trade`). Reads always safe; writes gated. `ClaimVestedToken` requires `Trade`.
...
- Nothing on the `base_mint` — the mint authority is revoked before `InitializeV2` returns (see below).
...
The `mint_authority` is revoked in the **initialize** instruction, not at graduation: `InitializeV2` and `InitializeWithToken2022` mint the full supply into `base_vault` and then set `MintTokens` to `None` in the same instruction. The supply is therefore fixed from the moment the launch exists, and migration has no mint-authority step. A side effect: nobody can sign for the base mint, so its rent cannot be reclaimed by [`CollectExcessLamports`](/products/launchlab/instructions#collectexcesslamports).
...
| `CollectExcessLamports` | any | — | — | — (moves lamports only, never token balances) |
