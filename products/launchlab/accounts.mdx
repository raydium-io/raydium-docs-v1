...
The SDK's `raydium.launchpad.getRpcPoolInfo({ poolId })` returns the decoded `PoolState` plus the binding `GlobalConfig`; `getRpcPoolsInfo` does the same for a batch. Graduation is read from `PoolState.status`, not from a separate flag.

## `PoolState`

The per-launch root state — the launch state, named `PoolState` in the IDL. Field names below match the IDL; some values are simplified for readability.
...
- The vaults and PDAs listed above. It is **not** the `base_mint`'s mint authority — that is revoked at `Initialize`.
...
The `mint_authority` is revoked by `Initialize` itself, so the supply is permanently fixed from the moment the mint exists — not at graduation. Launches still in the funding phase already read `mintAuthority: null`.
...
- `mint_authority = null` (revoked by `Initialize`).
