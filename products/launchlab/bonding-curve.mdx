...
| `migrate_type` | Must be `CPSWAP` for new initialization. A stored AMM v4 value is legacy state only. See [`instructions`](/products/launchlab/instructions). |

Fee rates are not launch parameters — they come from the `GlobalConfig` the launch binds to and from its `PlatformConfig`. See [`global-config`](/products/launchlab/global-config) and [`platform-config`](/products/launchlab/platform-config).

Field names above match the fields described in [`accounts`](/products/launchlab/accounts); units are conceptual.
...
Quadratic in `base_sold` — early buyers pay close to zero, late buyers pay substantially more, with the marginal price always rising at a fixed slope.
...
- `k                      = 40` (price scale)
- Fees: 1% on buys and sells, split between protocol, platform and creator.
...
```
total_fee_rate = trade_fee_rate + platform_fee_rate + creator_fee_rate + share_fee_rate
total_fee      = ceil(quote_in × total_fee_rate / 1_000_000)

platform_share = total_fee × platform_fee_rate / total_fee_rate
creator_share  = total_fee × creator_fee_rate  / total_fee_rate
referral_share = total_fee × share_fee_rate    / total_fee_rate
protocol_share = total_fee − platform_share − creator_share − referral_share
```

- `total_fee` comes off **before** the curve prices the trade: the curve sees `quote_in − total_fee`, so no part of the fee becomes curve liquidity.
- `protocol_share` increments the launch's protocol-fee counter, swept by the admin with `CollectFee`.
- `platform_share` increments the platform-fee counter, claimed with `ClaimPlatformFee`.
- `creator_share` accrues to the creator's fee vault, claimed with `ClaimCreatorFee`.
- `referral_share` is paid out during the trade to the share receiver, and is zero unless the caller passes a `share_fee_rate`.
...
- Newton solves for "buy exact quote" and "sell exact quote" iterate in `u128` fixed-point with a configurable max iteration count (default 10). Failure mode is `MathOverflow` — rare outside of near-graduation edge cases.
