...
- `max_share_fee_rate` — initialized to `10_000` (100 bps). Caps the `share_fee_rate` **argument** that `BuyExactIn`, `BuyExactOut`, `SellExactIn`, and `SellExactOut` accept — the referral fee paid straight to the share receiver during the trade. It does **not** bound `PlatformConfig.fee_rate`; that rate has its own caps, described in [`platform-config`](/products/launchlab/platform-config#rate-caps). Both `CreateConfig` and `UpdateConfig` enforce `trade_fee_rate + max_share_fee_rate < RATE_DENOMINATOR_VALUE`.
