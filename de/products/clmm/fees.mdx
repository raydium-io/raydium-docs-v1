...
1. **Referenz abklingen lassen**. Wenn `now - last_update_timestamp >= filter_period`, klingt die Volatilitätsreferenz ab. Beachten Sie, dass beide Grenzen `>=` sind, nicht `>`:
...
Die Layout-Felder heißen `tokenFeesOwedA` / `tokenFeesOwedB` (nicht `...Owed0` / `...Owed1`), und jeder Eintrag von `rewardInfos` ist `{ growthInsideLastX64, rewardAmountOwed }` — der Reward-Mint selbst kommt aus den `rewardInfos` des Pools, nicht aus denen der Position.

`tokenFeesOwed*` und `rewardAmountOwed` sind Snapshots vom letzten Zeitpunkt, zu dem die Position berührt wurde. Um die **aktuellen** Werte (einschließlich des seither aufgelaufenen Wachstums) zu sehen, simulieren Sie ein `DecreaseLiquidity` mit null Liquidität, oder berechnen Sie sie aus dem globalen `fee_growth_*` und den zwei Tick-Outside-Snapshots neu.
