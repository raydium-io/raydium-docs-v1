...
| Kategorie | Konto | Eigentümer | Rolle |
|----------|---------|-------|------|
| Pool | `AmmInfo` | Stable-Programm | Pool-Status, Verweise auf Vaults und das Model-Data-Konto. |
| Pool | `amm_authority` | Stable-Programm | Programmeigene PDA, die Vault-Bewegungen signiert. Wird von allen Stable AMM-Pools gemeinsam genutzt. |
...
| Pool | `pool_coin_token_account` | SPL Token | Vault der Pool-Coin-Seite. |
| Pool | `pool_pc_token_account` | SPL Token | Vault der Pool-PC-Seite. |
...
| Veraltet | `serum_bids`, `serum_asks` | OpenBook | Geld-/Brief-Warteschlangen. **Ungenutzt.** |
...
- **`fees`** — identische *Struktur* zu AMM v4, unterschiedliche *Werte*: `swap_fee = 2 / 10_000` (0,02%), `pnl = 12 / 100`, `trade_fee = 0`. Siehe [`products/stable/fees`](/de/products/stable/fees).
- **`coin_vault`**, **`pc_vault`** — die Vaults der Pools.
- **`status`** — Bitmask, das Swap/Deposit/Withdraw/Crank gatet.
- **`out_put.need_take_pnl_*`** — wird von `WithdrawPnl` geleert.

## `ModelDataInfo`

Die Nachschlagetabelle. Ein großes spärliches Array von Preis-/Mengen-Punkten.
...
- **Vaults** sind SPL Token-Konten, deren Eigentümer `amm_authority` ist, nicht ATAs.
...
- **`need_take_pnl_coin`**, **`need_take_pnl_pc`** — Protokollgebühren, die aufgelaufen, aber noch nicht geleert wurden. `WithdrawPnl` bewegt diese heraus.
...
Das **`ModelDataInfo` ist groß** (~1,2 MB, da 50.000 Elemente × 24 Bytes pro Element). Deshalb erfordert das Erstellen eines Stable-Pools explizite Rent- und Konten-Vorabzuweisung. Das Raydium SDK und die Tools handhaben dies transparent; Integratoren müssen selten manuell zuweisen.
