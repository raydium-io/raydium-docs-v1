...
Das SDK gibt die Anzeige-Ansicht über `raydium.api.fetchFarmInfoById({ ids })` zurück und die rohen Konten-Keys — Authority, LP-Vault, Reward-Vaults pro Slot — über `raydium.api.fetchFarmKeysById({ ids })`. (Ein `raydium.farm.getFarmById` gibt es nicht.) Dieselben Daten sind über HTTP unter `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>` verfügbar.
