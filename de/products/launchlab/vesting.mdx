...
| 6 | `user_base_token` | W |   | ATA des Begünstigten auf dem Basis-Mint; empfängt die entsperrten Token. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Basis-Mint des Pools. |
| 8 | `base_token_program` |   |   | Das Programm, dem der Basis-Mint gehört — SPL Token oder Token-2022. |
| 9 | `system_program` |   |   | Finanziert das ATA des Begünstigten, wenn es erstellt werden muss. |
| 10 | `associated_token_program` |   |   | Erstellt das ATA des Begünstigten. |

<Warning>
  `system_program` kommt **vor** `associated_token_program`. Beide sind als `Program<'info, _>`
  mit festen Adressen typisiert, sodass ein vertauschtes Paar die Adress-Constraint verletzt und der
  Begünstigte nicht claimen kann.
</Warning>

**Vorbedingungen**

- `block_time >= pool_state.vesting_schedule.start_time` (andernfalls `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — die Graduation muss bereits stattgefunden haben. Ein Aufruf davor wird mit `PoolNotMigrated` rückgängig gemacht.
- Das entsperrte-Betrag-Delta ist größer als Null. Ein No-Op-Aufruf (berechnetes Delta ist 0) wird rückgängig gemacht.
...
Am `2027-09-28` (90 Tage nach `start_time`) ruft Begünstigter A `ClaimVestedToken` auf:
...
Sechs Monate später (`2028-03-26`, 270 Tage nach `start_time`) macht A Anspruch:
