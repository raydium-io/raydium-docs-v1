...
(Für die nicht normalisierte Form `p(s) = k · s²` entfällt das `S_max²`: `P(s) = (k/3) · s³`. Wählen Sie eine Konvention und bleiben Sie dabei — das ausgearbeitete Beispiel unten verwendet durchgehend die normalisierte Form.)

Der Preis steigt quadratisch. Frühe Käufer erhalten einen nahezu Null-Preis (flache Startregion); späte Käufer zahlen einen steileren Aufschlag.

<Note>
  **LaunchLab implementiert keine quadratische *Preis*-Kurve.** Seine drei Kurventypen sind
  `0 = ConstantProduct` (der Standard), `1 = FixedPrice` und `2 = LinearPrice`. Die diesem Abschnitt
  am nächsten kommende Form ist `LinearPrice`, deren Preis `p(s) = a·s` ist und deren *Kosten* damit
  quadratisch sind. Dieser Abschnitt bleibt als allgemeine Behandlung einer Polynomkurve erhalten.
</Note>
...
Diese Variante hat die elegante Eigenschaft, dass bei der Graduierung (wo `s = S_graduate`) der Grenzpreis gleich dem Eröffnungspreis des nachgelagerten CPMM-Pools ist, der mit Reserves `(S_max − S_graduate, V_q + cost(0, S_graduate))` gefüllt wird. Die Übergabe ist nahtlos. LaunchLab stellt dies als `curve_type = 0` (`ConstantProduct`) bereit — der Standard und heute der einzige Typ mit aktiven Configs.
...
<Note>
  Dies ist das allgemeine numerische Rezept, nicht die Implementierung von LaunchLab. LaunchLab hat
  **keinen Newton-Solver und keinen `NotConverged`-Fehler** — keine seiner drei Kurven benötigt
  einen. Die Constant-Product-Kurve lässt sich in geschlossener Form invertieren, die
  Fixed-Price-Kurve ist eine Division, und die quadratischen Kosten der Linear-Price-Kurve
  invertieren sich mit einer Quadratwurzel. Selbst der oben behandelte Fall einer quadratischen
  Preiskurve hat die exakte geschlossene Form `s₁ = ∛(s₀³ + 3·cost/k)`, die das Snippet als
  Initialisierer verwendet.
</Note>
...
Für die quadratische Kurve gilt diese Gleichheit **nicht**: der integrierte Durchschnittspreis liegt strikt über dem Grenzpreis, sodass der CPMM um einen festen Faktor über `p(S_graduate)` eröffnet (siehe das ausgearbeitete Beispiel). Nur die Constant-Product-/Virtual-Reserves-Kurve — diejenige, die LaunchLab tatsächlich betreibt — ist bei der Übergabe preisstetig.
...
cpmm_quote ≈ 6.827e9   (minus Gebührenzähler-Abzüge)
cpmm_price ≈ 6.827e9 / 2e8 = 34.14 pro Base
```

<Warning>
  Das ist **nicht** `p(8e8) = 25.6` — es ist genau `4/3` davon. Für jede quadratische Preiskurve ist
  der integrierte Durchschnitt `p(s)/3 · s / s`, sodass `cost(0,s)/s = p(s)/3 · ...` auf
  `4/3 · p(s)` hinausläuft, sobald die verbleibende Supply `1 − 0.8` ist. Die quadratische Übergabe
  ist daher **nicht preisstetig**. Nur die Constant-Product-/Virtual-Reserves-Kurve besitzt die oben
  beschriebene Eigenschaft der exakten Gleichheit; ein quadratischer Launch zeigt bei der
  Graduierung eine Stufe.
</Warning>
