...
Auf Solana ist das, was widerrufen werden muss, ein SPL-Token-**Delegate**: die `Approve`-Instruktion benennt einen Delegate und einen Betrag auf einem Ihrer Token-Konten, und `Revoke` löscht ihn. Öffnen Sie in Ihrer Wallet die Ansicht der Token-Genehmigungen / verbundenen Apps und widerrufen Sie alles, was Sie nicht wiedererkennen; Phantom und Solflare zeigen dies beide an. Vom Terminal aus ist es ein Befehl pro Token-Konto:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

Dies verhindert zukünftige Drain-Transaktionen, stellt aber bereits geleerte Gelder nicht wieder her.

<Warning>
  [revoke.cash](https://revoke.cash) ist **nur für EVM** — es deckt Ethereum und über 100 EVM-Chains ab (einschließlich Neon, dem EVM-Layer von Solana), aber nicht Solana selbst. Schicken Sie Solana-Nutzer nicht dorthin.
</Warning>
