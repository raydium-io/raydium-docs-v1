...
|---------|---------------------|-------|
| **AMM v4** | `[b"amm authority"]` | **Einzelne PDA, die von allen AMM-v4-Pools gemeinsam genutzt wird.** Ihr Bump wird auf jedem `AmmInfo` als `nonce` gespiegelt. |
...
| **CLMM** | *(keine — `pool_state` ist selbst die Vault-Authority)* | Pro Pool. Vaults sind PDAs bei `[b"pool_vault", pool_state, mint]`, besessen von und signiert durch `pool_state`. CLMM hat keine globale Vault-Authority-PDA. |
| **Farm v3 / v5 / v6** | `[farmId]` | Pro-Farm. Die PDA besitzt Staking- + Reward-Vaults für diese Farm. |
| **LaunchLab** | `[b"vault_auth_seed"]` | **Einzelne PDA, die von allen Launches gemeinsam genutzt wird.** Zwei weitere globale PDAs, `[b"platform_fee_vault_auth_seed"]` und `[b"creator_fee_vault_auth_seed"]`, besitzen die aggregierten Gebühren-Vaults. |

Einige Dinge folgen daraus:

- Für AMM v4, CPMM und LaunchLab ist die Authority-PDA **ein globales Konto** — jeder Pool bzw. jeder Launch dieses Typs nutzt es. Wenn Sie in CPMM CPI-en, benötigen Sie es einmal, nicht pro Pool.
- CLMM ist die Ausnahme: es gibt überhaupt keine Authority-PDA. Jeder Vault ist eine PDA, die auf Pool und Mint geseedet ist, und das Pool-Konto signiert für sie.
- Für Pro-Farm-Authorities leiten Sie die PDA aus der Farm-ID ab. Das SDK macht dies in `getPoolKeys` / `getFarmKeys`; wenn Sie direkt integrieren, leiten Sie mit `findProgramAddressSync` ab.
