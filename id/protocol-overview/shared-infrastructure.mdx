...
|---------|---------------------|-------|
| **AMM v4** | `[b"amm authority"]` | **PDA tunggal yang dibagikan oleh semua pool AMM v4.** Bump-nya dicerminkan pada setiap `AmmInfo` sebagai `nonce`. |
| **CPMM** | `[b"vault_and_lp_mint_auth_seed"]` | **PDA tunggal yang dibagikan oleh semua pool CPMM.** Tidak ada seed spesifik pool. |
| **CLMM** | *(tidak ada — `pool_state` sendiri adalah otoritas vault)* | Per-pool. Vault adalah PDA di `[b"pool_vault", pool_state, mint]`, dimiliki oleh dan ditandatangani untuk oleh `pool_state`. CLMM tidak memiliki PDA otoritas vault global. |
| **Farm v3 / v5 / v6** | `[farmId]` | Per-farm. PDA memiliki vault staking + reward untuk farm tersebut. |
| **LaunchLab** | `[b"vault_auth_seed"]` | **PDA tunggal yang dibagikan oleh semua peluncuran.** Dua PDA global lainnya, `[b"platform_fee_vault_auth_seed"]` dan `[b"creator_fee_vault_auth_seed"]`, memiliki vault biaya teragregasi. |

Beberapa hal mengikuti dari ini:

- Untuk AMM v4, CPMM dan LaunchLab, PDA otoritas adalah **akun global** — setiap pool atau peluncuran dari tipe itu menggunakannya. Jika Anda melakukan CPI ke CPMM, Anda membutuhkannya sekali, bukan per pool.
- CLMM adalah pengecualiannya: tidak ada PDA otoritas sama sekali. Setiap vault adalah PDA yang di-seed pada pool dan mint, dan akun pool-lah yang menandatanganinya.
- Untuk otoritas per-farm, Anda menurunkan PDA dari ID farm. SDK melakukan ini di `getPoolKeys` / `getFarmKeys`; jika Anda mengintegrasikan secara langsung, Anda menurunkan dengan `findProgramAddressSync`.
