...
Di Solana yang perlu dicabut adalah **delegate** SPL-Token: instruksi `Approve` menyebutkan sebuah delegate dan sebuah jumlah pada salah satu akun token Anda, dan `Revoke` menghapusnya. Buka tampilan persetujuan token / aplikasi terhubung di dompet Anda dan cabut apa pun yang tidak Anda kenali; Phantom dan Solflare keduanya menampilkan ini. Dari terminal, satu perintah per akun token:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

Ini menghentikan transaksi pengurasan di masa depan tetapi tidak memulihkan dana yang sudah dikuras.

<Warning>
  [revoke.cash](https://revoke.cash) **hanya untuk EVM** — layanan ini mencakup Ethereum dan 100+ chain EVM (termasuk Neon, lapisan EVM Solana) tetapi bukan Solana itu sendiri. Jangan arahkan pengguna Solana ke sana.
</Warning>

### Anda melihat transaksi yang tidak Anda otorisasi

Dompet Anda mungkin telah disusupi. Pindahkan dana **segera**; jangan menyelidiki lebih dulu.

<Warning>
  [revoke.cash](https://revoke.cash) bersifat **EVM-only** — mencakup Ethereum dan 100+ chain EVM (termasuk Neon, lapisan EVM Solana) tetapi bukan Solana itu sendiri. Jangan arahkan pengguna Solana ke sana.
</Warning>
