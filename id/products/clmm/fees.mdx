...
1. **Decay reference**. Jika `now - last_update_timestamp >= filter_period`, referensi volatilitas meluruh. Perhatikan bahwa kedua batas memakai `>=`, bukan `>`:
...
Field pada layout-nya adalah `tokenFeesOwedA` / `tokenFeesOwedB` (bukan `...Owed0` / `...Owed1`), dan setiap entri `rewardInfos` berbentuk `{ growthInsideLastX64, rewardAmountOwed }` — mint reward-nya sendiri berasal dari `rewardInfos` milik pool, bukan milik posisi.

`tokenFeesOwed*` dan `rewardAmountOwed` adalah snapshot dari terakhir kali posisi disentuh. Untuk melihat nilai **saat ini** (yang mencerminkan pertumbuhan sejak saat itu), simulasikan `DecreaseLiquidity` berlikuiditas nol, atau hitung ulang dari `fee_growth_*` global dan dua snapshot tick-outside.
