...
| 6 | `user_base_token` | W |   | ATA penerima manfaat pada base mint; menerima token yang di-unlock. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Mint base pool. |
| 8 | `base_token_program` |   |   | Program yang memiliki base mint — SPL Token atau Token-2022. |
| 9 | `system_program` |   |   | Mendanai ATA penerima manfaat ketika ATA itu harus dibuat. |
| 10 | `associated_token_program` |   |   | Membuat ATA penerima manfaat. |

<Warning>
  `system_program` berada **sebelum** `associated_token_program`. Keduanya bertipe `Program<'info, _>`
  dengan alamat tetap, sehingga pasangan yang tertukar gagal pada constraint alamat dan penerima
  manfaat tidak dapat mengklaim.
</Warning>

**Prekondisi**

- `block_time >= pool_state.vesting_schedule.start_time` (sebaliknya `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — kelulusan harus sudah terjadi. Memanggilnya sebelum itu dibatalkan dengan `PoolNotMigrated`.
...
Pada `2027-09-28` (90 hari setelah `start_time`), Penerima A memanggil `ClaimVestedToken`:
...
Enam bulan kemudian (`2028-03-26`, 270 hari setelah `start_time`), A mengklaim lagi:
