...
SDK mengembalikan tampilan tingkat-display pada `raydium.api.fetchFarmInfoById({ ids })` dan kunci akun mentah — authority, vault LP, vault reward per-slot — pada `raydium.api.fetchFarmKeysById({ ids })`. (Tidak ada `raydium.farm.getFarmById`.) Data yang sama tersedia melalui HTTP di `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
