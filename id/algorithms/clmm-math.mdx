...
3. **Matematika tick seragam.** Karena tick didefinisikan sebagai `1.0001^i`, `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` juga merupakan tangga geometrik yang eksak. (`√1.0001 = 1.00004999875…`, sehingga memperlakukannya sebagai tepat `1.00005` menimbulkan penyimpangan hingga ~0.6% pada `MAX_TICK`.) Setiap penyeberangan tick diterjemahkan ke perkalian kecil dalam ruang `sqrt_price_x64`.
...
`tick_i` adalah `i32`. Rentang aktif adalah `[MIN_TICK, MAX_TICK] = [−443636, 443636]`, memberikan rentang harga sekitar `[2^−64, 2^64]` — sekitar `5.4e-20` hingga `1.8e19`. `tick_spacing` setiap pool diatur oleh tier biayanya: spasi lebih kecil untuk pasangan ketat (misalnya tier stablecoin 0,01% menggunakan spasi 1), spasi lebih besar untuk pasangan volatile (tier 0,25% menggunakan 60, tier 1% menggunakan 120).
...
- **Oracle.** Ring buffer `ObservationState` Raydium hanya menyimpan `(block_timestamp: u32, tick_cumulative: i64)` per slot ditambah `[u64; 4]` padding cadangan — **tidak ada** field `seconds_per_liquidity_cumulative`, sehingga derivasi berbobot likuiditas milik Uniswap tidak tersedia on-chain.
