...
- Volume 24h: \$120M
- Fee tier: 0.05% (LP mendapat bagian **84%** dari fee — setiap `AmmConfig` CLMM di mainnet memiliki `protocol_fee_rate = 12%` ditambah `fund_fee_rate = 4%`)
- Total TVL: \$40M
...
Raydium UI menampilkan **102% APR** untuk pool. (Contoh ini menggunakan pool yang dalam dan bervolume tinggi untuk membuat angka tetap konkret; pool CLMM tipikal menampilkan APR di mana saja dari 10% hingga 50%, dengan pool bervolume rendah jauh di bawah 10%.)
...
SDK Raydium mengekspos `PoolUtils.estimateAprsForPriceRangeMultiplier` dan `PoolUtils.estimateAprsForPriceRangeDelta` untuk estimasi ini.
...
const apr = PoolUtils.estimateAprsForPriceRangeMultiplier({
  ...
  positionTickLowerIndex: -100,   // range yang Anda maksudkan
...
Helper ini mengembalikan `{ feeApr, rewardsApr[], apr }` — `rewardsApr` adalah per aliran reward dan `apr` adalah totalnya. Tidak ada method `raydium.clmm.estimatedApr`. `estimateAprsForPriceRangeDelta` adalah perhitungan yang sama, hanya diparameterkan dengan delta ± di sekitar harga spot alih-alih tick eksplisit.
...
- **"APR dicompound secara otomatis."** Tidak. Fee harus diklaim — melalui `DecreaseLiquidity` dengan likuiditas nol, karena CLMM tidak memiliki instruksi collect mandiri — kemudian redeploy secara manual. Raydium tidak auto-compound fee CLMM.
...
- Implementasi `PoolUtils.estimateAprsForPriceRangeMultiplier` / `estimateAprsForPriceRangeDelta` pada Raydium SDK v2.
