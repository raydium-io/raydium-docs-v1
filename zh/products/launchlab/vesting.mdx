...
| 6 | `user_base_token` | W |   | 受益人在基础铸币上的 ATA；接收解锁的代币。`init_if_needed`。 |
| 7 | `base_token_mint` |   |   | 池的基础铸币。 |
| 8 | `base_token_program` |   |   | 拥有基础铸币的程序 —— SPL Token 或 Token-2022。 |
| 9 | `system_program` |   |   | 需要创建受益人 ATA 时为其出资。 |
| 10 | `associated_token_program` |   |   | 创建受益人 ATA。 |

<Warning>
  `system_program` 排在 `associated_token_program` **之前**。两者的类型都是 `Program<'info, _>`
  且地址固定，因此把这一对调换会导致地址约束失败，受益人也就无法领取。
</Warning>

**前置条件**

- `block_time >= pool_state.vesting_schedule.start_time`（否则 `VestingNotStarted`）。
- `pool_state.status == PoolStatus::Trade`（`2`）—— 毕业必须已经发生。在那之前调用会以 `PoolNotMigrated` 回滚。
...
在 `2027-09-28`（`start_time` 之后 90 天），受益人 A 调用 `ClaimVestedToken`：
...
六个月后（`2028-03-26`，`start_time` 之后 270 天），A 再次领取：
