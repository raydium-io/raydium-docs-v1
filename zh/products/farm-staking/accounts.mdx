...
SDK 通过 `raydium.api.fetchFarmInfoById({ ids })` 返回展示层视图，通过 `raydium.api.fetchFarmKeysById({ ids })` 返回原始账户密钥 —— 权限、LP 金库、每个槽位的奖励金库。（不存在 `raydium.farm.getFarmById`。）同样的数据也可以通过 HTTP 在 `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>` 获得。
