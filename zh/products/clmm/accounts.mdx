...
| Position NFT mint | 供应量为 1 的 mint，与 `PersonalPositionState` 关联。转移 NFT 即转移头寸。 | 每个头寸 1 个 |
...
| `LimitOrderState` | 每个开放的限价单一个。记录所有者、tick、方向、总金额、已结算输出快照。 | 每个订单 1 个 |
| `LimitOrderNonce` | 每个 `(wallet, nonce_index)` 的计数器，派生唯一的订单 PDA。 | 每个 (wallet, index) 1 个 |
| `Permission` | 授予其权限调用 `CreatePermissionedPool` 的权利。PDA 的**存在**就是授权。由管理员管理。 | 每个授权创建者 1 个 |
...
- **`fee_growth_global_{0,1}_x64`** 是整个池历史中每单位流动性赚取的累积费用。头寸读取这个值来计算它们应得的费用。
- **`tick_spacing`** 在初始化时锁定到 `AmmConfig`，永不改变。它决定了哪些 tick 索引可以作为头寸端点。
- **`tick_array_bitmap`** 是一个**内联**位图，覆盖现货价格周围常用的 tick 范围。对于头寸范围很广的池，溢出跟踪存在于单独的 `TickArrayBitmapExtension` 中。
...
- **`dynamic_fee_info`** 携带动态费用浮动的波动性状态。启用时，每次交换在 `AmmConfig.trade_fee_rate` 之上重新计算 `dynamic_fee_component`。布局在下面的 `DynamicFeeInfo` 中记录；没有动态费用的池将整个结构保留为零。
...
CLMM 不为每个 tick 存储单个记录。那样会有数十亿个账户。相反，它将**`TICK_ARRAY_SIZE` 个相邻的已初始化或未初始化 tick**（通常为 60 或 88，取决于程序版本）分组到一个 `TickArrayState` 中，该状态在首次使用时被懒惰地创建。
...
- `order_phase` 是队列 id。每次队列从"全部未填充"转换到"部分填充"时递增。
...
- tick 数组被懒惰地初始化：第一个接触未初始化数组的头寸或交换创建它，支付租金。
...
一个供应量为 1 的 SPL Token mint。mint 的地址是一个确定性 PDA；所有者钱包中的头寸 NFT 只是持有该单个代币的 ATA。转移 NFT 是头寸如何易手的方式——程序将授权密钥设置为 **NFT ATA 余额的当前持有者**，而不是存储在状态中的 Pubkey。
...
<Note>
  较早的 CLMM 版本在 `ProtocolPositionState` PDA 中存储每个 `(pool, tick_lower, tick_upper)` 的聚合簿记。**较新的版本不再创建或读取此账户。** 该槽仍然出现在 `OpenPosition` / `IncreaseLiquidity` / `DecreaseLiquidity` 账户列表中作为 `UncheckedAccount` 以保持 ABI 兼容性，但程序不会写入它。链上现有账户是残留的；管理员可以调用 `CloseProtocolPosition` 来回收它们的租金。

  聚合范围簿记现在直接从两个端点 tick（`liquidity_gross`、`liquidity_net` 和每个 tick 的 `fee_growth_outside_*` / `reward_growths_outside_x64`）在 `TickArrayState` 中派生。费用增长内部公式 `fee_growth_inside = global − outside_lower − outside_upper` 继续在没有聚合头寸账户的情况下工作。
</Note>
...
动态费用参数存在于两个地方。可重用的模板——`DynamicFeeConfig`——由管理员管理，在选择加入的池之间共享。每个池的运行时状态——`DynamicFeeInfo`——嵌入在 `PoolState` 中，由每次交换更新。
...
PDA 种子：`["dynamic_fee_config", index.to_be_bytes()]`。通过 `create_dynamic_fee_config`（管理员门控）创建，通过 `update_dynamic_fee_config` 修改。使用 `enable_dynamic_fee = true` 创建的池在创建时将配置的五个校准参数（`filter_period`、`decay_period`、`reduction_factor`、`dynamic_fee_control`、`max_volatility_accumulator`）快照到自己的 `DynamicFeeInfo` 中；对 `DynamicFeeConfig` 的后续编辑不会追溯影响现有池。
...
1. **打开** — 用户调用 `open_limit_order`，存入 `total_amount` 的输入代币，订单绑定到 `TickState` 队列。
2. **（可选）增加/减少** — `increase_limit_order` 添加到 `total_amount`；`decrease_limit_order` 返回未填充的代币（以及到该点为止的任何已结算输出）。
3. **结算** — 当队列被完全或部分填充时，所有者*或*操作保管人调用 `settle_limit_order` 将输出代币推送到所有者的 ATA。
4. **关闭** — 一旦 `unfilled_amount == 0`，账户就可以关闭。租金总是返回给 `owner`。

PDA 种子：`[owner.as_ref(), limit_order_nonce.key().as_ref(), limit_order_nonce.order_nonce.to_be_bytes().as_ref()]`。因此订单 PDA 对每个 `(owner, nonce_index, order_nonce)` 是唯一的。

## `LimitOrderNonce`

每个 `(wallet, nonce_index)` 的计数器，让单个用户运行多个并行的限价单管道而不会在 PDA 上碰撞。
...
一个能力账户，其**存在就是授权**：如果为给定的权限派生了 `Permission` PDA，该权限可以调用 `CreatePermissionedPool`。它除了创建它的权限外不存储任何内容。
...
PDA 种子：`["permission", authority.as_ref()]`。由管理员通过 `CreatePermissionPda` 创建，通过 `ClosePermissionPda` 拆除（租金退款给调用者）。两个管理员指令都接受程序 `admin` 或专用 `permission_pda_admin` 密钥。关闭 PDA 撤销授权——权限不能再创建额外的池，但它已经创建的池不受影响。
...
| `OpenPosition[WithToken22Nft]` | NFT mint + ATA、`personalPosition`、可能的新 `tickArrayState`(s)、`tickArrayBitmapExtension`（如果尚不存在） | — |
...
`TickArrayState` 账户**永远不会被程序关闭**——它们在池的整个生命周期内持续存在。一旦 tick 数组被初始化，即使其中的每个 tick 都返回到 `liquidity_gross == 0`，它也会保留在链上。重用现有 tick 数组是免费的；只有第一个接触从未初始化数组的头寸或交换支付其租金。
