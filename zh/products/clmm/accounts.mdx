...
`PoolState.tick_array_bitmap`（内联）覆盖"接近现货"范围 —— **±512** 个 tick 数组（`TICK_ARRAY_BITMAP_SIZE = 512`；`[u64; 16]` 字段共 1,024 位，在负和正的起始索引之间平均分配，因此数组索引为 −512 … +511）。超出该范围（对于极端 tick 值），程序维护一个扩展账户：
