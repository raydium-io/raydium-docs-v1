...
1. **衰减参考值**。若 `now - last_update_timestamp >= filter_period`，波动率参考值衰减。注意两个边界都是 `>=`，而不是 `>`：
...
布局字段是 `tokenFeesOwedA` / `tokenFeesOwedB`（不是 `...Owed0` / `...Owed1`），而 `rewardInfos` 的每一项是 `{ growthInsideLastX64, rewardAmountOwed }` —— 奖励 mint 本身来自池子的 `rewardInfos`，而不是头寸的。

`tokenFeesOwed*` 和 `rewardAmountOwed` 是头寸最后一次被触碰时的快照。若要查看**当前**值（包含此后的增长），可以模拟一次零流动性的 `DecreaseLiquidity`，或直接使用全局 `fee_growth_*` 及两个刻度外部快照重新计算。
