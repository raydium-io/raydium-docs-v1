...
| 6006 | `ZeroTradingTokens` | Given pool token amount results in zero trading tokens | 存入/提取数学将一侧四舍五入为零（头寸太小）。 | "金额低于此池的最小值。" |
...
LaunchLab 版本保持代码 `6000`–`6021` 不变。它移除了前平台全局访问错误，并将 `6022` 分配给新的允许账户验证错误：
...
旧的 `PlatformGlobalAccessDenied`（`6022`）和 `InvalidPlatformGlobalAccess`（`6023`）变体被移除。不要使用之前的 IDL 解码 `6022`。
