---
title: "2026-08-17 — CLMM：受限发行人头寸 NFT 冻结"
...
本次发布为精选的 KYC 受限发行人池添加了条件性不可转账路径。它不会冻结每个新头寸、冻结池资产或阻止头寸所有者管理流动性。CLMM 池 PDA 成为每个新头寸 NFT 铸币权的冻结权限，但 NFT 代币账户默认保持解冻状态。程序仅在头寸使用 `OpenPositionV2` 或 `OpenPositionWithToken22Nft` 且至少一个底层金库铸币权的冻结权限来自硬编码的受限发行人列表时，才会冻结它。
...
- **冻结意味着绑定到所有者。** SPL Token 会以其原生 `AccountFrozen` 错误拒绝 NFT 转账和代币账户所有者变更。当 NFT 所有者签名时，增加/减少流动性和费用/奖励收集继续工作。
- **关闭仍然可用。** `ClosePosition` 检测冻结的 NFT 账户，使用池 PDA 解冻它，然后原子性地销毁 NFT 并关闭头寸。
- **冻结关闭需要一个剩余账户。** 客户端必须将 `personal_position.pool_id` 作为第一个剩余账户追加。缺少它会返回 CLMM `AccountLack`；传递错误的池会返回 `NotApproved`。
...
缺少冻结权限不匹配。一侧匹配即可。列表为 `frozen_position_nft_authorities::IDS`，包含单个条目。当前 mainnet-beta 和 devnet 匹配键位于 [`reference/program-addresses`](/zh/reference/program-addresses#clmm-restricted-issuer-freeze-authorities)。
...
使用池 PDA 而非全局管理员或发行人密钥将权限范围限制在一个池。外部方无法作为 PDA 签名，CLMM 不公开任何通用指令来冻结或解冻任意头寸账户。新代码仅在两个地方使用权限：

1. 当底层发行人检查匹配时，铸造后立即冻结。
2. 在销毁前的 `ClosePosition` 期间立即解冻。
...
- `products/clmm/ticks-and-positions` — 权限模型、触发、所有者能力和关闭迁移警告。
...
- `reference/token-2022-support` — 池允许列表和头寸托管之间的区别。
- `reference/error-codes` — 扩展的 `AccountLack` 和 `NotApproved` 原因。
- `reference/program-addresses` — 受限发行人权限匹配键。
