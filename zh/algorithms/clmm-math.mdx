...
3. **跳价格数学统一。** 由于跳价格定义为 `1.0001^i`，所以 `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` 也是一个精确的几何梯级。（`√1.0001 = 1.00004999875…`，因此把它当作恰好 `1.00005` 处理在 `MAX_TICK` 处最多会有约 0.6% 的偏差。）每次跳价格穿越在 `sqrt_price_x64` 空间中转化为一个小的乘法。
...
`tick_i` 是一个 `i32`。活跃范围是 `[MIN_TICK, MAX_TICK] = [−443636, 443636]`，对应的价格范围约为 `[2^−64, 2^64]` — 约 `5.4e-20` 到 `1.8e19`。每个池的 `tick_spacing` 由其费用等级设定：紧密交易对使用较小间距（例如稳定币 0.01% 等级使用间距 1），波动性交易对使用较大间距（0.25% 等级使用 60，1% 等级使用 120）。
...
- **预言机。** Raydium 的 `ObservationState` 环形缓冲每个槽位只存储 `(block_timestamp: u32, tick_cumulative: i64)` 以及 `[u64; 4]` 的保留填充字段 — **没有** `seconds_per_liquidity_cumulative` 字段，因此 Uniswap 那套以流动性加权的推导在链上无法实现。
