...
有效价格：`1000 / 1993.01 ≈ 0.5018`。之前现货：`0.5`。相对现货的滑点：**约 0.35%**。价格影响 — 按上面的定义，即池子现货价格的移动幅度 — 为**约 0.20%**。
...
有效价格：`100_000 / 181_405 ≈ 0.5513`。相对现货的滑点：**约 10.3%**。价格影响：**约 17.3%** — 接近 `2 · 10% = 20%` 的经验法则，交易费用与曲线反演使其略低于该值。这两个数字常被混为一谈：滑点是交换者相对现货实际支付的部分，价格影响则是池子自身报价移动的幅度。
