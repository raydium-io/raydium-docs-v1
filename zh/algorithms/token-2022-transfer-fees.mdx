...
**支持矩阵：** **CPMM** 完整支持 Token-2022，包括带转账费的 mint。**CLMM** 通过显式的 `SwapV2` 账户支持带转账费的 Token-2022。**AMM v4** 完全不支持 Token-2022。**LaunchLab** 通过 `InitializeV2` 创建经典的 SPL 基础 mint，但另有一个专门的 `InitializeWithToken2022` 入口，可创建 Token-2022 基础 mint 并可选地附加 `TransferFeeConfig`；其报价一侧接受任一代币程序。见 [`products/launchlab/platform-config`](/zh/products/launchlab/platform-config#token-2022-transfer-fee-authorities)。**Farm v6** 在质押 mint 和奖励 mint 上都支持 Token-2022。
...
≈ 986.03 USDC。没有出站转账费，所以用户收到约 986.03 USDC。
...
- [SPL Token-2022 转账费用扩展文档](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
