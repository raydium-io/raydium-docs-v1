...
Raydium 不关心 authority 是谁——它只关心 `authority` 签名覆盖交易，以及输入 ATA 由该 authority 拥有。验证发生在 `anchor_spl::token::transfer` 中：ATA 的 `authority` 字段必须等于签名者。

常见错误：将 `user` 作为 authority 传递（并从由托管 PDA 拥有的 `escrow_input_ata` 转账）。SPL Token 程序以 `owner mismatch` 拒绝。始终使 `authority` 字段与 ATA 所有者匹配。
...
CPI 本身的调用框架成本约 1,500 CU；被调用方自己的 CU 使用堆叠在顶部。每个 Raydium CPI 的粗略预算：
...
为每个 CPI 框架添加约 1,500，为你自己程序的开销添加约 20,000。执行 `harvest → swap A → swap B → deposit LP → stake LP` 的自动复利器轻松达到 700k CU。
...
保管人支付交易费（他们在其他地方获得保管人费——未显示）。托管 PDA 签署 CPI。Raydium 端滑点检查*和*托管自己的增量检查都强制执行下限——双重保险。
...
如果你的程序将账户作为 `mut` 传递，但 Raydium 期望它为只读（或反之），运行时以 `InvalidAccountData` 拒绝调用。始终在 IDL 中检查 Raydium 指令的预期可变性；`anchor_cp_swap::cpi::accounts::Swap` 通过其字段类型强制执行。
