...
<Info>
  Матрица поддержки: **CPMM** полностью поддерживает Token-2022, включая минты с transfer-fee. **CLMM** поддерживает Token-2022 с transfer-fee через явные аккаунты `SwapV2`. **AMM v4** вообще не поддерживает Token-2022. **LaunchLab** через `InitializeV2` создаёт классический SPL-минт для базового токена, но имеет отдельную точку входа `InitializeWithToken2022`, которая создаёт базовый минт Token-2022 и при необходимости прикрепляет `TransferFeeConfig`; на стороне quote принимается любая из двух token-программ. См. [`products/launchlab/platform-config`](/ru/products/launchlab/platform-config#token-2022-transfer-fee-authorities). **Farm v6** поддерживает Token-2022 на обоих минтах: staking и reward.
</Info>
...
≈ 986.03 USDC. Нет исходящей transfer-fee, поэтому пользователь получает ≈ 986.03 USDC.
...
- [Документация расширения transfer-fee SPL Token-2022](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
