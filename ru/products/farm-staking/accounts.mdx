...
| `UserStake` (v3/v5) / `UserLedger` (v6) | Farm программа | Реестр на `(farm, user)`: сумма в ставке + снимок вознаграждения за акцию на каждый поток. |

SDK возвращает отображаемое представление `FarmState` через `raydium.api.fetchFarmInfoById({ ids })`, а сырые ключи аккаунтов — authority, LP-хранилище, хранилища вознаграждений по слотам — через `raydium.api.fetchFarmKeysById({ ids })`. (Метода `raydium.farm.getFarmById` не существует.) Те же данные доступны по HTTP: `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
