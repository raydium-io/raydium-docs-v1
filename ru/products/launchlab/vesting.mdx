...
| 6 | `user_base_token` | W |   | ATA бенефициара на базовом минте; получает разблокированные токены. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Базовый минт пула. |
| 8 | `base_token_program` |   |   | Программа, которой принадлежит базовый минт — SPL Token или Token-2022. |
| 9 | `system_program` |   |   | Финансирует ATA бенефициара, когда его нужно создать. |
| 10 | `associated_token_program` |   |   | Создаёт ATA бенефициара. |

<Warning>
  `system_program` идёт **перед** `associated_token_program`. Оба типизированы как
  `Program<'info, _>` с фиксированными адресами, поэтому перестановка этой пары не проходит
  проверку адреса, и бенефициар не может получить выплату.
</Warning>

**Предусловия**

- `block_time >= pool_state.vesting_schedule.start_time` (иначе `VestingNotStarted`).
- `pool_state.status == PoolStatus::Trade` (`2`) — выпуск должен был уже произойти. Вызов до этого откатывается с `PoolNotMigrated`.
- Дельта разблокированной суммы больше нуля. Холостой вызов (рассчитанная дельта 0) откатывается.
...
На `2027-09-28` (90 дней после `start_time`), получатель A вызывает `ClaimVestedToken`:
...
Шесть месяцев спустя (`2028-03-26`, 270 дней после `start_time`), A получает снова:
