...
<Info>
  На этой странице описаны **структура и назначение** каждого аккаунта. Seeds указаны в [`reference/program-addresses`](/ru/reference/program-addresses). Пул CLMM требует больше аккаунтов, чем пул CPMM, потому что ликвидность хранится разреженно по диапазону тиков; понимание этой разреженности — основная часть этой страницы.
</Info>
...
| `TickArrayBitmapExtension` | Битовая карта переполнения, отслеживающая, какие массивы тиков существуют за пределами встроенной битовой карты в `PoolState`. | 0 или 1 |
...
| Position NFT mint | Mint с предложением 1, связанный с `PersonalPositionState`. Передача передает позицию. | 1 на позицию |
| `ObservationState` | Кольцевой буфер наблюдений цены для TWAP. | 1 |
| `token_0_vault`, `token_1_vault` | Аккаунты токенов, содержащие балансы пула. Принадлежат authority пула. | 2 |
...
| `LimitOrderState` | По одному на открытый лимитный ордер. Записывает владельца, тик, сторону, общую сумму, снимок выполненного выхода. | 1 на ордер |
| `LimitOrderNonce` | Счетчик на `(wallet, nonce_index)`, который выводит уникальные PDA ордеров. | 1 на (wallet, index) |
| `Permission` | Дает своему authority право вызывать `CreatePermissionedPool`. Существование PDA **является** грантом. Управляется администратором. | 1 на авторизованного создателя |
...
Поля, которые вы будете использовать:

- **`sqrt_price_x64`** и **`tick_current`** — состояние цены пула. Обновляются вместе при каждом свопе. `tick_current` — это пол `log_{1.0001}(price)`.
- **`liquidity`** — это **активная** ликвидность — сумма значений `L` для всех позиций, диапазон которых содержит `tick_current`. Изменяется каждый раз, когда своп пересекает тик, и каждый раз, когда позиция открывается/закрывается/изменяется.
...
- **`tick_spacing`** привязан к `AmmConfig` при инициализации и никогда не меняется. Он определяет, какие индексы тиков могут быть конечными точками позиции.
- **`tick_array_bitmap`** — это **встроенная** битовая карта, охватывающая диапазон, близкий к спотовой цене — ±1024 массива тиков. Для пулов, позиции которых выходят далеко, отслеживание переполнения находится в отдельном аккаунте `TickArrayBitmapExtension`.
- **`fee_on`** фиксируется при создании пула. `0` (FromInput) воспроизводит классическое поведение Uniswap-V3. `1` и `2` направляют комиссию свопа на одну сторону книги — см. [`products/clmm/fees`](/ru/products/clmm/fees) для компромиссов.
- **`seed_index`** — это `[0, 0]` для каждого пула, созданного через `CreatePool` / `CreateCustomizablePool` (один канонический пул на пару). **Ненулевое** значение означает, что пул был создан через `CreatePermissionedPool`, и индекс является частью seeds PDA пула, позволяя нескольким пулам сосуществовать для одной и той же `(config, mint0, mint1)`. Чтобы повторно вывести адрес такого пула, вы должны знать его `seed_index`.
...
Типичный опубликованный набор уровней комиссий CLMM (проверьте против `GET https://api-v3.raydium.io/main/clmm-config`):
...
CLMM не хранит одну запись на тик. Это были бы миллиарды аккаунтов. Вместо этого он группирует **`TICK_ARRAY_SIZE` соседних инициализированных или нет тиков** (обычно 60 или 88 в зависимости от версии программы) в `TickArrayState`, который ленивым образом создается при первом использовании.
...
- `order_phase` — это id когорты. Он увеличивается каждый раз, когда когорта переходит из состояния «все не заполнено» в состояние «частично заполнено».
...
- `unfilled_ratio_x64` — это множитель Q64.64, переносимый на когорту: когда своп заполняет X% когорты, коэффициент умножается на `(1 − X)`. Каждый открытый ордер хранит свой собственный снимок `(order_phase, unfilled_ratio_x64)` во время открытия, поэтому математика расчета сводится к сравнению снимков.
...
- Массив тиков инициализируется ленивым образом: первая позиция или своп, который трогает неинициализированный массив, создает его, оплачивая ренту.
- Массив тиков **никогда не закрывается** программой. После выделения он сохраняется на протяжении всей жизни пула, даже после того, как каждый тик внутри него вернется к `liquidity_gross == 0`. Последующие позиции и свопы переиспользуют существующий аккаунт без дополнительной ренты. Нет пути очистки `ClosePosition` для массивов тиков.

### `TickArrayBitmapExtension`

`PoolState.tick_array_bitmap` (встроенная) охватывает диапазон «близко к спотовой цене» — ±1024 массива тиков. За пределами этого диапазона (для экстремальных значений тиков) программа поддерживает аккаунт расширения:
...
Если диапазон вашей позиции «нормальный», вы никогда не думаете об аккаунте расширения. Позиции полного диапазона (например, `(MIN_TICK, MAX_TICK)`) требуют его; SDK разрешает его для вас.
...
SPL Token mint с предложением 1. Адрес mint — это детерминированный PDA; позиция NFT в кошельке владельца — это просто ATA, содержащий этот единственный токен. Передача NFT — это то, как позиция переходит из рук в руки — программа привязывает авторизацию к **текущему держателю баланса ATA NFT**, а не к Pubkey, хранящемуся в состоянии.
...
Параметры динамической комиссии находятся в двух местах. Переиспользуемый шаблон — `DynamicFeeConfig` — управляется администратором и общий для пулов, которые подключаются. Состояние выполнения на пул — `DynamicFeeInfo` — встроено в `PoolState` и обновляется каждым свопом.
...
Нижние четыре поля — это состояние; верхние пять — это калибровка, скопированная из `DynamicFeeConfig`. Математика комиссии и правила затухания документированы под [`products/clmm/math`](/ru/products/clmm/math) и [`products/clmm/fees`](/ru/products/clmm/fees).
...
1. **Открыть** — пользователь вызывает `open_limit_order`, депонирует `total_amount` входного токена, ордер привязан к когорте `TickState`.
2. **(опционально) Увеличить / Уменьшить** — `increase_limit_order` добавляет к `total_amount`; `decrease_limit_order` возвращает незаполненные токены (и любой выполненный выход до этого момента).
3. **Расчет** — когда когорта полностью или частично заполнена, владелец *или* оперативный хранитель вызывает `settle_limit_order`, чтобы отправить выходные токены на ATA владельца.
4. **Закрыть** — как только `unfilled_amount == 0`, аккаунт закрываемый. Рента всегда возвращается `owner`.
...
Точные строки seed всегда должны проверяться по IDL в цепи и [`reference/program-addresses`](/ru/reference/program-addresses).
...
| `OpenPosition[WithToken22Nft]` | NFT mint + ATA, `personalPosition`, возможно новые `tickArrayState`(s), `tickArrayBitmapExtension`, если еще не существует | — |
...
| `CreateCustomizablePool` | `poolState`, `observation`, хранилища — то же, что `CreatePool`. Снимает `dynamicFeeConfig`, если `enable_dynamic_fee = true`. | — |
| `CreatePermissionedPool` | `poolState` (по адресу, выведенному из `seed_index`), `observation`, хранилища — как `CreateCustomizablePool`. Требует существующего `permission`. | — |
| `CreatePermissionPda` / `ClosePermissionPda` | `permission` (грант администратора) | `permission` (отзыв администратора, рента → вызывающий) |
| `CollectRewards` | — | — |
| `UpdateRewardInfos` | — | — |
| `CloseProtocolPosition` (admin) | — | Рудиментарный `protocolPositionState` (рента → администратор) |
...
- **Прохождение свопа и математика прироста комиссий**: [`products/clmm/math`](/ru/products/clmm/math).
...
- **Канонические ID программ и seeds**: [`reference/program-addresses`](/ru/reference/program-addresses).
