...
`PoolState.tick_array_bitmap` (встроенная) охватывает диапазон "близко к спот" — **±512** массивов тиков (`TICK_ARRAY_BITMAP_SIZE = 512`; поле `[u64; 16]` — это 1,024 бита, поделённые ровно между отрицательными и положительными начальными индексами, то есть индексы массивов от −512 до +511). За пределами этого диапазона (для экстремальных значений тиков) программа ведет аккаунт расширения:
