...
1. **Затухание эталона.** Если `now - last_update_timestamp >= filter_period`, эталон волатильности затухает. Обратите внимание, что обе границы — это `>=`, а не `>`:
...
Поля разметки называются `tokenFeesOwedA` / `tokenFeesOwedB` (а не `...Owed0` / `...Owed1`), и каждая запись `rewardInfos` — это `{ growthInsideLastX64, rewardAmountOwed }`; сам reward mint берётся из `rewardInfos` пула, а не позиции.

`tokenFeesOwed*` и `rewardAmountOwed` — это снимки с момента последнего обращения к позиции. Чтобы увидеть **актуальные** значения (учитывающие рост с тех пор), просимулируйте `DecreaseLiquidity` с нулевой ликвидностью или пересчитайте их из глобальных `fee_growth_*` и снимков двух граничных тиков.
