...
<Info>
  **Raydium Perps — это белый лейбл на базе Orderly Network.** Биржевой стакан, механизм маршрутизации заявок и состояние счета полностью размещены в Orderly. SDK Raydium v2 (`@raydium-io/raydium-sdk-v2`) **не** покрывает perps — для программного доступа используйте REST и WebSocket API Orderly напрямую. Приведённые ниже примеры показывают наиболее распространённые сценарии; канонической справкой является [введение в API Orderly](https://orderly.network/docs/build-on-omnichain/introduction), под которым документированы [REST-эндпоинты](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order) и [WebSocket-потоки](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction).
</Info>

<Info>
  **Информация о версии.**

  - Бэкенд: [Orderly Network REST + WebSocket API](https://orderly.network/docs/build-on-omnichain/introduction)
  ...
  API-поверхность Orderly эволюционирует; перед использованием этих примеров в боевом коде проверьте [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction).
</Info>
...
- [Документация разработчика Orderly Network](https://orderly.network/docs/build-on-omnichain/introduction) — каноническая справка для API-поверхности, используемой выше. Raydium Perps потребляет это напрямую.
- [Поддерживаемые сети Orderly](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — подтверждает статус Solana и адреса расчётных хранилищ для каждой сети.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — видимая сообществу JS-обёртка над тем же уровнем REST. У неё **нет опубликованных релизов** и нет заявленной политики поддержки, поэтому относитесь к ней как к справочному коду, а не как к зависимости; уровень подписи выше достаточно мал, чтобы держать его у себя.
