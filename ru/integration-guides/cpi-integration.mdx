...
Raydium не волнует, кто является authority — ему важно только, что подпись `authority` покрывает транзакцию и что входной ATA принадлежит этому authority. Проверка происходит в `anchor_spl::token::transfer`: поле `authority` ATA должно равняться подписанту.

Частая ошибка: передача `user` как authority (и передача из `escrow_input_ata`, которым владеет PDA эскроу). Программа SPL Token отклоняет с `owner mismatch`. Всегда делайте поле `authority` совпадающим с владельцем ATA.
...
CPI стоит ~1,500 CU для самого фрейма вызова; собственное использование CU вызываемой стороны складывается сверху. Примерный бюджет на CPI Raydium:

| Вызов | CU (SPL Token) | CU (Token-2022) |
|------|----------------|-----------------|
| CPMM swap_base_input | ~150,000 | ~200,000 |
| CLMM swap_v2 (один массив тиков) | ~180,000 | ~230,000 |
| CLMM swap_v2 (пересекает 2 тика) | ~220,000 | ~270,000 |
| Farm v6 deposit | ~120,000 | ~150,000 |
| Farm v6 harvest (на слот награды) | +30,000 | +40,000 |
| AMM v4 swap_base_in | ~140,000 | n/a |

Добавьте ~1,500 на каждый фрейм CPI и ~20,000 на накладные расходы вашей собственной программы. Автокомпаундер, выполняющий `harvest → swap A → swap B → deposit LP → stake LP`, легко достигает 700k CU.
...
Хранитель платит комиссию за транзакцию (они получают комиссию хранителя где-то ещё — не показано). PDA эскроу подписывает CPI. Как проверка проскальзывания на стороне Raydium, *так и* собственная проверка дельты эскроу обеспечивают минимум — подстраховка.
...
Если ваша программа передаёт аккаунт как `mut`, но Raydium ожидает его только для чтения (или наоборот), runtime отклоняет вызов с `InvalidAccountData`. Всегда проверяйте ожидаемую изменяемость инструкции Raydium в IDL; `anchor_cp_swap::cpi::accounts::Swap` обеспечивает это через типы полей.
