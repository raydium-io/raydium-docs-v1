...
On Solana the thing to revoke is an SPL-Token **delegate**: the `Approve` instruction names a delegate and an amount on one of your token accounts, and `Revoke` clears it. Open your wallet's token-approvals / connected-apps view and revoke anything you do not recognise; Phantom and Solflare both surface this. From a terminal it is one command per token account:

```bash
spl-token revoke <YOUR_TOKEN_ACCOUNT_ADDRESS>
```

This stops future drain transactions but does not recover already-drained funds.

<Warning>
  [revoke.cash](https://revoke.cash) is **EVM-only** — it covers Ethereum and 100+ EVM chains (including Neon, Solana's EVM layer) but not Solana itself. Don't send Solana users there.
</Warning>
