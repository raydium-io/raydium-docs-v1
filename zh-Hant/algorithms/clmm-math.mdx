...
3. **跳點數學均勻。** 由於跳點定義為 `1.0001^i`，`sqrt(price) = (√1.0001)^i ≈ 1.00005^i` 也是一個精確的幾何階梯。（`√1.0001 = 1.00004999875…`，因此把它當作恰好 `1.00005` 時，在 `MAX_TICK` 處會產生最多約 0.6% 的偏差。）每次跳點穿越轉換為 `sqrt_price_x64` 空間中的小乘法。
...
`tick_i` 是一個 `i32`。活躍範圍是 `[MIN_TICK, MAX_TICK] = [−443636, 443636]`，提供大約 `[2^−64, 2^64]` 的價格範圍——約 `5.4e-20` 到 `1.8e19`。每個流動性池的 `tick_spacing` 由其費用層級設定：緊密交易對使用較小的間距（例如穩定幣 0.01% 層級使用間距 1），波動性交易對使用較大的間距（0.25% 層級使用 60，1% 層級使用 120）。
...
- **預言機。** Raydium 的 `ObservationState` 環形緩衝區每個槽位僅存儲 `(block_timestamp: u32, tick_cumulative: i64)` 加上 `[u64; 4]` 的保留填充——**沒有** `seconds_per_liquidity_cumulative` 欄位，因此 Uniswap 的流動性加權推導在鏈上無法使用。
...
- **跳點位圖。** Raydium 將初始化跳點位圖打包到每個池的 `[u64; 16]` 中以快速 `find_next_initialized_tick`；Uniswap 使用鏈上的每字映射。權衡是租金 vs 查找成本。
