...
pnl_share = swap_fee · pnl_numerator / pnl_denominator               // 12 / 100 = swap_fee 的 12%
...
有效價格：`1000 / 1993.01 ≈ 0.5018`。之前現貨：`0.5`。相對現貨的滑點：**約 0.35%**。價格影響——依照上面的定義，即流動性池現貨價格的變動——為**約 0.20%**。
...
有效：`100_000 / 181_405 ≈ 0.5513`。相對現貨的滑點：**約 10.3%**。價格影響：**約 17.3%**——接近 `2 · 10% = 20%` 的經驗法則，交易手續費與曲線反演使其略低於該值。這兩個數字經常被混為一談：滑點是交換者相對於現貨所付出的代價，而價格影響是流動性池自身報價移動的幅度。
