...
SDK 在 `raydium.api.fetchFarmInfoById({ ids })` 上返回顯示層級的視圖，並在 `raydium.api.fetchFarmKeysById({ ids })` 上返回原始帳户金鑰 — 授權、LP 金庫、每個插槽的獎勵金庫。（並不存在 `raydium.farm.getFarmById`。）相同的資料也可以透過 HTTP 從 `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>` 取得。
