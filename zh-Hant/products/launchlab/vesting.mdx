...
| 6 | `user_base_token` | W |   | 受益人在基礎鑄幣上的 ATA；接收已解鎖的代幣。`init_if_needed`。 |
| 7 | `base_token_mint` |   |   | 池的基礎鑄幣。 |
| 8 | `base_token_program` |   |   | 擁有基礎鑄幣的程式 — SPL Token 或 Token-2022。 |
| 9 | `system_program` |   |   | 在必須建立受益人 ATA 時提供資金。 |
| 10 | `associated_token_program` |   |   | 建立受益人 ATA。 |

<Warning>
  `system_program` 排在 `associated_token_program` **之前**。兩者的型別都是 `Program<'info, _>`
  且位址固定，所以調換這一對會使位址約束失敗，受益人就無法領取。
</Warning>

**前置條件**

- `block_time >= pool_state.vesting_schedule.start_time`（否則為 `VestingNotStarted`）。
- `pool_state.status == PoolStatus::Trade`（`2`） — 畢業必須已經發生。在那之前呼叫會以 `PoolNotMigrated` 還原。
...
在 `2027-09-28`（`start_time` 後 90 天），受益人 A 呼叫 `ClaimVestedToken`：
...
六個月後（`2028-03-26`，`start_time` 後 270 天），A 再次領取：
