...
1. **衰減參考值**。若 `now - last_update_timestamp >= filter_period`，波動率參考值會衰減。請注意兩個邊界都是 `>=`，而不是 `>`：
...
版面欄位是 `tokenFeesOwedA` / `tokenFeesOwedB`（不是 `...Owed0` / `...Owed1`），而 `rewardInfos` 的每一項都是 `{ growthInsideLastX64, rewardAmountOwed }` — 獎勵 mint 本身來自池子的 `rewardInfos`，而不是倉位的。

`tokenFeesOwed*` 與 `rewardAmountOwed` 是倉位最後一次被觸碰時的快照。若要查看**當前**數值（反映此後的成長），可模擬一次零流動性的 `DecreaseLiquidity`，或直接利用全域 `fee_growth_*` 及兩個 tick 外部快照重新計算。
