---
title: "CLMM 帳戶"
description: "CLMM 池涉及的每個帳戶：PoolState、AmmConfig、TickArrayState、TickArrayBitmapExtension、PersonalPositionState 和 Observation。"
---
...
<Info>
  本頁說明每個帳戶的**配置和角色**。種子（Seeds）是規範的，列在 [`reference/program-addresses`](/zh-Hant/reference/program-addresses)。CLMM 池比 CPMM 池需要更多帳戶，因為流動性稀疏地分佈在整個 tick 範圍內；理解這種稀疏性是本頁的重點。
</Info>
...
| 帳戶 | 用途 | 每個池的數量 |
|---------|---------|----------------|
| `AmmConfig` | 費用等級：交易費率、協議份額、基金份額、預設 tick 間距。在此等級的所有池之間共享。 | 1（共享） |
| `PoolState` | 目前的 `sqrt_price_x64`、目前 tick、總流動性、費用增長全局、獎勵資訊、觀察指標。 | 1 |
...
| `PersonalPositionState` | 每個 LP 位置一個。儲存範圍、流動性和最後看到的費用/獎勵增長。授權 = NFT 擁有者。 | 每個位置 1 個 |
| 位置 NFT mint | 供應量為 1 的 mint，與 `PersonalPositionState` 相關聯。其代幣帳戶預設未凍結；V2 開啟時，僅當基礎 mint 符合受限發行者篩選時才凍結。 | 每個位置 1 個 |
| `ObservationState` | TWAP 價格觀察的環形緩衝區。 | 1 |
| `token_0_vault`、`token_1_vault` | 持有池餘額的代幣帳戶。由池授權擁有。 | 2 |
| `DynamicFeeConfig` | 動態費用機制的可重用參數集。透過 `create_customizable_pool` 建立的池可以選擇加入。由管理員管理。 | 共享（每個索引） |
| `LimitOrderState` | 每個開放限價單一個。記錄擁有者、tick、方向、總金額、已結算輸出快照。 | 每個訂單 1 個 |
| `LimitOrderNonce` | 每個 `(wallet, nonce_index)` 計數器，衍生唯一的訂單 PDA。 | 每個 (wallet, index) 1 個 |
| `Permission` | 授予其授權呼叫 `CreatePermissionedPool` 的權利。PDA 的**存在就是授權**。由管理員管理。 | 每個授權建立者 1 個 |
...
- **`sqrt_price_x64`** 和 **`tick_current`** 是池的價格狀態。它們在每次交換時一起更新。`tick_current` 是 `log_{1.0001}(price)` 的下限。
- **`liquidity`** 是**活躍**流動性——所有位置的 `L` 值之和，其範圍包含 `tick_current`。每次交換跨越 tick 時以及每次位置開啟/關閉/調整大小時都會變更。
...
- **`tick_array_bitmap`** 是一個**內聯**位圖，涵蓋現貨價格周圍的「接近」範圍——±1,024 個 tick 陣列。對於位置範圍很遠的池，溢出追蹤位於單獨的 `TickArrayBitmapExtension`。
- **`fee_on`** 在池建立時固定。`0`（FromInput）重現經典 Uniswap-V3 行為。`1` 和 `2` 將交換費路由到書籍的單一側——參見 [`products/clmm/fees`](/zh-Hant/products/clmm/fees) 以了解權衡。
- **`seed_index`** 對於透過 `CreatePool` / `CreateCustomizablePool` 建立的每個池都是 `[0, 0]`（每對一個規範池）。**非零**值表示池是透過 `CreatePermissionedPool` 建立的，索引是池 PDA 種子的一部分，允許多個池為相同的 `(config, mint0, mint1)` 共存。要重新衍生這樣的池的地址，你必須知道其 `seed_index`。
...
一組典型發佈的 CLMM 費用等級（確認對比 `GET https://api-v3.raydium.io/main/clmm-config`）：
...
`protocol_fee_rate` 和 `fund_fee_rate` 是交易費的分數；與 CPMM 相同的慣例。參見 [`products/clmm/fees`](/zh-Hant/products/clmm/fees)。
...
- tick 陣列**永遠不會被程式關閉**。一旦分配，它就會在池的整個生命週期內持續存在，即使其中的每個 tick 都回到 `liquidity_gross == 0`。後續位置和交換無需額外租金即可重用現有帳戶。沒有 `ClosePosition` 驅動的 tick 陣列清理路徑。

### `TickArrayBitmapExtension`

`PoolState.tick_array_bitmap`（內聯）涵蓋「接近現貨」範圍——**±512** 個 tick 陣列（`TICK_ARRAY_BITMAP_SIZE = 512`；`[u64; 16]` 欄位共有 1,024 位，在負與正的起始索引之間平均分配，因此陣列索引為 −512 … +511）。超出該範圍（對於極端 tick 值），程式維護一個擴展帳戶：
...
CLMM 位置是三個帳戶加一個 mint 的**組合**：

### 位置 NFT mint

一個供應量為 1 的 SPL Token 或 Token-2022 mint。擁有者錢包中的位置 NFT 是持有該單一代幣的 ATA。程式將授權鑰匙設定為 **NFT ATA 餘額的目前持有者**，而不是儲存在狀態中的 Pubkey。

新位置 NFT mint 在鑄造單一代幣並移除 mint 授權之前將 `pool_state` 設定為其凍結授權。**設定凍結授權本身不會凍結 NFT 帳戶。** 帳戶保持未凍結和可轉移，除非兩個條件都成立：呼叫者使用 `OpenPositionV2` 或 `OpenPositionWithToken22Nft`，且至少一個基礎金庫 mint 的凍結授權出現在 CLMM 的受限發行者清單上。只有這樣，CLMM 才會在鑄造後凍結 NFT 帳戶。這不會改變任何 `PersonalPositionState` 或 `PoolState` 位元組。
...
<Note>
  較舊的 CLMM 版本在 `ProtocolPositionState` PDA 中儲存每個 `(pool, tick_lower, tick_upper)` 的聚合簿記。**較新的版本不再建立或讀取此帳戶。** 該位置仍然出現在 `OpenPosition` / `IncreaseLiquidity` / `DecreaseLiquidity` 帳戶清單上，作為 `UncheckedAccount` 以保持 ABI 相容性，但程式不會寫入它。鏈上現有帳戶是遺留的；管理員可以呼叫 `CloseProtocolPosition` 以回收其租金。

  聚合範圍簿記現在直接從兩個端點 tick（`liquidity_gross`、`liquidity_net` 和每個 tick 的 `fee_growth_outside_*` / `reward_growths_outside_x64`）在 `TickArrayState` 中衍生。費用增長內部公式 `fee_growth_inside = global − outside_lower − outside_upper` 無需聚合位置帳戶即可繼續工作。
</Note>
...
CLMM 的觀察緩衝區儲存**累積 tick**，而不是累積價格。外部消費者從 `(tick_cumulative[t1] − tick_cumulative[t0]) / (t1 − t0)` 計算區間上的幾何平均價格，然後 `price = 1.0001 ** tick`。參見 [`algorithms/clmm-math`](/zh-Hant/algorithms/clmm-math)。
...
底部四個欄位是狀態；頂部五個是從 `DynamicFeeConfig` 複製的校準。費用數學和衰減規則記錄在 [`products/clmm/math`](/zh-Hant/products/clmm/math) 和 [`products/clmm/fees`](/zh-Hant/products/clmm/fees) 下。

公式使用的常數：

| 常數 | 值 | 含義 |
|---|---|---|
| `VOLATILITY_ACCUMULATOR_SCALE` | `10_000` | 波動性累加器的粒度 |
...
一個能力帳戶，其**存在就是授權**：如果為給定的授權衍生 `Permission` PDA，該授權可以呼叫 `CreatePermissionedPool`。它除了建立它的授權外不儲存任何內容。
...
確切的種子字串應始終對照鏈上 IDL 和 [`reference/program-addresses`](/zh-Hant/reference/program-addresses) 進行雙重檢查。
...
| `OpenPosition[V2]` / `OpenPositionWithToken22Nft` | NFT mint + ATA、`personalPosition`、可能的新 `tickArrayState`(s)、`tickArrayBitmapExtension`（如果尚不存在）；對於受限發行者池可能凍結 NFT ATA | — |
| `IncreaseLiquidity` | 可能的新 `tickArrayState`(s) | — |
| `DecreaseLiquidity` | — | 可能清除 tick 條目（但 `tickArrayState` 本身不會關閉） |
| `ClosePosition` | — | NFT ATA、`personalPosition` 和 Token-2022 NFT mint；經典 SPL NFT mint 保持供應量為零。凍結路徑首先透過傳入剩餘帳戶的池解凍。 |
| `SwapV2` | 可能的新 `tickArrayState` | — |
| `OpenLimitOrder` | `limitOrderState`、可能的 `limitOrderNonce`（初始化如需）、可能的新 `tickArrayState` | — |
| `IncreaseLimitOrder` | — | — |
| `DecreaseLimitOrder` | — | 如果訂單已完全消費，關閉 `limitOrderState` |
| `SettleLimitOrder` | — | — |
| `CloseLimitOrder` | — | `limitOrderState`（租金 → `owner`） |
| `CreateDynamicFeeConfig` | `dynamicFeeConfig` | — |
| `CreateCustomizablePool` | `poolState`、`observation`、金庫——與 `CreatePool` 相同。如果 `enable_dynamic_fee = true`，快照 `dynamicFeeConfig`。 | — |
...
`TickArrayState` 帳戶**永遠不會被程式關閉**——它們在池的整個生命週期內持續存在。一旦 tick 陣列已初始化，即使其中的每個 tick 都回到 `liquidity_gross == 0`，它也會保留在鏈上。重用現有 tick 陣列是免費的；只有第一個接觸從未初始化陣列的位置才支付其租金。

## 在哪裡閱讀什麼

- **Tick 數學和範圍機制**：[`products/clmm/ticks-and-positions`](/zh-Hant/products/clmm/ticks-and-positions)。
- **交換遍歷和費用增長數學**：[`products/clmm/math`](/zh-Hant/products/clmm/math)。
- **指令帳戶清單**：[`products/clmm/instructions`](/zh-Hant/products/clmm/instructions)。
- **費用和獎勵應計**：[`products/clmm/fees`](/zh-Hant/products/clmm/fees)。
- **規範程式 ID 和種子**：[`reference/program-addresses`](/zh-Hant/reference/program-addresses)。
