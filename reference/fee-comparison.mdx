...
CPMM's `AmmConfig` carries a fourth rate that this payload does not necessarily surface: `creator_fee_share_rate`, the protocol's share **of the creator fee**, added on 2026-09-13. It is a fraction of the accrued creator bucket, not of volume and not of the trade fee, it is applied when the creator fee is collected rather than on a swap, and a per-creator `CreatorFeeShare` PDA can override it. Read it off the `AmmConfig` account. Details in [`products/cpmm/fees`](/products/cpmm/fees#protocol-share-of-the-creator-fee).
