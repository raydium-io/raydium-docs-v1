---
title: "2026-09-19 — CPMM: the protocol takes a share of the creator fee"
description: "CPMM can now retain a configurable share of the creator fee, applied when the fee is collected rather than when it is charged. AmmConfig gains creator_fee_share_rate (carved from padding, same size), a new CreatorFeeShare PDA overrides it per creator, and CreateCreatorFeeShare / CloseCreatorFeeShare manage it. Both CollectCreatorFee paths gain trailing accounts — breaking for existing clients. UpdateAmmConfig gains param 8. No new error codes, no PoolState change, swap math untouched."
---

<Info>
  This entry covers an upcoming CPMM program update. It was verified against the local release branch (`bdeee23`, 15 September 2026) before deployment. Confirm the deployed program before relying on the new instructions or the changed account lists.
</Info>
...
- **Both creator-fee collection instructions changed their account lists. This is breaking, but positionally clean.** `CollectCreatorFee` gains `creator_fee_share` at position 15. `CollectCreatorFeePermissionless` gains `amm_config` at 15 and `creator_fee_share` at 16. Both are appended after `system_program`, so every account an existing client already passes keeps its index — the old transaction simply comes up short. Append the new accounts; do not reorder anything else.
...
`CollectCreatorFee` — one account appended:

| # | Before | After |
|---|--------|-------|
| 1–14 | `creator` … `system_program` | unchanged, same order |
| 15 | — | **`creator_fee_share`** (new) |

`CollectCreatorFeePermissionless` — two accounts appended:

| # | Before | After |
|---|--------|-------|
| 1–14 | `payer` … `system_program` | unchanged, same order |
| 15 | — | **`amm_config`** (new) |
| 16 | — | **`creator_fee_share`** (new) |

<Warning>
  The new accounts sit at the **end** of both lists, so nothing an existing client already passes moves — and the failure mode is the readable one: an old transaction is short of accounts and Anchor rejects it with `AccountNotEnoughKeys` (`3005`) before any constraint runs, rather than misreading a vault as a config. That does not make the change optional. Both accounts are required on every call. Regenerate from the new IDL, and check that any SDK release you pin carries the new accounts before pointing it at the upgraded program.
</Warning>
