...
Treat binding a Token-2022 quote mint as a high-trust action, for the same reason registering a mint in the CPMM/CLMM support-mint registry is — see [`reference/token-2022-support`](/reference/token-2022-support). A `TransferHook` or `PermanentDelegate` quote mint would be admitted at config-creation time and would then run on, or be able to sweep, every pool quoted in it.
