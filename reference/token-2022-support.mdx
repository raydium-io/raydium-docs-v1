...
Position NFT accounts remain unfrozen by default. CLMM freezes one only when the position uses a V2 open path **and** at least one vault mint's current `freeze_authority` matches the hardcoded restricted-issuer list. The initial list contains the same authority used by the Superstate detection path below. Setting the pool PDA as the position NFT mint's freeze authority is not itself a freeze. This is a position-custody rule, not another pool-creation allow-list:
...
- **Base mint** — LaunchLab creates it. A Token-2022 base mint is only reachable through `initialize_with_token_2022`, and the program will only attach `MetadataPointer` and (optionally) `TransferFeeConfig`. Anything else returns `NoSupportExtension`. A pre-existing Token-2022 mint cannot be supplied as the base at all.
...
- Its freeze authority equals `superstate_allowlist::ID`. See [`reference/program-addresses`](/reference/program-addresses#clmm-restricted-issuer-freeze-authorities) for the mainnet-beta and devnet values.
