...
| 6030 | `CurveRuleFieldNotSupportedByCurve` | A constraint reads `total_base_sell` on a config whose curve derives it — the fixed-price and linear-price curves. Raised when the rule is written, not when a launch runs. |
