...
|------|---------|-----------------|---------------|----------------|
| 6000 | `NotApproved` | Not approved | Overloaded across eight instructions; only two are admin checks. **Swaps** — the pool's `Swap` status bit is paused, or trading has not opened yet (`block_timestamp < open_time`). **Deposit / withdraw** — the matching status bit is paused. **Pool creation** — `amm_config.disable_create_pool` is set, or a pool already exists for that config and mint pair. **Support-mint admin instructions** — wrong signer, or the mint is not Token-2022. See [`products/cpmm/instructions`](/products/cpmm/instructions) for the status bitmask. | "Swaps are paused on this pool." / "This pool is not open for trading yet." / "Deposits are paused on this pool." / "A pool already exists for this pair and fee tier." |
...
The recent LaunchLab releases keep codes `6000`–`6021` unchanged. The 2026-08-17 release removed the former platform-global-access errors and assigned `6022` to the allow-account validation error; the 2026-08-24 release appends `6023`; the 2026-08-31 release appends `6024`–`6030`:
...
`6020` is retained deliberately. Anchor numbers variants by position, so deleting it would shift every code after it and break every client that decodes `6021`–`6030`.
