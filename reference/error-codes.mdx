...
| 6014 | `NoFeeCollect` | Fee is zero | A protocol, fund, or creator-fee collection instruction found no collectable fees. For `CollectCreatorFeePermissionless`, both creator counters were zero. | "Nothing to collect." Treat it as a benign race and stop. |
| 6015 | `LamportsCalculateError` | Lamports calculate error | `CollectExcessLamports` swept a wrapped-SOL account and the `SyncNative` → `UnwrapLamports` round-trip did not leave the wrapped token balance exactly where it started. Appended by the 2026-09 upgrade; codes `6000`–`6014` are unchanged. | N/A — admin-only path. |
...
| Code | Variant | `#[msg]` string | Typical cause | Recommended UX |
|------|---------|-----------------|---------------|----------------|
| 6000 | `NotApproved` | Not approved | Caller is not the configured admin for this instruction; **or** (permission instructions) the signer is neither `admin` nor `permission_pda_admin`; **or** (`OpenLimitOrder`) the input/output token account is frozen or the pool has swap / limit-order disabled; **or** (`ClosePosition`, frozen path) the supplied remaining account is not `personal_position.pool_id`. | "Only the pool admin can perform this action." / "This token account is frozen — thaw it before placing the order." / "Internal: wrong position pool account." |
...
| 6002 | `AccountLack` | Account lack | Required remaining account missing from the tx (typically tick-array or oracle extension); or `ClosePosition` is closing a frozen NFT without the pool as its first remaining account. | "Internal: missing account — refresh pool data or update the position-close builder." |
...
| 6051 | `InvalidTickArrayBitmapExtensionAccount` | TickArrayBitmapExtension account does not belong to this pool | The supplied bitmap-extension account is not the one derived for this pool. Added when the swap path stopped calling `find_program_address` per swap and started binding the extension by `pool_id` instead. | "Internal: wrong tick-array extension — refresh pool data." |

> **Note on renumbering.** The CLMM `ErrorCode` enum was renumbered in this release: five legacy variants (`LOK`, `ZeroMintAmount`, `InvalidLiquidity`, `TransactionTooOld`, `InvalidRewardDesiredAmount`) and several typos (`Liquitity`, `enought`, `emissiones`) were removed/fixed, and eleven new variants were appended (`6040`–`6050`) by the limit-order release; `6051` (`InvalidTickArrayBitmapExtensionAccount`) came from a separate, later change. That makes 52 variants in total, `6000`–`6051` — the same count the deployed program's on-chain IDL reports. Because Anchor numbers errors by source order, every code at or after `6000` has shifted relative to pre-release builds. Clients that hard-coded numeric codes against an earlier version need to remap.
...
|---------|--------------------------|-----------|
| AMM v4 | Custom u32 codes from `0`, not Anchor-style | [below](#amm-v4-ammerror-is-not-anchor-numbered) and [`products/amm-v4/instructions`](/products/amm-v4/instructions) |
...
The recent LaunchLab releases keep codes `6000`–`6021` unchanged. The 2026-08-17 release removed the former platform-global-access errors and assigned `6022` to the allow-account validation error; the 2026-08-24 release appends `6023`; the 2026-08-31 release appends `6024`–`6030`; the 2026-09-09 release appends `6031`:

| Code | Name | Meaning |
|---|---|---|
| 6018 | `NotEnoughRemainingAccounts` | A required remaining account is missing. This includes a required `PlatformAllowConfig`, a required `PlatformCurveRule`, and the expanded CPMM migration account set. |
| 6020 | `CurveParamIsNotExist` | Retired. Kept as a placeholder so the codes after it do not shift; the 2026-08-31 release removed its last caller. |
| 6022 | `InvalidPlatformAllowConfig` | The supplied allow account does not encode the expected platform and global config pair. |
| 6023 | `CalculateOverflow` | A Token-2022 transfer-fee or inverse-transfer-fee calculation could not be represented. Reachable on the exact-out trade paths when the quote mint's fee cannot be inverted (for example a 100%-fee mint). |
| 6024 | `InvalidPlatformCurveRule` | The supplied rule account is not the PDA of the platform and config pair being launched under. |
| 6025 | `CurveParamNotMatchPlatformRule` | The launch parameters satisfy no check group of the platform's rule. The one error a creator can actually hit — surface it as "this launch shape is not allowed on this platform". |
| 6026 | `CurveRuleGroupNotExist` | `RemovePlatformCurveRule` was called with a `group_id` the rule does not hold. |
| 6027 | `CurveRuleGroupsExceeded` | A rule cannot hold more than 10 check groups. |
| 6028 | `InvalidCurveRuleConstraint` | A constraint has an unknown field or operator, a group has more than 25 constraints, or the same `(field, op)` pair appears twice in one group. |
| 6029 | `InvalidCurveRuleAuthority` | The signer is neither `PlatformConfig.curve_rule_manager` nor the platform admin. |
| 6030 | `CurveRuleFieldNotSupportedByCurve` | A constraint reads one of the four constant-product-only fields — `TotalSellA` (`4`), `SellRateA` (`13`), `MigrateAmountA` (`15`), `MigrateRateA` (`16`) — on a config whose curve derives `total_base_sell` rather than taking it from the creator (the fixed-price and linear-price curves). Raised when the rule is written, not when a launch runs. |
| 6031 | `LamportsCalculateError` | `CollectExcessLamports` swept a wrapped-SOL vault and the `SyncNative` → `UnwrapLamports` round-trip did not leave the wrapped token balance exactly where it started. Admin-only path. |

The old `PlatformGlobalAccessDenied` (`6022`) and `InvalidPlatformGlobalAccess` (`6023`) variants are removed. Do not decode `6022` using the previous IDL, and note that `6023` — vacated by the 2026-08-17 release — is reused by the 2026-08-24 release for an unrelated error, so it must not be decoded with an IDL from either earlier version.

`6020` is retained deliberately. Anchor numbers variants by position, so deleting it would shift every code after it and break every client that decodes `6021`–`6031`.

### AMM v4: `AmmError` is not Anchor-numbered

AMM v4 predates Anchor. `AmmError` is a plain `thiserror` enum with `FromPrimitive`, converted through `ProgramError::Custom(e as u32)`, so its codes start at **`0`**, not `6000` — and, like Anchor's, are assigned by source order. The 2026-09 upgrade appends one variant:

| Code | Variant | `#[error]` string | Cause |
|---|---|---|---|
| 60 | `LamportsCalculateError` | Lamports calculate error. | `WithdrawExcessLamports` swept a wrapped-SOL account and the `SyncNative` → `UnwrapLamports` round-trip did not leave the wrapped token balance exactly where it started. Admin-only path. |

Codes `0`–`59` are unchanged, so nothing shifts. In transaction logs this surfaces as `custom program error: 0x3c`. Regeneration source: [github.com/raydium-io/raydium-amm — error.rs](https://github.com/raydium-io/raydium-amm/blob/master/program/src/error.rs).
