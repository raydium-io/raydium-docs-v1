...
The LaunchLab release keeps codes `6000`–`6021` unchanged. It removes the former platform-global-access errors and assigns `6022` to the new allow-account validation error:
...
The old `PlatformGlobalAccessDenied` (`6022`) and `InvalidPlatformGlobalAccess` (`6023`) variants are removed. Do not decode `6022` using the previous IDL.
