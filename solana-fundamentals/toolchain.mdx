...
<Warning>
  `release.solana.com` no longer serves the installer — the HTTPS request fails outright, so any script or CI job still pointing at it dies before the install even starts. Anza took over the validator client; the installer lives at `release.anza.xyz`.
</Warning>

Verify:

```bash
solana --version   # → solana-cli 4.2.2 (or later)
```
...
`0.32.1` is the version Raydium's own `raydium-cp-swap` and `raydium-clmm` crates pin, so it is the version to match if you are going to CPI into them. Anchor's latest release line is `1.x`; you can install it alongside and `avm use` per project.
...
anchor idl type program.idl.json -o program.ts          # generate a TS type from an IDL
anchor build                                             # build a program from source
```

<Warning>
  `anchor idl fetch` needs a CLI matching the program's IDL mechanism, and Raydium uses both. CLMM and LaunchLab keep a legacy `anchor:idl` account (`0.29`–`0.32`); CPMM moved to Program Metadata and needs `1.x`. The wrong CLI reports `Account not found` at the wrong address. See [`sdk-api/anchor-idl`](/sdk-api/anchor-idl).
</Warning>
...
```bash
rustup install stable           # Anchor does not publish an MSRV; stable tracks it
rustup component add rustfmt clippy
agave-install init 4.2.2        # Anza's installer; BPF toolchain included
```

Anchor `0.32.1` builds against the `solana-*` `2.x` program crates, so anything from the Agave `2.x` line onward works. `solana-install` is the pre-Anza name for `agave-install` and still exists as an alias on older installs. Pin whatever version your CI has and bump deliberately — a mismatch between the BPF toolchain and your `solana-*` crate versions produces link errors rather than readable ones.
...
<Warning>
  **Use the git dependencies, not the same-named crates on crates.io.** Raydium publishes neither crate. The `raydium-cp-swap` and `raydium-clmm` entries that exist on crates.io are third-party uploads by unrelated accounts, and their versions do not track the programs' own — `raydium-cp-swap = "0.2"` does not even resolve. Depending on them means running someone else's code inside your program.
</Warning>

Pin the anchor version to the one the Raydium crates pin — currently `0.32.1`. `raydium-clmm` pins it exactly (`anchor-lang = "=0.32.1"`), so there is no version range to negotiate. Two different `anchor-lang` versions in one dependency graph will not compile, because the `Accounts` and `CpiContext` traits are not shared across versions.
