...
| `@anchor-lang/core` | `^1.0` (currently resolves to `1.2.0`) | The Anchor 1.x TypeScript client. Raydium's own repos pin exactly `1.0.2`; for client-side use any 1.x works, but pin `1.0.2` if you want to match their test suites. |
| `@raydium-io/raydium-sdk-v2` | `0.2.64-alpha` | The pin every code demo in these docs uses. |

<Note>
  **`@coral-xyz/anchor` is frozen; the package is now `@anchor-lang/core`.** `@coral-xyz/anchor` stopped at `0.32.1` when Anchor moved out of Coral. Anchor 1.x publishes as `@anchor-lang/core`, and Raydium's own program repositories switched their test suites to it in the 2026-09-09 upgrade. The imports you actually use are unchanged — `import { Program, BN, AnchorProvider } from "@anchor-lang/core"` — so migrating is a package rename plus a lockfile update.
