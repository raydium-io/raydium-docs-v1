...
For a 165-byte account (e.g., SPL Token account), rent-exemption was ~0.00204 SOL for years. It is now falling: SIMD-0437 cuts the rate by 90% across five steps, and accounts funded before each step end up holding more than they need. See [Rent and reclaimable rent](/solana-fundamentals/rent-and-reclaimable-rent) for the schedule, the arithmetic, and how the token programs hand the excess back.

<Note>
  The figures in this section are quoted at the original rate of 6,960 lamports per byte. Compute live values with `getMinimumBalanceForRentExemption` rather than reusing them as constants.
</Note>
...
// The exported layout is CpmmPoolInfoLayout, and it already accounts for the
// 8-byte discriminator — do not slice it off.
...
- [Solana Account docs](https://solana.com/docs/core/accounts).
- [SPL Token program](https://www.solana-program.com/docs/token) — where token accounts live.
