...
Step 1 activated on mainnet on 3 September 2026. Step 2 reached testnet the same day and is expected on mainnet in mid-September 2026; steps 3–5 are held for Agave 4.4, expected around November 2026. Treat the schedule as subject to change — the Foundation has said it will pause the rollout if state growth misbehaves — and read the live value from the cluster rather than hardcoding it.
...
Wrapped SOL accounts are the one token-program exception: their lamport balance *is* their token balance, so both programs reject them with `TokenError::NativeNotSupported`. **Both** programs instead expose `UnwrapLamports` (discriminant 45) for that case — it is in `spl-token-interface` alongside `WithdrawExcessLamports`, so the legacy program has it too, not just Token-2022 — and `@solana/spl-token` ships `createUnwrapLamportsInstruction` for it as of 0.4.15. Skip native accounts in a plain rent sweep and handle them deliberately; the pattern for doing that safely is the one Raydium's own programs use, [below](#what-the-raydium-programs-sweep-on-their-own-side).
...
`@solana/spl-token` does not export a builder for it. As of `0.4.15` the enum entry is still commented out — and note upstream spells the identifier `WithdrawalExcessLamports`, with the extra "al", so grep for that:
...
<Info>
  **Version banner.** These demos target `@raydium-io/raydium-sdk-v2@0.2.64-alpha` against Solana mainnet-beta, verified 2026-09; the `raydium-sdk-V2-demo` repo itself currently installs `0.2.62-alpha`, and the two are interchangeable here. `WithdrawExcessLamports` is encoded by hand and is independent of the SDK version — the SDK is used only for transaction building and batching.
</Info>
...
## What the Raydium programs sweep on their own side

Your wallet is not the only place the reduction frees lamports. Every pool holds rent too — vaults, LP mints, and the program-owned state accounts that were all funded at the old rate. That rent belongs to the protocol, not to LPs: it was paid by whoever created the account, it is not part of any pool's reserves, and it never entered the curve.

Three programs gained an admin instruction on 2026-09-09 to return it:

| Program | Instruction | Signer |
|---|---|---|
| AMM v4 | `WithdrawExcessLamports` (tag 18) | A dedicated collect-lamports wallet only |
| CPMM | `CollectExcessLamports` | The program admin or a dedicated collect-lamports wallet |
| LaunchLab | `CollectExcessLamports` | The program admin or a dedicated collect-lamports wallet |

Addresses are in [`reference/program-addresses`](/reference/program-addresses#excess-lamports-collection-wallets). CLMM and Stable AMM were not part of that release.

**Nothing here affects an LP or a trader.** These instructions move lamports and only lamports. Token balances, account data, owners, pool status, LP supply, fee counters and the curve are all untouched, and none of them can close an account. A pool's quote of a swap is identical before and after a sweep. There is no user-side action, no opt-in, and no deadline.

### The three account shapes each program handles

All three follow the same dispatch, on the source account's owner:

- **A token account or mint owned by a program authority PDA** — the program CPIs the token program's `WithdrawExcessLamports` (discriminant 38), signing as that PDA.
- **A wrapped-SOL vault** — `WithdrawExcessLamports` refuses native accounts, so the program `SyncNative`s first (which folds the donated excess into the wrapped `amount`), measures exactly how much the wrapped amount grew, `UnwrapLamports` for that delta, and then asserts the wrapped balance is back to its pre-sync value. If that check fails the whole instruction reverts with a `LamportsCalculateError`. This is why a SOL-side pool vault keeps its full liquidity through a sweep.
- **A program-owned state account** — `AmmInfo`, `PoolState`, `AmmConfig`, `ObservationState`, a `PlatformConfig`, and so on. A program can debit its own accounts directly, so it moves the balance down to `rent.minimum_balance(data_len)` with no CPI at all.

Accounts owned by anything else are skipped silently, so passing an unrelated account is harmless rather than fatal.

The wrapped-SOL sequence is worth copying if you hold native accounts of your own: it is the only way to take the excess off a wSOL account without changing what the account reports as its token balance.

<Note>
  **These paths depend on the deployed token program, not on a crate version.** All three programs hand-encode the token instructions — a single `38` byte for `WithdrawExcessLamports`, `45` plus a `COption<u64>` for `UnwrapLamports` — and send them to whichever token program owns the source account. Both instructions exist in the current mainnet SPL Token and Token-2022 programs. A local validator or test harness running an older bundled SPL Token build does not implement them, and a sweep against it fails on an unknown discriminator rather than on anything in the Raydium program. Test these paths against a mainnet-cloned token program.
</Note>

### What still cannot be reclaimed in place

A CLMM position's rent is unchanged by all of this — it comes back when the position is closed, as the table above says. Same for the LaunchLab base mint: the initialize instruction revokes `MintTokens` in the same call that mints the supply, so no key can ever sign a `WithdrawExcessLamports` for that mint and its rent is stranded by design.
