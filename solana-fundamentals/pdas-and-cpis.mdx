...
```rust
// CPMM's real field, on a zero-copy PoolState loaded via AccountLoader:
...
(CLMM's `PoolState` stores `bump: [u8; 1]` instead, so check the specific program's struct rather than assuming one shape.)
...
// Anchor 1.0 takes the program's Pubkey here; the AccountInfo form is 0.3x only.
...
```rust
// The crate is `raydium-cp-swap`; its lib name — and so the `use` path — is
// `raydium_cp_swap`. There is no `raydium_cpmm`.
...
let cpi_ctx = CpiContext::new_with_signer(
    cpmm_program.key(),     // Anchor 1.0: a Pubkey, not an AccountInfo
    cpi_accounts,
    &[seeds],
);
...
- [Anchor PDA docs](https://www.anchor-lang.com/docs/basics/pda) and [Anchor CPI docs](https://www.anchor-lang.com/docs/basics/cpi) — Anchor has no combined page.
