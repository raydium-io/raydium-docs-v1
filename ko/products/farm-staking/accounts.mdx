...
SDK는 `raydium.api.fetchFarmInfoById({ ids })`에서 표시 수준 뷰를, `raydium.api.fetchFarmKeysById({ ids })`에서 원시 계정 키 — 권한, LP 볼트, 슬롯별 보상 볼트 — 를 반환합니다. (`raydium.farm.getFarmById`는 존재하지 않습니다.) 동일한 데이터는 HTTP `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`로도 이용할 수 있습니다.
