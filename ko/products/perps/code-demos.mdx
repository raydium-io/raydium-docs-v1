...
<Info>
  **Raydium Perps는 Orderly Network의 화이트라벨 배포입니다.** 오더북, 매칭 엔진, 계정 상태 모두 Orderly에 존재합니다. Raydium SDK v2 (`@raydium-io/raydium-sdk-v2`)는 perps를 다루지 **않습니다** — 프로그래매틱 접근을 위해서는 Orderly의 REST + WebSocket API를 직접 사용하세요. 아래 스니펫은 가장 흔한 흐름들을 보여주며, 정식 참고자료는 [Orderly API 소개](https://orderly.network/docs/build-on-omnichain/introduction)이고, 그 아래에 [REST 엔드포인트](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order)와 [WebSocket 스트림](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction)이 문서화되어 있습니다.

  Orderly의 API 표면은 계속 진화합니다. 이 스니펫들을 프로덕션에 복사하기 전에 [Orderly API 문서](https://orderly.network/docs/build-on-omnichain/introduction)를 확인하세요. (온체인 입금용 Solana 클러스터는 `mainnet-beta`입니다.)
</Info>
...
<Info>
  **버전 배너.**
  ...
  Orderly의 API는 진화합니다. 이 스니펫들을 프로덕션에 복사하기 전에 [orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction)를 확인하세요.
</Info>
...
- [Orderly Network 개발자 문서](https://orderly.network/docs/build-on-omnichain/introduction) — 위에서 사용된 API 표면의 정식 참고자료. Raydium Perps는 이를 직접 사용합니다.
- [Orderly 지원 체인](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — Solana의 상태와 체인별 정산 볼트 주소를 확인합니다.
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — 동일한 REST 계층을 감싼, 커뮤니티에 공개된 JS 래퍼입니다. **게시된 릴리스가 없고** 명시된 유지 관리 정책도 없으므로 의존성이 아니라 참고 코드로 취급하세요. 위의 서명 계층은 직접 소유할 만큼 작습니다.
