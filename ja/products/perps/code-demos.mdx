...
<Info>
  **Raydium Perpsは、Orderly Networkのホワイトラベルデプロイメントです。** オーダーブック、マッチングエンジン、アカウント状態はすべてOrderlyに配置されています。Raydium SDK v2（`@raydium-io/raydium-sdk-v2`）はPerpsをサポート**していません**。プログラマティックアクセスの場合は、OrderlyのREST + WebSocket APIを直接使用してください。以下のスニペットは最も一般的なフローを示しています。正規リファレンスは[Orderly API イントロダクション](https://orderly.network/docs/build-on-omnichain/introduction)で、その下に[REST エンドポイント](https://orderly.network/docs/build-on-omnichain/restful-api/private/create-order)と[WebSocket ストリーム](https://orderly.network/docs/build-on-omnichain/websocket-api/introduction)が文書化されています。

  オンチェーン入金は Solana `mainnet-beta` 上で行われます。OrderlyのAPI サーフェスは進化しています。本番環境にこれらのスニペットをコピーする前に、[Orderly API ドキュメント](https://orderly.network/docs/build-on-omnichain/introduction)を確認してください。
</Info>
...
<Info>
  **バージョン情報**
  ...
  OrderlyのAPI サーフェスは進化しています。本番環境にこれらのスニペットをコピーする前に、[orderly.network/docs](https://orderly.network/docs/build-on-omnichain/introduction)を確認してください。
</Info>
...
- [Orderly Network開発者ドキュメント](https://orderly.network/docs/build-on-omnichain/introduction) — 上記で使用されるAPIサーフェスの正規リファレンス。Raydium Perpsは直接これを消費しています。
- [Orderly のサポート対象チェーン](https://orderly.network/docs/introduction/trade-on-orderly/supported-chains) — Solana のステータスとチェーンごとの決済ボルトアドレスを確認できます。
- [`OrderlyNetwork/orderly-sdk-js`](https://github.com/OrderlyNetwork/orderly-sdk-js) — 同じ REST レイヤーをラップするコミュニティから見える JS ラッパー。**公開リリースがなく**、メンテナンス方針も明示されていないため、依存先ではなくリファレンス コードとして扱ってください。上記の署名レイヤーは自前で持てるほど小さいものです。
