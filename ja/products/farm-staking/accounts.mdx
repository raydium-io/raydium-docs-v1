...
SDK は `raydium.api.fetchFarmInfoById({ ids })` で表示レベルのビューを返し、`raydium.api.fetchFarmKeysById({ ids })` で生のアカウント キー — オーソリティ、LP ボルト、スロットごとのリワード ボルト — を返します。（`raydium.farm.getFarmById` は存在しません。）同じデータは HTTP でも `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>` から取得できます。
