...
| 6 | `user_base_token` | W |   | ベースミント上の受益者の ATA；アンロックされたトークンを受け取ります。`init_if_needed`。 |
| 7 | `base_token_mint` |   |   | プールの基本ミント。 |
| 8 | `base_token_program` |   |   | ベースミントを所有するプログラム — SPL Token または Token-2022。 |
| 9 | `system_program` |   |   | 受益者 ATA を作成する必要がある場合に資金を提供します。 |
| 10 | `associated_token_program` |   |   | 受益者 ATA を作成します。 |

<Warning>
  `system_program` は `associated_token_program` の **前** に来ます。どちらも固定アドレスを持つ
  `Program<'info, _>` 型なので、入れ替えるとアドレス制約に失敗し、受益者はクレームできません。
</Warning>

**前提条件**

- `block_time >= pool_state.vesting_schedule.start_time`（そうでなければ `VestingNotStarted`）。
- `pool_state.status == PoolStatus::Trade`（`2`） — 卒業が既に発生している必要があります。それより前に呼び出すと `PoolNotMigrated` でリバートされます。
...
`2027-09-28`（`start_time` の 90 日後）、受益者 A が `ClaimVestedToken` を呼び出します：
...
6 ヶ月後（`2028-03-26`、`start_time` の 270 日後）、A が再度クレームします：
