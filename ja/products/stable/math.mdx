...
AMM v4 と同じ算術です — 完全な導出については [`products/amm-v4/math`](/ja/products/amm-v4/math) を参照してください — ただし Stable プール独自の数値を使用します。

```
gross_fee = amount_in * (swap_fee_numerator / swap_fee_denominator)    // 2/10_000 = 0.02%
lp_portion = gross_fee - (gross_fee * pnl_numerator / pnl_denominator) // その 88% = 0.0176%
pnl_portion = gross_fee * (pnl_numerator / pnl_denominator)            // その 12% = 0.0024%
```
