...
<Info>
  このページでは、各アカウントの**レイアウトと役割**について説明します。シード値は正規のものであり、[`reference/program-addresses`](/ja/reference/program-addresses) に記載されています。CLMM プールは CPMM プールよりもアカウント数が多いです。これはリクイディティがティック範囲全体に疎らに保存されるためです。この疎性の理解がこのページの大部分を占めます。
</Info>
...
| Position NFT ミント | 供給量 1 のミント、`PersonalPositionState` に関連付けられています。NFT を転送するとポジションが転送されます。 | ポジションごとに 1 つ |
...
| `LimitOrderState` | オープン リミット オーダーごとに 1 つ。所有者、ティック、サイド、総額、決済済み出力スナップショットを記録。 | オーダーごとに 1 つ |
| `LimitOrderNonce` | `(wallet, nonce_index)` ごとのカウンター。一意のオーダー PDA を導出。 | (wallet, index) ごとに 1 つ |
...
- **`liquidity`** は**アクティブ**リクイディティです。範囲に `tick_current` を含むすべてのポジションの `L` 値の合計です。スワップがティックを越えるたびに、またはポジションが開かれる/閉じられる/サイズ変更されるたびに変わります。
...
- **`tick_array_bitmap`** は、スポット価格周辺の一般的に使用されるティック範囲をカバーする**インライン**ビットマップです。ポジションが遠くまで達するプールの場合、オーバーフロー追跡は別の `TickArrayBitmapExtension` に存在します。
- **`fee_on`** はプール作成時に固定されます。`0`（FromInput）は古典的な Uniswap-V3 の動作を再現します。`1` と `2` はスワップ フィーをブックの単一側にルーティングします。詳細は [`products/clmm/fees`](/ja/products/clmm/fees) を参照してください。
...
- **`dynamic_fee_info`** はダイナミック フィー サージの揮発性状態を保持します。有効な場合、すべてのスワップは `AmmConfig.trade_fee_rate` の上に `dynamic_fee_component` を再計算します。レイアウトは以下の `DynamicFeeInfo` に記載されています。ダイナミック フィーのないプールは構造体全体をゼロのままにします。
...
- `order_phase` はコホート ID です。コホートが「すべて未充足」から「部分的に充足」に遷移するたびにインクリメントされます。
- `orders_amount` は現在の（最新の）コホートの入力トークン合計です。
- `part_filled_orders_remaining` は、現在進行中のスワップによって充足されている前のコホートを追跡します。
- `unfilled_ratio_x64` はコホートに実行される Q64.64 乗数です。スワップがコホートの X% を充足すると、比率は `(1 − X)` で乗算されます。各オープン オーダーはオープン時に独自の `(order_phase, unfilled_ratio_x64)` スナップショットを保存するため、決済数学はスナップショットの比較に縮小されます。

ルール：

- ポジション エンドポイント ティック *t* は `t % tick_spacing == 0` を満たす必要があります。プログラムはスペーシング外のポジションを拒否します。
- ティックの**配列**は `floor(t / (TICK_ARRAY_SIZE * tick_spacing)) * (TICK_ARRAY_SIZE * tick_spacing)` に位置します。
- ティック配列は遅延初期化されます。初めてティックに触れるポジションまたはスワップが未初期化配列を作成し、レントを支払います。
- ティック配列は**プログラムによって決して閉じられません**。割り当てられると、プール全体の存続期間にわたって存続し、その中のすべてのティックが `liquidity_gross == 0` に戻った後でも。既存のアカウントを再利用することは無料です。ティック配列の追加レントはありません。
...
ポジションの範囲が「通常」の場合、拡張アカウントについて考える必要はありません。フルレンジ ポジション（例：`(MIN_TICK, MAX_TICK)`）にはそれが必要です。SDK がそれを解決します。
...
供給量 1 の SPL Token ミント。ミントのアドレスは決定論的 PDA です。所有者のウォレット内のポジション NFT は、そのシングル トークンを保持する ATA です。NFT を転送することは、ポジションが手を変える方法です。プログラムは、状態に保存された Pubkey ではなく、**NFT の ATA 残高の現在の保有者**に認可をキーします。

### `PersonalPositionState`

オープン ポジションごとに 1 つ。NFT ミントでキーされます。
...
<Note>
  古い CLMM リリースは、`(pool, tick_lower, tick_upper)` ごとの集計ブックキープを `ProtocolPositionState` PDA に保存していました。**新しいリリースはこのアカウントを作成または読み取りません。** スロットは ABI 互換性のために `OpenPosition` / `IncreaseLiquidity` / `DecreaseLiquidity` アカウント リストに `UncheckedAccount` として表示されますが、プログラムはそれに書き込みません。チェーン上の既存のアカウントは痕跡です。管理者は `CloseProtocolPosition` を呼び出してレントを回収できます。

  集計範囲ブックキープは、`TickArrayState` の 2 つのエンドポイント ティック（`liquidity_gross`、`liquidity_net`、およびティックごとの `fee_growth_outside_*` / `reward_growths_outside_x64`）から直接導出されるようになりました。フィー成長内部式 `fee_growth_inside = global − outside_lower − outside_upper` は、集計ポジション アカウントなしで機能し続けます。
</Note>
...
| 定数 | 値 | 意味 |
|---|---|---|
| `VOLATILITY_ACCUMULATOR_SCALE` | `10_000` | 揮発性アキュムレータの粒度 |
...
1. **オープン** — ユーザーが `open_limit_order` を呼び出し、入力トークンの `total_amount` をデポジットし、オーダーは `TickState` コホートにバインドされます。
2. **（オプション）増加 / 減少** — `increase_limit_order` は `total_amount` に追加します。`decrease_limit_order` は未充足トークン（およびその時点までの決済済み出力）を返します。
3. **決済** — コホートが完全または部分的に充足されると、所有者**または**運用キーパーが `settle_limit_order` を呼び出して、出力トークンを所有者の ATA にプッシュします。
...
`(wallet, nonce_index)` ごとのカウンター。単一ユーザーが PDA 衝突なしでリミット オーダーの複数の並列パイプラインを実行できます。
...
PDA シード：`["permission", authority.as_ref()]`。管理者が `CreatePermissionPda` 経由で作成し、`ClosePermissionPda` 経由で削除されます（レント払い戻しは呼び出し元に）。両方の管理者命令は、プログラム `admin` または専用の `permission_pda_admin` キーのいずれかを受け入れます。PDA をクローズすると許可が取り消されます。権限はそれ以上プールを作成できなくなりますが、既に作成したプールは影響を受けません。
...
正確なシード文字列は常にオンチェーン IDL と [`reference/program-addresses`](/ja/reference/program-addresses) に対して二重チェックする必要があります。
...
| `OpenPosition[WithToken22Nft]` | NFT ミント + ATA、`personalPosition`、場合によっては新しい `tickArrayState`(s)、まだ存在しない場合は `tickArrayBitmapExtension` | — |
| `IncreaseLiquidity` | 場合によっては新しい `tickArrayState`(s) | — |
...
| `OpenLimitOrder` | `limitOrderState`、場合によっては `limitOrderNonce`（初期化が必要な場合）、場合によっては新しい `tickArrayState` | — |
| `IncreaseLimitOrder` | — | — |
...
| `CreateCustomizablePool` | `poolState`、`observation`、ボルト — `CreatePool` と同じ。`enable_dynamic_fee = true` の場合は `dynamicFeeConfig` をスナップショット。 | — |
...
`TickArrayState` アカウントは**プログラムによって決して閉じられません** — プール全体の存続期間にわたって存続します。ティック配列が初期化されると、その中のすべてのティックが `liquidity_gross == 0` に戻った後でも、チェーン上に残ります。既存のティック配列を再利用することは無料です。初めて未初期化配列に触れるポジションのみがそのレントを支払います。
