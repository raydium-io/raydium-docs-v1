...
`PoolState.tick_array_bitmap`（インライン）は「スポットに近い」範囲をカバーします — **±512** ティック配列です（`TICK_ARRAY_BITMAP_SIZE = 512`；`[u64; 16]` フィールドは 1,024 ビットで、負と正の開始インデックスに均等に分割されるため、配列インデックスは −512 … +511）。その範囲外（極端なティック値の場合）、プログラムは拡張アカウントを保持します：
