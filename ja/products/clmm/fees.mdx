...
1. **リファレンスのデケイ**。`now - last_update_timestamp >= filter_period` の場合、ボラティリティリファレンスがデケイします。境界はどちらも `>` ではなく `>=` であることに注意してください：
...
レイアウトのフィールドは `tokenFeesOwedA` / `tokenFeesOwedB` であり（`...Owed0` / `...Owed1` ではありません）、`rewardInfos` の各エントリは `{ growthInsideLastX64, rewardAmountOwed }` です — リワードのミント自体は、ポジションではなくプールの `rewardInfos` から取得します。

`tokenFeesOwed*` と `rewardAmountOwed` は、ポジションが最後にタッチされた時点のスナップショットです。**現在の**値（それ以降の成長を反映したもの）を確認するには、リクイディティ 0 の `DecreaseLiquidity` をシミュレートするか、グローバルの `fee_growth_*` と 2 つのティック外スナップショットから再計算してください。
