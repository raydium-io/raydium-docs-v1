...
<Info>
  **このページは AI による自動翻訳です。すべての内容は英語版を正とします。**
  ...
  サポートマトリクス：**CPMM** は Token-2022 を、トランスファーフィー付きミントを含めて完全にサポートします。**CLMM** は明示的な `SwapV2` アカウント経由で、トランスファーフィー付きの Token-2022 をサポートします。**AMM v4** は Token-2022 をまったくサポートしません。**LaunchLab** は `InitializeV2` を通じてクラシックな SPL ベースミントを作成しますが、Token-2022 のベースミントを作成し、オプションで `TransferFeeConfig` を付加する専用の `InitializeWithToken2022` エントリポイントを持ちます。そのクォート側はどちらのトークンプログラムも受け付けます。[`products/launchlab/platform-config`](/ja/products/launchlab/platform-config#token-2022-transfer-fee-authorities)を参照してください。**Farm v6** はステーキングミントとリワードミントの両方で Token-2022 をサポートします。
</Info>
...
≈ 986.03 USDC。アウトバウンドトランスファーフィーなしなので、ユーザーは ≈ 986.03 USDC を受け取ります。
...
- [SPL Token-2022 トランスファーフィー拡張機能ドキュメント](https://www.solana-program.com/docs/token-2022/extensions#transfer-fees)
