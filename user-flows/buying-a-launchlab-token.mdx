...
LaunchLab tokens are issued on a **bonding curve**: a smart-contract-managed price curve where price rises as more tokens are bought and falls when tokens are sold. The curve operates until a graduation threshold is reached, at which point every newly initialized launch migrates to a Raydium CPMM pool and becomes tradable through normal DEX routing.
