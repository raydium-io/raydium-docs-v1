...
2. **Pick fee tier.** Use the UI recommendation as a starting point; see [picking the right fee tier](/user-flows/dynamic-fees#picking-the-right-fee-tier).
3. **(Optional) Enable [dynamic fees](/user-flows/dynamic-fees#dynamic-fees-clmm).** A volatility-driven surcharge on top of the base tier.
4. **(Optional) Enable [quote-only fees](/user-flows/dynamic-fees#quote-only-fees-clmm).** All fee income settles in one token you pick.
5. **Set initial price.** Use an existing market price. If a pool already exists for the pair, compare against it.
6. **Choose the first range.** Enter the lower and upper price for the initial position.
7. **Enter deposit amount.** The UI computes the required token ratio from your range.
8. **Review and sign.** Confirm the pair, fee tier, fee features, initial price, range, deposit amounts, and rent.
