...
If you are creating a new pool, start with [Why create a pool?](/user-flows/why-create-a-pool).

## Add liquidity to a CPMM pool

1. Navigate to the CPMM pool page from the [Liquidity Pools page](https://raydium.io/liquidity-pools/).
2. Click **Deposit**.
...
### Stake your LP tokens (optional)

Depositing gives you fungible LP tokens that represent your share of the pool. Staking is a separate, optional step: if the pool has an active farm, you can stake those LP tokens to earn additional rewards on top of your share of swap fees.

- After depositing, the UI may prompt you to stake your LP tokens in the farm. You can accept to stake right away.
- If you skip it, you can stake later from the [Portfolio page](https://raydium.io/portfolio/) whenever you like.

Staking is optional and independent from providing liquidity. Your liquidity keeps earning swap fees whether or not the LP tokens are staked, and you can unstake at any time before removing liquidity.

## Remove liquidity from a CPMM pool

1. Open your position from the [Portfolio page](https://raydium.io/portfolio/).
...
<Info>
  Most new CLMM positions remain unfrozen and transferable. A position is bound to its initial NFT owner only when it uses a V2 open path and either pool mint's freeze authority matches CLMM's restricted-issuer list. That matching NFT account is frozen, so it cannot be transferred or moved into an escrow. Its owner can still increase or decrease liquidity, collect fees and rewards, and close the position through Raydium.
</Info>

## Open a CLMM position

1. Navigate to the CLMM pool page from the [Liquidity Pools page](https://raydium.io/liquidity-pools/).
...
If the position belongs to a restricted-issuer pool, use the current Raydium UI or an updated SDK to close it. The close transaction must include the pool account so the CLMM program can atomically thaw and burn the frozen position NFT.
