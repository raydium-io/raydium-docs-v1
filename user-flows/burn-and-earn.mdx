...
<Warning>
  A frozen CLMM position from a restricted-issuer pool cannot enter Burn & Earn because its position NFT cannot be transferred into the lock escrow. Check the position NFT token account before offering this flow. Ordinary CLMM positions remain transferable and supported.
</Warning>
...
Tokens graduated from [LaunchLab](/products/launchlab) use LP-Lock for locked LP. Before the 2026-08-14 upgrade, `creatorScale` produced a creator-owned Fee Key. Migrations executed afterward combine `platformScale + creatorScale` into one platform Fee Key position and burn `burnScale`. Fee Keys from earlier completed migrations remain unchanged. See [`products/launchlab/platforms`](/products/launchlab/platforms) and [`products/launchlab/creator-fees`](/products/launchlab/creator-fees).
