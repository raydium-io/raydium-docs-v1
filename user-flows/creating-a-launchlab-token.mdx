...
| Post-migration fees | Platform-defined CPMM creator fee and LP policy | Platform-defined CPMM creator fee and LP policy |
...
- Read the current platform config before signing. Before the 2026-08-17 upgrade, `creatorScale` produced a creator Fee Key. For migrations executed afterward, `platformScale + creatorScale` becomes a platform-owned Fee Key position and `burnScale` is burned. Existing creator Fee Keys from earlier migrations are unchanged.
- Your post-graduation creator revenue comes from CPMM creator fees only when you remain the recorded CPMM pool creator and the selected config enables them.

See [How creator fees work](/user-flows/how-creator-fees-work) for details.
...
No action is required from you as creator, but it is worth knowing that graduation is two separate events rather than one:

1. **The curve closes by itself.** The buy that pushes the curve past its fundraising target closes the curve inside that same transaction. Nobody calls anything, there is no bounty, and there is nothing to race.
2. **Raydium's migration wallet creates the pool.** Only that wallet can sign the migration instruction, so there is a short window where your token trades on neither the curve nor a pool. This is normal; do not tell your community the pool is live until you can see it.

After graduation:
- A CPMM pool is created with the collected SOL and remaining curve tokens as initial liquidity.
- If post-migration fees are enabled, a **Fee Key NFT** is minted — to the platform's NFT wallet for launches migrated after the 2026-08-17 upgrade, and to the creator for launches migrated before it. See [how creator fees work](/user-flows/how-creator-fees-work).
