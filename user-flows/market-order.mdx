---
title: "Market orders"
description: "Placing a market swap through the Raydium UI: choose tokens, review the route, set slippage and priority fees, sign, and troubleshoot common failures."
---

<Info>
  A market order fills immediately at the current best price the router can find across Raydium's pools. It's the default order type and the most common way to swap. This page walks through placing one — the inputs, the settings you control, and the failures you might hit. For limit orders, see [Limit order](/user-flows/limit-order).
</Info>
...
- **AMM v4** pools hold the deepest legacy liquidity on long-established pairs (SOL-USDC, RAY-USDC). They now trade as pure constant-product AMMs (OpenBook integration deactivated). Most new pairs never touch AMM v4.
