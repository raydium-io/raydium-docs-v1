...
Cùng số học như AMM v4 — xem [`products/amm-v4/math`](/vi/products/amm-v4/math) để xem đầy đủ cách dẫn xuất — nhưng với các con số riêng của các pool Stable.

```
gross_fee = amount_in * (swap_fee_numerator / swap_fee_denominator)    // 2/10_000 = 0.02%
lp_portion = gross_fee - (gross_fee * pnl_numerator / pnl_denominator) // 88% của nó = 0.0176%
pnl_portion = gross_fee * (pnl_numerator / pnl_denominator)            // 12% của nó = 0.0024%
```
