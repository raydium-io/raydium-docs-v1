...
Các tài khoản `GlobalConfig` có thể được tạo **và cập nhật** bởi quản trị viên chương trình LaunchLab hoặc bởi cơ quan tạo cấu hình được ủy quyền; bất kỳ người ký nào khác sẽ nhận `InvalidOwner` (`6001`). Xem [`reference/program-addresses`](/vi/reference/program-addresses) để biết các cơ quan có thẩm quyền chính thức.
...
- `max_share_fee_rate` — được khởi tạo thành `10_000` (100 bps). Giới hạn **đối số** `share_fee_rate` mà `BuyExactIn`, `BuyExactOut`, `SellExactIn`, và `SellExactOut` chấp nhận — phí giới thiệu được trả trực tiếp cho người nhận chia sẻ trong quá trình giao dịch. Nó **không** giới hạn `PlatformConfig.fee_rate`; tỷ lệ đó có các giới hạn riêng của nó, được mô tả trong [`platform-config`](/vi/products/launchlab/platform-config#rate-caps). `CreateConfig` thực thi `trade_fee_rate + max_share_fee_rate < RATE_DENOMINATOR_VALUE` tại thời điểm khởi tạo. Trên đường dẫn cập nhật, chỉ `param = 6` (`max_share_fee_rate`) kiểm tra lại tổng; `param = 0` (`trade_fee_rate`) chỉ khẳng định `trade_fee_rate < RATE_DENOMINATOR_VALUE`, nên việc chỉ tăng phí giao dịch có thể đẩy cặp này vượt quá giới hạn kết hợp. Hãy đặt chúng theo thứ tự giữ được bất biến.
...
`CreateConfig` chấp nhận một `quote_mint` Token-2022, và các pool được liên kết với cấu hình như vậy mang nó từ đầu đến cuối: kho báo giá, cả hai kho phí, và tài khoản token của người nhận phí chia sẻ đều được tạo trên Token-2022, và các lệnh swap tính toán `TransferFeeConfig` ở phía báo giá. Bit1 `PoolState.token_program_flag` ghi lại lựa chọn cho mỗi lần phát hành.

Hai ràng buộc đáng biết trước khi bạn dựa vào một:

- **`Initialize` không dùng nữa không thể phát hành chống lại nó.** Chỉ `InitializeV2` và `InitializeWithToken2022` chấp nhận chương trình báo giá Token-2022. Xem [`instructions`](/vi/products/launchlab/instructions#instruction-inventory).
- **Không có danh sách cho phép tiện ích nào được áp dụng.** Không giống như CPMM và CLMM, LaunchLab không sàng lọc các tiện ích của mint báo giá — `CreateConfig` lấy mint như đã cho, vì vậy cổng duy nhất là quyết định của quản trị viên về những mint nào được liên kết với cấu hình. Kiểm tra `NoSupportExtension` của chương trình chỉ bao gồm các mint cơ sở Token-2022 mà LaunchLab tự tạo. Coi việc liên kết một mint là một hành động đáng tin cậy cao; xem [`reference/token-2022-support`](/vi/reference/token-2022-support#launchlab-quote-mints).
...
`UpdateConfig` lấy một cặp `(param: u8, value: u64)` và chấp nhận người ký là quản trị viên chương trình LaunchLab **hoặc** cơ quan tạo cấu hình được ủy quyền — bất kỳ người ký nào khác sẽ nhận `InvalidOwner` (`6001`). Mỗi giá trị `param` chọn trường nào để thay đổi:

| `param` | Trường được thay đổi | Giá trị mới đến từ | Xác thực |
|---------|---------------|----------------------|------------|
| 0 | `trade_fee_rate` | `value` | `< 1_000_000` (được khẳng định — vi phạm sẽ panic thay vì trả về lỗi) |
| 1 | `protocol_fee_owner` | `remaining_accounts[0].key` | không được là pubkey mặc định |
| 2 | `migrate_fee_owner` | `remaining_accounts[0].key` | không được là pubkey mặc định |
| 3 | `migrate_to_amm_wallet` | `remaining_accounts[0].key` | không được là pubkey mặc định |
| 4 | `migrate_to_cpswap_wallet` | `remaining_accounts[0].key` | không được là pubkey mặc định |
| 5 | `migrate_fee` | `value` | không có |
...
<Warning>
  Bốn lần xoay ví là các giá trị `param` thông thường trong cùng bảng điều phối — chúng **không** phải
  một hình dạng đối số riêng. Chúng bỏ qua hoàn toàn `value` và đọc khóa mới từ `remaining_account`
  *đầu tiên*; việc bỏ qua tài khoản đó sẽ panic tại một `unwrap()`. Bất kỳ `param` nào lớn hơn `11`
  đều trả về `InvalidInput` (`6002`).
</Warning>
...
Lưu ý rằng tên các trường trong layout khác với tên Rust trên chuỗi: giới hạn dưới về huy động vốn được giải mã thành `minFundRaisingB`, giới hạn dưới về cung cấp thành `minSupplyA`, và các giới hạn dưới về tỷ lệ bán / di chuyển thành `minSellRateA` / `minMigrateRateA`.

Đối với giao diện người dùng liệt kê tất cả các cấu hình có sẵn, hãy truy vấn điểm cuối API LaunchLab phản ánh chúng (`raydium.api.fetchLaunchConfigs()`); số lượng tài khoản trên chuỗi rất nhỏ (một chữ số trong thực tế).
...
- [`products/launchlab/instructions`](/vi/products/launchlab/instructions) — `Initialize` và hướng dẫn cập nhật.
