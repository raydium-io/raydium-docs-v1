...
Một nhóm, ba ràng buộc, và người tạo có một hành lang 50–200 SOL. Theo danh sách trắng cũ, điều này cần một mục cho mỗi giá trị được phép, và giới hạn mười mục làm cho nó không thể.
...
<Info>
  **Biểu ngữ phiên bản.**
  ...
  - SDK: `@raydium-io/raydium-sdk-v2@0.2.64-alpha` — phiên bản mà mọi bản demo mã khác trên trang này được ghim, và cũng là phiên bản đã có hỗ trợ curve-rule. Cả hai hàm trợ giúp bên dưới, cộng với `getPdaPlatformCurveRule`, layout `PlatformCurveRule` và các enum `LaunchpadCurveRuleField` / `LaunchpadCurveRuleOp`, đều được export từ barrel của package; không cần tự chuyển tay từ `platform_curve_rule.rs`.
  - Cluster: kiểm tra trên Solana `devnet` trước — xem [Kiểm tra trên devnet trước tiên](#test-on-devnet-first).
