...
| 6 | `user_base_token` | W |   | ATA của người thụ hưởng trên base mint; nhận các token được mở khóa. `init_if_needed`. |
| 7 | `base_token_mint` |   |   | Mint base của pool. |
| 8 | `base_token_program` |   |   | Chương trình sở hữu base mint — SPL Token hoặc Token-2022. |
| 9 | `system_program` |   |   | Cấp vốn cho ATA của người thụ hưởng khi ATA đó phải được tạo. |
| 10 | `associated_token_program` |   |   | Tạo ATA của người thụ hưởng. |

<Warning>
  `system_program` đứng **trước** `associated_token_program`. Cả hai đều được gõ `Program<'info, _>`
  với địa chỉ cố định, nên một cặp bị đảo sẽ không thỏa ràng buộc địa chỉ và người thụ hưởng không
  thể claim.
</Warning>
...
- `pool_state.status == PoolStatus::Trade` (`2`) — graduation phải đã xảy ra. Gọi trước thời điểm đó sẽ revert với `PoolNotMigrated`.
- Delta lượng được mở khóa lớn hơn không. Một lệnh gọi không có tác dụng (delta được tính toán là 0) sẽ revert.
...
Trên `2027-09-28` (90 ngày sau `start_time`), Người thụ hưởng A gọi `ClaimVestedToken`:
...
Sáu tháng sau (`2028-03-26`, 270 ngày sau `start_time`), A yêu cầu lại:
