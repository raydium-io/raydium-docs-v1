...
1. **Phân rã tham chiếu.** Nếu `now - last_update_timestamp >= filter_period`, tham chiếu biến động sẽ phân rã. Lưu ý rằng cả hai ranh giới đều là `>=`, không phải `>`:
...
Các trường trong layout là `tokenFeesOwedA` / `tokenFeesOwedB` (không phải `...Owed0` / `...Owed1`), và mỗi mục của `rewardInfos` là `{ growthInsideLastX64, rewardAmountOwed }` — bản thân reward mint đến từ `rewardInfos` của pool, chứ không phải của vị thế.

`tokenFeesOwed*` và `rewardAmountOwed` là các snapshot từ lần cuối vị thế được chạm đến. Để xem giá trị **hiện tại** (phản ánh tăng trưởng kể từ đó), hãy mô phỏng một `DecreaseLiquidity` với thanh khoản bằng không, hoặc tính lại từ `fee_growth_*` toàn cục và hai snapshot tick-outside.
