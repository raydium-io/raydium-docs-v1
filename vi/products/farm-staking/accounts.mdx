...
SDK trả về khung nhìn ở cấp hiển thị qua `raydium.api.fetchFarmInfoById({ ids })` và các khóa tài khoản thô — authority, vault LP, các vault phần thưởng theo từng slot — qua `raydium.api.fetchFarmKeysById({ ids })`. (Không có `raydium.farm.getFarmById`.) Cùng dữ liệu đó cũng có sẵn qua HTTP tại `GET https://api-v3.raydium.io/main/farms/info?ids=<FARM_ID>`.
