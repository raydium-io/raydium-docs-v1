...
Giả sử bạn nạp \$10,000 SOL/USDC vào một CLMM ở mức SOL = \$160, khoảng `[$120, $200]`. Lưu ý rằng khoản nạp **không** chia đều 50/50: ở mức giá đó trong khoảng đó, vị thế nhận 27.54 SOL (\$4,407) và \$5,593 USDC, cố định `L ≈ 3,300`. Mốc so sánh HODL bên dưới là khoản nắm giữ 50/50 gồm \$5,000 + \$5,000, khớp với phép so sánh CPMM.

- Độ rộng khoảng: `P_hi / P_lo = 200 / 120 ≈ 1.67`. Hệ số đòn bẩy ≈ `1 / (1 − √(120/200)) ≈ 4.4×`.
- Nếu SOL chuyển tới \$180 (+12.5%), giá trị HODL = \$10,625; vị thế CLMM ≈ \$10,397; **IL ≈ 2.1%**.
- Nếu SOL chuyển tới \$200 (+25%), giá trị HODL = \$11,250; vị thế CLMM bây giờ là 100% USDC trị giá ≈ \$10,520; **IL ≈ 6.5%**.
- Nếu SOL chuyển tới \$240 (+50%, ngoài khoảng), giá trị HODL = \$12,500; vị thế CLMM vẫn ≈ \$10,520; **IL ≈ 15.8%**.

Cùng số vốn đó trong một CPMM (không có khoảng) sẽ chỉ có IL khoảng ~2.02% ở \$240 và tiếp tục kiếm phí. Khoảng cách đó chính là mức khuếch đại mà trang này mở đầu: xấp xỉ hệ số đòn bẩy nhân với IL của CPMM trong khi còn ở trong khoảng. Vị thế CLMM có khả năng bắt giữ phí cao hơn *khi ở trong khoảng* nhưng IL tệ hơn nhiều khi ở ngoài khoảng trong một khoảng thời gian dài.
...
- [`sdk-api/typescript-sdk`](/vi/sdk-api/typescript-sdk) — `raydium.clmm.getOwnerPositionInfo({ programId })` cùng với `PoolUtils.getAmountsFromLiquidity(...)` trả về exposure hiện tại; so sánh với cơ sở giá vào lệnh của bạn.
