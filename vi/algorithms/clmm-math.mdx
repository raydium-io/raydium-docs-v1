...
2. **Kiểm soát tràn số.** `sqrt_price · L` vừa trong `u256` cho tất cả các tham số hợp lý; `price · L` có thể tràn sớm hơn nhiều.
3. **Toán học tick đều đặn.** Vì tick được định nghĩa là `1.0001^i`, nên `sqrt(price) = (√1.0001)^i ≈ 1.00005^i` cũng là một thang hình học chính xác. (`√1.0001 = 1.00004999875…`, nên việc coi nó đúng bằng `1.00005` gây sai lệch tới ~0.6% tại `MAX_TICK`.) Mỗi lần vượt qua tick được dịch thành phép nhân nhỏ trong không gian `sqrt_price_x64`.
...
- **Oracle.** Vòng đệm `ObservationState` của Raydium chỉ lưu trữ `(block_timestamp: u32, tick_cumulative: i64)` cho mỗi slot cùng với `[u64; 4]` phần đệm dự trữ — **không có** trường `seconds_per_liquidity_cumulative`, nên các dẫn xuất có trọng số theo thanh khoản của Uniswap không khả dụng on-chain.
