...
|---------|---------------------|-------|
| **AMM v4** | `[b"amm authority"]` | **Single PDA shared by all AMM v4 pools.** Its bump is mirrored on every `AmmInfo` as `nonce`. |
| **CPMM** | `[b"vault_and_lp_mint_auth_seed"]` | **Single PDA shared by all CPMM pools.** No pool-specific seed. |
| **CLMM** | *(none — `pool_state` is itself the vault authority)* | Per-pool. Vaults are PDAs at `[b"pool_vault", pool_state, mint]`, owned by and signed for by `pool_state`. CLMM has no global vault-authority PDA. |
| **Farm v3 / v5 / v6** | `[farmId]` | Per-farm. The PDA owns staking + reward vaults for that farm. |
| **LaunchLab** | `[b"vault_auth_seed"]` | **Single PDA shared by all launches.** Two further global PDAs, `[b"platform_fee_vault_auth_seed"]` and `[b"creator_fee_vault_auth_seed"]`, own the aggregated fee vaults. |

A few things follow from this:

- For AMM v4, CPMM and LaunchLab the authority PDA is **a global account** — every pool or launch of that type uses it. If you are CPI-ing into CPMM you need it once, not per pool.
- CLMM is the exception: there is no authority PDA at all. Each vault is a PDA seeded on the pool and the mint, and the pool account signs for it.
- For per-farm authorities, you derive the PDA from the farm ID. The SDK does this in `getPoolKeys` / `getFarmKeys`; if you are integrating directly, you derive with `findProgramAddressSync`.
...
2. **Protocol/fund/creator portions are deducted from that LP-side accrual into per-pool counter accounts.** They sit on the pool state (`protocol_fees_token{0,1}`, `fund_fees_token{0,1}`, etc.) until somebody calls the corresponding collection instruction. They do not leave the pool's vaults until then; from a swap's perspective they are still "in the pool".
3. **Collection moves them out.** Protocol and fund paths require the respective `protocol_owner` / `fund_owner` signer from `AmmConfig`. CPMM creator fees use either the creator-signed `CollectCreatorFee` path or `CollectCreatorFeePermissionless`, which any payer can trigger but which fixes the destinations to the creator's canonical ATAs.
