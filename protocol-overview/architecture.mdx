...
3. **Token launch** — **LaunchLab**, a bonding-curve program. Newly initialized launches **graduate** into CPMM. The legacy AMM v4 migration instruction remains for existing launch state.
...
- **New LaunchLab pools graduate to CPMM.** Initialization now requires `migrate_type = CPSWAP`. `MigrateToAmm` remains for existing legacy state. Before the 2026-08-17 upgrade, CPMM migration locked `creator_scale` separately for the creator. Migrations executed afterward combine it with `platform_scale` into one platform-owned locked-LP position. Earlier Fee Keys remain unchanged.
