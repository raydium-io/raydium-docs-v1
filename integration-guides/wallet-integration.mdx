...
3. Enumerate the wallet's positions with `raydium.clmm.getOwnerPositionInfo({ programId })` and match on `nftMint` (there is no `getPositionInfo`). Each entry gives you:
...
The three modules do **not** share a signature: `liquidity.getPoolInfoFromRpc` takes `{ poolId }`, while `cpmm.getPoolInfoFromRpc` and `clmm.getPoolInfoFromRpc` take a positional base58 string.
...
// `poolInfo.lpMint` is an ApiV3Token and has no `supply` field. Read the LP mint's
// supply from the chain, or use the pool's own mirror:
...
// getAmountsFromLiquidity is async, takes epochInfo + poolInfo + add,
// and returns fee-wrapped amounts.
...
// amountA.amount / amountB.amount are the BNs; .fee is the Token-2022 transfer fee.
...
// Pending rewards — a CLMM pool has 0–3 reward streams. The position's
// rewardInfos entries are { growthInsideLastX64, rewardAmountOwed } only, so the
// mint has to come from the POOL's rewardInfos at the same index.
