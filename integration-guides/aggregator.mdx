...
<Warning>
  The three products do **not** share one signature. `computeAmountOut` exists only on
  `raydium.liquidity` (AMM v4) and as `PoolUtils.computeAmountOut(Format)` for CLMM; CPMM's
  equivalent is called `computeSwapAmount` and takes different parameters. Write the three calls
  out explicitly rather than parameterizing over the product.
</Warning>
...
For aggregator comparison use `amountOut` (pre-slippage) from each.
...
For a single pool, call that pool type's own swap builder — `raydium.liquidity.swap`, `raydium.cpmm.swap` or `raydium.clmm.swap`. `raydium.tradeV2.swap` is the *multi-hop route* executor and takes a completely different shape (`{ swapInfo, swapPoolKeys, routeProgram, ownerInfo, txVersion }`); there is no `raydium.trade`.
